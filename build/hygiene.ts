...
const v3CodeDistributionHeaderLines = [
	'/* Part of V3Code, distributed by KandD Labs LLC.',
	' * Existing copyright and license notices remain applicable.',
	' */',
];
...
export function hygiene(some: NodeJS.ReadWriteStream | string[] | undefined, runEslint = true): NodeJS.ReadWriteStream {
	...
	const productJson = es.through(function (file: VinylFile) {
		...
		if (product.extensionsGallery && product.nameShort !== 'V3Code') {
	...
	const copyrights = es.through(function (file: VinylFileWithLines) {
		...
		const matches = (expected: string[]) => expected.every((line, index) => lines[index] === line);

		if (!matches(copyrightHeaderLines) && !matches(v3CodeDistributionHeaderLines)) {
			console.error(file.relative + ': Missing or bad copyright or distribution statement');
			errorCount++;
		}
...
if (import.meta.main) {
	...
	if (process.argv.length > 2) {
		...
		cp.exec(
			// Deleted paths have no index blob to lint. Excluding them also keeps a
			// locally recreated, ignored path from making `git show :path` fail.
