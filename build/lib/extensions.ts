...
/**
 * VSCE can return a symlink to a directory as a package file. This is common in git worktrees
 * whose extension dependencies are shared through node_modules symlinks; createReadStream then
 * fails with EISDIR after every extension has already been bundled. Follow symlinks with stat,
 * but only turn actual files into vinyl entries.
 */
function toExtensionFiles(extensionPath: string, fileNames: string[]): File[] {
...
function fromLocalNormal(extensionPath: string): Stream {
	...
	vsce.listFiles({ cwd: extensionPath, packageManager: vsce.PackageManager.Npm })
		.then(fileNames => {
			const files = toExtensionFiles(extensionPath, fileNames);
