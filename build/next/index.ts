...
const desktopResourcePatterns = [
	...
	// V3Code/Void runtime assets loaded via FileAccess.asBrowserUri() (not ESM-imported),
	// so the min bundler won't fold them in — copy explicitly or they 404 at runtime
	// (v3codeGreyChromeContribution.ts references v3.css + the v3code_mark logo).
	'vs/workbench/contrib/void/browser/media/*.css',
	'vs/workbench/contrib/void/browser/media/*.svg',
	// Raster brand art too: the agent-welcome devil is a PNG, and listing only *.svg here
	// meant every packaged release shipped without it (blank welcome icon + 288 console
	// ERR_FILE_NOT_FOUND per session). Dev builds hid it — they read from source.
	'vs/workbench/contrib/void/browser/media/*.png',
	'vs/workbench/contrib/void/browser/media/*.gif',
	'vs/workbench/browser/parts/editor/media/v3code_mark.svg',
	...
	await Promise.all(allEntryPoints.map(async (entryPoint) => {
		...
		const buildOptions: esbuild.BuildOptions = {
			...
			logOverride: {
				...
				// CSS parser warnings can truncate the rest of a stylesheet while still producing a
				// bundle. A release build must fail instead of shipping partially-applied UI styles.
