	...
	private updateBackgroundColor(windowId: number, splash: IPartsSplash): void {
		for (const window of getAllWindowsExcludingOffscreen()) {
			if (window.id === windowId) {
				// Agents glass windows use native macOS vibrancy. Splash must not
				// paint an opaque Electron background over that material or the
				// desktop never shows through (CSS transparency alone is not enough).
				if ((window as electron.BrowserWindow & { __v3AgentsGlass?: boolean }).__v3AgentsGlass) {
