...
suite('AgentService (node dispatcher)', () => {
	...
	test('slash trigger is advertised by both initialization and direct API', async () => {
		assert.ok(service.completionTriggerCharacters.includes('/'));
		assert.deepStrictEqual(await service.getCompletionTriggerCharacters(), service.completionTriggerCharacters);
		assert.strictEqual(new Set(service.completionTriggerCharacters).size, service.completionTriggerCharacters.length);
	});
