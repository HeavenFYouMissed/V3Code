...
import { getBundledClaudeSdkPath } from '../node/claudeSdkLocation.js';
...
export class ElectronAgentHostStarter extends Disposable implements IAgentHostStarter {
	...
	async start(): Promise<IAgentHostConnection> {
		...
		// V3Code bundles `@anthropic-ai/claude-agent-sdk` as a production
		// dependency, so the Claude agent is ON by default via the bundled copy.
		// Precedence: the setting (now an override for local SDK testing) wins,
		// then an env var already set on the parent process (developer override),
		// then the bundled package. A build without the SDK resolves nothing and
		// the provider simply does not register.
		const claudeSdkPath = this._configurationService.getValue<string>(AgentHostClaudeAgentSdkPathSettingId)
			...
			|| getBundledClaudeSdkPath(this._environmentMainService.appRoot)
			|| '';
