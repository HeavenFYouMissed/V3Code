...
export function isAgentHostEnabled(configurationService: IConfigurationService): boolean {
	// Main starts before the workbench registers its desktop default. Treat an absent
	// value consistently with that default; an explicit opt-out still wins everywhere.
	return !isWeb && (configurationService.getValue<boolean>(AgentHostEnabledSettingId) ?? true);
}
...
/**
 * Configuration key that OVERRIDES the path to the `@anthropic-ai/claude-agent-sdk`
 * package (for local SDK testing). V3Code bundles the SDK as a production
 * dependency, so by default the starters resolve the bundled copy and the
 * Claude provider registers without any setting. When non-empty, this path
 * wins over the bundled copy and the SDK module is loaded via dynamic
 * `import()` from it. The agent host process must be restarted for changes
 * to take effect.
 */
export const AgentHostClaudeAgentSdkPathSettingId = 'chat.agentHost.claudeAgent.path';

/**
 * Environment variable that holds the absolute path to the
 * `@anthropic-ai/claude-agent-sdk` package the agent host loads. Set by the
 * agent host starters — precedence: {@link AgentHostClaudeAgentSdkPathSettingId}
 * (override) > a value already present on the parent process env (developer
 * override) > the bundled production copy (the default; see
 * `node/claudeSdkLocation.ts`). Non-empty -> the Claude provider registers.
 */
...
/**
 * V3Code hub resource. When the editor is signed in to a V3Code account, the
 * renderer authenticates this resource with the account bearer token; the Claude
 * agent then routes inference through the hub's Anthropic passthrough via
 * `directHubAuth` (bypassing the GitHub Copilot proxy). See
 * docs/V3CODE-AGENTS-BACKEND-SCOPE.md for the full wiring.
 */
export const V3CODE_HUB_PROTECTED_RESOURCE: ProtectedResourceMetadata = {
	resource: 'https://app.v3code.dev/hub',
	resource_name: 'V3Code Hub',
	authorization_servers: ['https://app.v3code.dev/login'],
	scopes_supported: [],
	required: true,
};
...
export interface IAgent {
	/** Session-specific commands supplied by this backend, not editor commands. */
