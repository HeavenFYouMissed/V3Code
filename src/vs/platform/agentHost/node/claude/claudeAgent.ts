...
import { AgentProvider, AgentSession, AgentSignal, GITHUB_COPILOT_PROTECTED_RESOURCE, V3CODE_HUB_PROTECTED_RESOURCE, IAgent, IAgentCreateSessionConfig, IAgentCreateSessionResult, IAgentDescriptor, IAgentMaterializeSessionEvent, IAgentModelInfo, IAgentResolveSessionConfigParams, IAgentSessionConfigCompletionsParams, IAgentSessionMetadata, IAgentSessionProjectInfo } from '../../common/agentService.js';
...
import { IDirectHubAuth } from './claudeSdkOptions.js';
...
/**
 * V3Code hub model catalog (provider-agnostic seeds). When the agent is
 * authenticated to the V3Code hub (not GitHub Copilot), the model picker is
 * populated from this static list instead of the Copilot models endpoint. The
 * `id`s are forwarded verbatim to the hub's Anthropic passthrough, so they must
 * be valid Anthropic model ids. TODO(agents-backend): source this from the hub
 * so the catalog tracks entitlement/tiers instead of being hardcoded.
 */
const V3CODE_HUB_MODEL_SEEDS: readonly Omit<IAgentModelInfo, 'provider'>[] = [
	{ id: 'claude-sonnet-4-6', name: 'Claude Sonnet 4.6', supportsVision: true, maxContextWindow: 1_000_000 },
	{ id: 'claude-opus-4-8', name: 'Claude Opus 4.8', supportsVision: true, maxContextWindow: 1_000_000 },
	// Dated snapshot id: editor-wide convention. The hub's agent allowlist and wire
	// rates keep the dateless alias too (old persisted sessions may still send it).
	{ id: 'claude-haiku-4-5-20251001', name: 'Claude Haiku 4.5', supportsVision: true, maxContextWindow: 200_000 },
];
...
export class ClaudeAgent extends Disposable implements IAgent {
	...
	/**
	 * V3Code hub auth: when the renderer authenticates the V3Code hub resource,
	 * this holds the account bearer token. Presence of a hub token means the
	 * session routes inference through the hub's Anthropic passthrough
	 * (`directHubAuth`) instead of the GitHub Copilot loopback proxy.
	 */
	private _hubToken: string | undefined;
	...
	getProtectedResources(): ProtectedResourceMetadata[] {
		return [GITHUB_COPILOT_PROTECTED_RESOURCE, V3CODE_HUB_PROTECTED_RESOURCE];
	}
	...
	/**
	 * The auth context handed to a materializing session. V3Code hub mode (a hub
	 * token is present) routes the SDK straight at the hub via `directHubAuth` and
	 * needs no proxy; otherwise fall back to the GitHub Copilot loopback proxy.
	 */
	/**
	 * Base URL for the V3Code hub's Anthropic passthrough. The Claude Agent SDK
	 * appends `/v1/messages`, so this points at `<hub>/api/inference/anthropic`
	 * (the hub route is `/api/inference/anthropic/v1/messages`). product.json
	 * `hubApiUrl` is the source of truth (the agent host utility process gets the
	 * full product.json via `_VSCODE_PRODUCT_JSON`). The Railway origin is only a
	 * fallback for builds missing the field.
	 */
	private get _hubInferenceBaseUrl(): string {
		const hubApiUrl = this._productService.hubApiUrl ?? 'https://backend-production-fc598.up.railway.app';
		return `${hubApiUrl}/api/inference/anthropic`;
	}

	private _materializeAuth(): { proxyHandle?: IClaudeProxyHandle; directHubAuth?: IDirectHubAuth } {
		if (this._hubToken) {
			return { directHubAuth: { baseUrl: this._hubInferenceBaseUrl, authToken: this._hubToken } };
		}
		return { proxyHandle: this._ensureAuthenticated() };
	}

	async authenticate(resource: string, token: string): Promise<boolean> {
		// V3Code hub: store the account bearer token. No loopback proxy is started
		// here — the session points the SDK straight at the hub's Anthropic
		// passthrough via `directHubAuth`, so there is no GitHub dependency.
		if (resource === V3CODE_HUB_PROTECTED_RESOURCE.resource) {
			this._hubToken = token;
			this._logService.info('[Claude] V3Code hub auth token updated');
	...
	private async _refreshModels(): Promise<void> {
		// V3Code hub mode: no GitHub Copilot models endpoint. Publish the static
		// V3Code catalog so the picker is populated for signed-in hub users.
		if (this._hubToken) {
			this._models.set(V3CODE_HUB_MODEL_SEEDS.map(seed => ({ ...seed, provider: this.id })), undefined);
			return;
		}
	...
	async createSession(config: IAgentCreateSessionConfig = {}): Promise<IAgentCreateSessionResult> {
		// Require either V3Code hub auth or the GitHub Copilot proxy before creating a session.
		if (!this._hubToken) {
	...
	private async _materializeProvisional(sessionId: string): Promise<ClaudeAgentSession> {
		...
		const auth = this._materializeAuth();
		...
		try {
			await session.materialize({ ...auth, canUseTool, isResume: false });
		} catch (err) {
	...
	private async _resumeSession(sessionId: string, sessionUri: URI): Promise<ClaudeAgentSession> {
		...
		try {
			await session.materialize({ ...auth, canUseTool, isResume: true });
		} catch (err) {
			this._sessions.deleteAndDispose(sessionId);
			throw err;
		}
