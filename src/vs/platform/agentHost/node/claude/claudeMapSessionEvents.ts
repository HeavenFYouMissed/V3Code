...
function mapResult(
	...
	if (message.subtype === 'success') {
		...
		// Error-subtype result (`error_during_execution`, `error_max_turns`, ...):
		// without this branch the turn maps to zero signals while the pipeline
		// still counts the `result` as completion, so the transcript stayed
		// blank. End the turn visibly instead — the reducer maps SessionError to
		// TurnState.Error and both render paths already display it. Never
		// surface `total_cost_usd` or any dollar figure here; the
		// `error_max_budget_usd` detail strings can embed internal dollars, so
		// that subtype always gets the generic message.
		const errs = message.errors ?? [];
		const detail = (message.subtype === 'error_max_budget_usd' || errs.length === 0)
			? localize('claude.turnFailedNoDetail', "The agent turn failed ({0}).", message.subtype)
			: errs.join('; ');
		signals.push({
			kind: 'action',
			session,
			action: {
				...
				turnId,
				error: { errorType: message.subtype, message: detail },
			},
		});
	}
