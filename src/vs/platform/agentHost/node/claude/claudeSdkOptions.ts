 ...
 */
/**
 * V3Code direct-hub auth: point the SDK at the V3Code hub's Anthropic-passthrough
 * (`<hub>/api/inference/anthropic`) with the account token as the bearer, bypassing the
 * GitHub-Copilot proxy + CAPI entirely. When present, this overrides the proxy base URL /
 * auth token so the agent runs on OUR platform key with NO GitHub dependency.
 */
export interface IDirectHubAuth {
...
export async function buildOptions(
	...
	directHubAuth?: IDirectHubAuth,
): Promise<Options> {
	const subprocessEnv = buildSubprocessEnv();
	const resolvedRgDiskPath = await rgDiskPath();
	// V3Code hub mode supplies `directHubAuth` and no proxy; the Copilot path
	// supplies a `proxyHandle` (loopback) and no directHubAuth. Exactly one is set.
	const baseUrl = directHubAuth?.baseUrl ?? proxyHandle?.baseUrl;
	const authToken = directHubAuth?.authToken ?? (proxyHandle ? `${proxyHandle.nonce}.${input.sessionId}` : undefined);
	if (!baseUrl || !authToken) {
		throw new Error('buildOptions: neither directHubAuth nor a proxy handle was provided');
	}
