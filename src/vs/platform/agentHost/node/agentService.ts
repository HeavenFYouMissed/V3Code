...
export class AgentService extends Disposable implements IAgentService {
	...
	/** Providers removed via {@link unregisterProvider}; sessions with their scheme must not fall back to the default provider. */
	private readonly _removedProviders = new Set<AgentProvider>();
	...
	/** Subscriptions to provider progress events, one store per provider so a provider can be unregistered on its own. */
	 ...
	 */
	get completionTriggerCharacters(): readonly string[] { return [...new Set([...this._completions.triggerCharacters, '/'])]; }
	...
	registerProvider(provider: IAgent): void {
		...
		this._removedProviders.delete(provider.id);
	...
	/**
	 * Removes a previously registered provider. Live sessions owned by the
	 * provider are forgotten (the provider is expected to dispose them), the
	 * default provider is reassigned if needed and root state is republished.
	 */
	unregisterProvider(id: AgentProvider): void {
		...
		this._removedProviders.add(id);
		this._providerSubscriptions.deleteAndDispose(id);
		for (const [key, owner] of [...this._sessionToProvider]) {
			if (owner === id) {
				this._sessionToProvider.delete(key);
			}
		}
		// Drop the provider's sessions from live state so clients stop showing
		// them. Persisted transcripts are untouched; re-registering lists them again.
		for (const key of this._stateManager.getSessionUrisWithPrefix(`${id}:`)) {
			this._changesetCoordinator.onSessionDisposed(key);
			this._sideEffects.removeSubagentSessions(key);
			this._stateManager.deleteSession(key);
		}
	...
	async completions(params: CompletionsParams): Promise<CompletionsResult> {
		const provider = this._findProviderForSession(URI.parse(params.channel));
		if (provider?.completions && params.text.startsWith('/')) {
			return provider.completions(params);
		}
	...
	private _findProviderForSession(session: URI | string): IAgent | undefined {
		...
		if (schemeProvider) {
			if (this._removedProviders.has(schemeProvider)) {
				return undefined;
			}
