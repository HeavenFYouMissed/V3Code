...
import { AcpAgentRegistrar } from './acp/acpAgentRegistrar.js';
import { resolveAcpSdkPath } from './acp/acpSdkLocation.js';
import { AcpSdkService, IAcpSdkService } from './acp/acpSdkService.js';
import { ExternalAgentCatalogueService } from './acp/externalAgentCatalogueService.js';
...
async function startAgentHost(): Promise<void> {
	...
	try {
		...
		// V3Code ships without the GitHub Copilot agent provider (proprietary
		// @github/copilot-sdk cannot be redistributed; the module is stripped
		// from the packaged app via build/.moduleignore).
		// The Claude agent provider is ON by default: V3Code bundles
		// `@anthropic-ai/claude-agent-sdk` as a production dependency and the
		// starters resolve the bundled copy into `VSCODE_AGENT_HOST_CLAUDE_SDK_PATH`
		// (a user-set `chat.agentHost.claudeAgent.path` or a pre-set env var still
		// wins — see claudeSdkLocation.ts). An empty env var means this build has
		// no SDK on disk, so the provider does not register.
		if (process.env[AgentHostClaudeSdkPathEnvVar]) {
			agentService.registerProvider(instantiationService.createInstance(ClaudeAgent));
		}
		// External agents speaking the Agent Client Protocol (ACP). The SDK is
		// bundled the same way as the other agent SDK above; without it on disk
		// no external provider registers. Providers come and go with the user's catalogue file,
		// which the catalogue service watches (see ExternalAgentCatalogueService).
		const acpSdkPath = resolveAcpSdkPath(environmentService.appRoot, process.env);
		if (acpSdkPath) {
			diServices.set(IAcpSdkService, new AcpSdkService(acpSdkPath, logService));
			const catalogueService = disposables.add(instantiationService.createInstance(ExternalAgentCatalogueService));
			const registrar = disposables.add(instantiationService.createInstance(AcpAgentRegistrar, agentService, catalogueService));
			const catalogue = await catalogueService.initialize();
			await registrar.apply(catalogue);
		} else {
			logService.info('[ACP] SDK not found on disk; external agent providers are unavailable');
		}
	} catch (err) {
