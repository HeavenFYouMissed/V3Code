...
export interface IActionListOptions {
	...
	/**
	 * Extra height (in px) inside the filter row beyond the default search box
	 * (e.g. V3Code mounts a toggle strip under the filter after open). Used so
	 * the first above/below decision and max-height math account for real chrome.
	 */
	readonly filterChromeHeight?: number;

	/**
	 * When true, prefer opening above the anchor (e.g. bottom-anchored chat composer).
	 */
	readonly preferShowAbove?: boolean;
...
export class ActionList<T> extends Disposable {
	...
	private computeHeight(): number {
		...
		const filterHeight = this._options?.filterChromeHeight ?? (this._widget.filterContainer ? 36 : 0);
		...
		if (this.hasDynamicHeight()) {
			...
			if (this._showAbove === undefined) {
				...
				// preferShowAbove is a preference, not a lock. Honoring it when the anchor sits
				// near the top of the window (the empty-chat composer is pinned to the top of the
				// panel) collapsed availableHeight to the 3-row floor, and layout2d then flipped
				// the popup below the anchor anyway — rendering it tiny AND cut off. Only go above
				// when the space above can hold at least as much of the list as the space below.
				const wantedHeight = Math.min(fullHeight, Math.floor(targetWindow.innerHeight * 0.6));
				this._showAbove = this._options?.preferShowAbove === true
					? (spaceAbove >= wantedHeight || spaceAbove > spaceBelow)
					: (fullHeight > spaceBelow && spaceAbove > spaceBelow);
			}
