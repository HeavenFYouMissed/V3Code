...
import type { IBrowserNetworkEntry } from '../common/playwrightService.js';
...
export class PlaywrightTab {
	...
	private _nextDialogResponse: { accept: boolean; promptText?: string; expiresAt: number } | undefined;
	...
	private _networkCapturePattern: RegExp | undefined;
	private _networkCaptureBodies = false;
	private readonly _networkLog: IBrowserNetworkEntry[] = [];
	...
	constructor(
		...
		page.on('console', event => this._handleConsoleMessage(event))
			...
			.on('response', response => this._handleResponse(response))
	...
	private _handleDialog(dialog: playwright.Dialog) {
		this._dialog = dialog;
		const response = this._nextDialogResponse;
		this._nextDialogResponse = undefined;
		if (response && response.expiresAt > Date.now()) {
			void this.replyToDialog(response.accept, response.promptText).catch(error => {
				this._logs.push({ type: 'error', time: Date.now(), description: `Dialog response failed: ${String(error)}` });
			});
			return;
		}
		...
		}).catch(() => { /* The page may close while a modal is open. */ });
		this._onDialogStateChanged.fire();
	}

	async replyToDialog(accept?: boolean, promptText?: string) {
		if (!this._dialog) {
			this._nextDialogResponse = { accept: accept === true, promptText, expiresAt: Date.now() + 30_000 };
			return true;
		}
	...
	private _handleResponse(response: playwright.Response) {
		...
		if (resourceType === 'xhr' || resourceType === 'fetch' || resourceType === 'script') {
			const url = request.url();
			if (url.length > 300) {
				this._pushLog('network', `${request.method()} ${response.status()} ${url.slice(0, 300)}…`);
			} else {
				this._pushLog('network', `${request.method()} ${response.status()} ${url}`);
			}
		}
		if (this._networkCaptureEnabled && this._networkCapturePattern?.test(request.url())) {
			void this._captureNetworkEntry(response);
		}
	}

	private async _captureNetworkEntry(response: playwright.Response): Promise<void> {
		const request = response.request();
		const entry: IBrowserNetworkEntry = {
			time: Date.now(),
		...
		if (this._networkCaptureBodies) {
			try {
				entry.requestBody = request.postData() ?? undefined;
			} catch { /* ignore */ }
			try {
				const text = await response.text();
				entry.responseBody = text.length > 32_000 ? `${text.slice(0, 32_000)}…[truncated]` : text;
			} catch { /* ignore */ }
		}
		this._networkLog.push(entry);
		const MAX = 500;
		if (this._networkLog.length > MAX) {
			this._networkLog.splice(0, this._networkLog.length - MAX);
		}
	}

	enableNetworkCapture(urlPattern: string, includeBodies: boolean): void {
		this._networkCapturePattern = new RegExp(urlPattern, 'i');
		this._networkCaptureBodies = includeBodies;
		this._networkCaptureEnabled = true;
	}

	getNetworkLog(clear: boolean): readonly IBrowserNetworkEntry[] {
		const copy = [...this._networkLog];
		if (clear) {
			this._networkLog.length = 0;
		}
		return copy;
	}

	private _pushLog(type: string, description: string) {
		this._logs.push({ type, time: Date.now(), description });
		const MAX = 250;
		if (this._logs.length > MAX) {
			this._logs.splice(0, this._logs.length - MAX);
		}
	}
