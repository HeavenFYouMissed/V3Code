...
import { IInvokeFunctionResult, IBrowserNetworkEntry, IBrowserStorageState, IPlaywrightService } from '../common/playwrightService.js';
...
export class PlaywrightService extends Disposable implements IPlaywrightService {
	...
	async enableNetworkCapture(sessionId: string, pageId: string, urlPattern: string, includeBodies: boolean): Promise<void> {
		const session = await this._getOrCreateSession(sessionId);
		return session.enableNetworkCapture(pageId, urlPattern, includeBodies);
	}

	async getNetworkLog(sessionId: string, pageId: string, clear = false): Promise<readonly IBrowserNetworkEntry[]> {
		const session = await this._getOrCreateSession(sessionId);
		return session.getNetworkLog(pageId, clear);
	}

	async exportStorageState(sessionId: string, pageId: string): Promise<IBrowserStorageState> {
		const session = await this._getOrCreateSession(sessionId);
		return session.exportStorageState(pageId);
	}

	async importStorageState(sessionId: string, pageId: string, state: IBrowserStorageState): Promise<void> {
		const session = await this._getOrCreateSession(sessionId);
		return session.importStorageState(pageId, state);
	}
...
class PlaywrightSession extends Disposable {
	...
	async replyToDialog(pageId: string, accept: boolean, promptText?: string): Promise<{ summary: string }> {
		...
		const armed = await tab.replyToDialog(accept, promptText);
		if (armed) { return { summary: 'Response armed for the next dialog on this page for 30 seconds. Trigger the dialog now. Electron pages may not support prompt dialogs.' }; }
		const summary = await tab.getSummary();
		return { summary };
	}

	async enableNetworkCapture(pageId: string, urlPattern: string, includeBodies: boolean): Promise<void> {
		const tab = await this._getTab(pageId);
		tab.enableNetworkCapture(urlPattern, includeBodies);
	}

	async getNetworkLog(pageId: string, clear: boolean): Promise<readonly IBrowserNetworkEntry[]> {
		const tab = await this._getTab(pageId);
		return tab.getNetworkLog(clear);
	}

	async exportStorageState(pageId: string): Promise<IBrowserStorageState> {
		const page = await this._getPage(pageId);
		const state = await page.context().storageState();
		return state as IBrowserStorageState;
	}

	async importStorageState(pageId: string, state: IBrowserStorageState): Promise<void> {
		const page = await this._getPage(pageId);
	...
	private async _getTab(pageId: string): Promise<PlaywrightTab> {
		const page = await this._getPage(pageId);
		const tab = this._tabs.get(page);
		if (!tab) {
			throw new Error('Failed to get page tab');
		}
		return tab;
	}
	...
	private async _runWithDeferral(pageId: string, callback: (page: Page) => Promise<unknown>, timeoutMs: number, existingDeferredId?: string): Promise<IInvokeFunctionResult> {
		...
		const summary = await raceTimeout(this._getSummary(pageId), 5_000) ?? 'Page snapshot timed out. The action may still be running; use its deferred result ID before retrying a mutation.';
