...
/** Dark slim scrollbars for integrated browser pages (Windows default track is bright white). */
const V3_BROWSER_PAGE_SCROLLBAR_CSS = `
html {
...
export class BrowserView extends Disposable {
	...
	private _pageScrollbarCssKey: string | undefined;
	...
	constructor(
		...
		// Clear — Agents workplace glass / device frame must not flash a white island.
		this._view.setBackgroundColor('#00000000');

		// Present a clean, standard UA to sites — strip the `Electron/x.y.z` and app-product
		// (`v3code/…`) tokens so pages don't feature-detect us as a non-browser and serve
		// degraded/blocked variants. Must run BEFORE the emulator is created (it snapshots the
		// current UA as its default and restores to it when emulation exits).
		try {
			const rawUA = this._view.webContents.getUserAgent();
			const cleanedUA = rawUA
				.replace(/\s*Electron\/\S+/i, '')
				.replace(/\s*v3code\/\S+/i, '')
				.replace(/\s{2,}/g, ' ')
				.trim();
			if (cleanedUA && cleanedUA !== rawUA) {
				this._view.webContents.setUserAgent(cleanedUA);
			}
		} catch (err) {
			this.logService.trace('[browserView] user agent clean failed', err);
		}
	...
	private setupEventListeners(): void {
		...
		webContents.on('did-finish-load', () => {
			fireLoadingEvent(false);
			void this._injectPageScrollbarStyles();
		});
	...
	/**
	 * Dark slim scrollbars inside loaded pages so the native Windows track does not
	 * bleed over the rounded browser chrome border. WINDOWS ONLY: on macOS this CSS
	 * kills the native overlay scrollbars, forcing an 8px layout gutter + a reflow on
	 * every page load. Remove/insert are split so a stale-key removal failure cannot
	 * skip the (re)insert.
	 */
	private async _injectPageScrollbarStyles(): Promise<void> {
		if (!isWindows) {
			return;
		}
		const webContents = this._view.webContents;
		...
		if (this._pageScrollbarCssKey) {
			try {
				await webContents.removeInsertedCSS(this._pageScrollbarCssKey);
			} catch (err) {
				this.logService.trace('[browserView] page scrollbar CSS remove failed', err);
			}
			this._pageScrollbarCssKey = undefined;
		}
		try {
			this._pageScrollbarCssKey = await webContents.insertCSS(V3_BROWSER_PAGE_SCROLLBAR_CSS);
		} catch (err) {
			this.logService.trace('[browserView] page scrollbar CSS inject failed', err);
		}
	}
	...
	layout(bounds: IBrowserViewBounds): void {
		...
		// Round the EDGES, then derive the size from them. Rounding position and size
		// independently puts the right edge at round(x*z) + round(w*z), which is not
		// round((x+w)*z): when both round up, the far edge overshoots by up to a pixel.
		// x and y are pinned by their own rounding, so the error lands entirely on the
		// right and bottom — the native view spills past the container onto the sash and
		// covers the sharing ring, which is drawn 2px outside the container box and cannot
		// win against a native WebContentsView at any z-index. containerLeft is routinely
		// fractional (the wrapper centres the container with a /2), so this is the common
		// case rather than an edge case.
		const zoom = bounds.zoomFactor;
		const left = Math.round(bounds.x * zoom);
		const top = Math.round(bounds.y * zoom);
		const right = Math.round((bounds.x + bounds.width) * zoom);
		const bottom = Math.round((bounds.y + bounds.height) * zoom);
	...
	/** Run JavaScript in the page's isolated world (shared DOM) and return its result. Used by
	 *  Visual Edit to inject its in-page panel and apply live edits. */
