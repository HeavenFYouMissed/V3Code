...
export interface IBrowserStorageState {
...
export interface IPlaywrightService {
	...
	/** Start capturing network requests matching a URL pattern (regex). */
	enableNetworkCapture(sessionId: string, pageId: string, urlPattern: string, includeBodies: boolean): Promise<void>;

	/** Return captured network entries; optionally clear the buffer. */
	getNetworkLog(sessionId: string, pageId: string, clear?: boolean): Promise<readonly IBrowserNetworkEntry[]>;

	/** Export cookies + localStorage origins for persistence. */
	exportStorageState(sessionId: string, pageId: string): Promise<IBrowserStorageState>;

	/** Apply a previously exported storage state (reload page afterward). */
	importStorageState(sessionId: string, pageId: string, state: IBrowserStorageState): Promise<void>;
}
