...
export class DarwinUpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	protected override async doDownloadUpdate(state: AvailableForDownload): Promise<void> {
		...
		// The feed must carry the RUNNING build's commit — the update server 204s
		// when the requested commit equals the manifest version, so passing the
		// update's own commit (state.update.version) would answer "no update"
		// right after the user clicked Download.
