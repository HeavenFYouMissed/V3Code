...
export interface ILifecycleMainService {
	...
	/**
	 * Close a window after running its unload lifecycle. Returns whether the close was vetoed.
	 */
	close(window: ICodeWindow): Promise<boolean /* veto */>;
...
export class LifecycleMainService extends Disposable implements ILifecycleMainService {
	...
	private readonly mapWindowIdToPendingClose = new Map<number, Promise<boolean>>();
	...
	registerWindow(window: ICodeWindow): void {
		...
		windowListeners.add(Event.fromNodeEventEmitter<electron.Event>(win, 'close')(e => {
			...
			// Otherwise prevent unload and handle it through the lifecycle service.
			e.preventDefault();
			void this.close(window);
		}));
	...
	close(window: ICodeWindow): Promise<boolean /* veto */> {
		const pendingClosePromise = this.mapWindowIdToPendingClose.get(window.id);
		if (pendingClosePromise) {
			return pendingClosePromise;
		}

		const closePromise = this.doClose(window).finally(() => {
			this.mapWindowIdToPendingClose.delete(window.id);
		});
		this.mapWindowIdToPendingClose.set(window.id, closePromise);

		return closePromise;
	}

	private async doClose(window: ICodeWindow): Promise<boolean /* veto */> {
		const veto = await this.unload(window, UnloadReason.CLOSE);
		if (veto) {
			this.windowToCloseRequest.delete(window.id);
			return true;
		}

		this.windowToCloseRequest.add(window.id);

		// Fire onBeforeCloseWindow before actually closing
		this.trace(`Lifecycle#onBeforeCloseWindow.fire() - window ID ${window.id}`);
		this._onBeforeCloseWindow.fire(window);

		// No veto, close window now. The registered close listener consumes
		// windowToCloseRequest so this does not start another unload cycle.
		window.close();
		return false;
	}
