...
export class AuxiliaryWindow extends BaseWindow implements IAuxiliaryWindow {
	...
	private doTryClaimWindow(options?: BrowserWindowConstructorOptions): void {
		...
		if (window) {
			...
			// Disable Menu
			window.setMenu(null);
			if (isMacintosh && options?.transparent && options.vibrancy) {
				const material = options.vibrancy === 'appearance-based' ? 'fullscreen-ui' : options.vibrancy;
				const pinGlass = () => {
					try {
						window.setBackgroundColor('#00000000');
						window.setVibrancy(material);
					} catch (error) {
						this.logService.warn('[aux window] Failed to restore transparent material', error);
					}
				};
				pinGlass();
				window.once('ready-to-show', pinGlass);
				window.on('focus', pinGlass);
				window.on('blur', () => {
					try {
						window.setBackgroundColor('#00000000');
					} catch { /* ignore */ }
