...
export class AuxiliaryWindowsMainService extends Disposable implements IAuxiliaryWindowsMainService {
	...
	createWindow(details: HandlerDetails): BrowserWindowConstructorOptions {
		const { state, overrides, agentWorkspace } = this.computeWindowStateAndOverrides(details);
		...
		if (agentWorkspace) {
			// The Agent Workspace owns its complete workbench chrome. Force the
			// content beneath the native controls even when the user's ordinary
			// auxiliary windows are configured to use a native title bar.
			options.titleBarStyle = 'hidden';
		}
		if (agentWorkspace && isMacintosh) {
	...
	private computeWindowStateAndOverrides(details: HandlerDetails): { readonly state: IWindowState; readonly overrides: IDefaultBrowserWindowOptionsOverrides; readonly agentWorkspace: boolean } {
		const windowState: IWindowState = {};
		const overrides: IDefaultBrowserWindowOptionsOverrides = {};
		let agentWorkspace = false;
		...
		return { state, overrides, agentWorkspace };
	}
