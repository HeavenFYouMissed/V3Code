...
import { FocusMode, IAgentWorkspaceWindowSwapOptions, ICommonNativeHostService, INativeHostOptions, IOSProperties, IOSStatistics, IToastOptions, IToastResult, PowerSaveBlockerType, SystemIdleState, ThermalState } from '../common/native.js';
...
export class NativeHostMainService extends Disposable implements INativeHostMainService {
	...
	private pendingAgentWorkspaceWindowSwap: Promise<void> | undefined;
	private pendingAgentWorkspaceWindowSwapSourceWindowId: number | undefined;
	private readonly agentWorkspaceWindowPairs = new Map<number, number>();
	...
	constructor(
		...
		this._register(this.windowsMainService.onDidDestroyWindow(window => {
			// The other half of an IDE/Agents pair is hidden rather than closed, so if
			// the visible one is destroyed the app would be left running with nothing
			// on screen. Bring its partner back instead.
			//
			// NOT while quitting. Without this guard, closing the window you are looking
			// at resurrects the one you already dismissed: the app refuses to die, you
			// close it twice, and on Windows a surviving instance makes the next launch
			// open an EMPTY window instead of your folder (launchMainService prefers a
			// new window off macOS). Reviving a partner mid-teardown also races the
			// windows that are already being destroyed.
			const paired = this.lifecycleMainService.quitRequested ? undefined : this.pairedAgentWorkspaceWindow(window.id);
			if (paired?.win && !paired.win.isVisible()) {
				paired.win.show();
				paired.focus({ mode: FocusMode.Force });
			}
			this.unlinkAgentWorkspaceWindowPair(window.id);
		}));

		// The handler above only covers CRASHES. onDidDestroyWindow is fired from exactly one
		// place — CodeWindow.destroyWindow() — which is reachable only from the window-error
		// handlers; its own comment says "'close' event will not be fired on destroy(), so
		// signal crash via explicit event". A normal close fires onBeforeCloseWindow instead.
		//
		// So when the user clicked X on the visible half of a pair, NOTHING unlinked or closed
		// the hidden half. A hidden BrowserWindow is still a live window: windowCounter stays
		// > 0, so 'window-all-closed' never fires, the app never quits, and it keeps the
		// single-instance handle. The next Windows launch then connects to that ghost and,
		// because launchMainService prefers a new window off macOS, opens an EMPTY window
		// instead of the folder — which also means no workspace scope and therefore no memory.
		// That is the "V3Code won't open / opens blank / my memory is gone" report.
		this._register(this.lifecycleMainService.onBeforeCloseWindow(window => {
			const paired = this.pairedAgentWorkspaceWindow(window.id);
			// Unlink FIRST: closing the partner re-enters this handler, and an already-unlinked
			// pair terminates that recursion.
			this.unlinkAgentWorkspaceWindowPair(window.id);
			if (paired?.win && !paired.win.isVisible() && !paired.win.isDestroyed()) {
				// Closing the visible half closes the pair — the two halves are one workspace.
				// Show it before closing: a hidden window that prompts to save unsaved work
				// would put an invisible modal on screen and hang the quit instead.
				paired.win.show();
				paired.win.close();
			}
		}));
	...
	async swapAgentWorkspaceWindow(windowId: number | undefined, options: IAgentWorkspaceWindowSwapOptions): Promise<void> {
		this.logService.info(`[Agent Workspace] swap request source=${windowId ?? 'unknown'} target=${options.agentWorkspace ? 'agents' : 'ide'}`);
		while (this.pendingAgentWorkspaceWindowSwap) {
			const pendingSwap = this.pendingAgentWorkspaceWindowSwap;
			if (this.pendingAgentWorkspaceWindowSwapSourceWindowId === windowId) {
				return pendingSwap;
			}

			// A request from the window that the active swap just opened is a real
			// reverse transition (Agent Workspace -> IDE, or IDE -> Agent Workspace).
			// Queue it behind the current close instead of silently dropping it.
			await pendingSwap;
		}

		const swap = this.doSwapAgentWorkspaceWindow(windowId, options);
		this.pendingAgentWorkspaceWindowSwap = swap;
		this.pendingAgentWorkspaceWindowSwapSourceWindowId = windowId;
		try {
			await swap;
		} finally {
			if (this.pendingAgentWorkspaceWindowSwap === swap) {
				this.pendingAgentWorkspaceWindowSwap = undefined;
				this.pendingAgentWorkspaceWindowSwapSourceWindowId = undefined;
			}
		}
	}

	private async doSwapAgentWorkspaceWindow(windowId: number | undefined, options: IAgentWorkspaceWindowSwapOptions): Promise<void> {
		const source = this.codeWindowById(windowId);
		if (options.agentWorkspace && source) {
			const existingAgentWindow = this.pairedAgentWorkspaceWindow(source.id);
			if (existingAgentWindow?.config?.isAgentWorkspaceWindow) {
				if (options.sessionUri) {
					existingAgentWindow.sendWhenReady('vscode:openAgentWorkspace', CancellationToken.None, URI.revive(options.sessionUri).toString());
				}
				this.logService.info(`[Agent Workspace] reusing Agent window=${existingAgentWindow.id} for IDE source=${source.id}`);
				this.revealAgentWorkspaceWindow(existingAgentWindow, source);
				return;
			}
		}

		if (!options.agentWorkspace && source) {
			const existingIdeWindow = this.pairedAgentWorkspaceWindow(source.id);
			if (existingIdeWindow && !existingIdeWindow.config?.isAgentWorkspaceWindow) {
				this.logService.info(`[Agent Workspace] focusing paired IDE window=${existingIdeWindow.id} for Agent source=${source.id}`);
				this.revealAgentWorkspaceWindow(existingIdeWindow, source);
				return;
			}
		}

		const openable: IWindowOpenable | undefined = options.workspaceUri
			? { workspaceUri: URI.revive(options.workspaceUri) }
			: options.folderUri
				? { folderUri: URI.revive(options.folderUri) }
				: undefined;
		this.logService.info(`[Agent Workspace] opening ${options.agentWorkspace ? 'Agent companion' : 'clean IDE'} source=${windowId ?? 'unknown'} workspace=${openable ? 'configured' : 'empty'}`);
		const windowState = options.agentWorkspace
			? this.agentWorkspaceCompanionState(source)
			: source?.serializeWindowState();
		const windows = openable
			? await this.windowsMainService.open({
				context: OpenContext.API,
				contextWindowId: windowId,
				windowState,
				isAgentWorkspaceWindow: options.agentWorkspace,
				urisToOpen: [openable],
				cli: this.environmentMainService.args,
				forceNewWindow: true,
				forceDuplicateWorkspace: true,
			})
			: await this.windowsMainService.openEmptyWindow({
				context: OpenContext.API,
				contextWindowId: windowId,
		...
		this.logService.info(`[Agent Workspace] window open returned ${windows.length} window(s)`);
		const target = windows.at(0);
		if (!target) {
			this.logService.warn('[Agent Workspace] window open returned no target window');
			return;
		}

		this.logService.info(`[Agent Workspace] target=${target.id} source=${source?.id ?? 'missing'}`);
		if (options.agentWorkspace && options.sessionUri) {
			target.sendWhenReady('vscode:openAgentWorkspace', CancellationToken.None, URI.revive(options.sessionUri).toString());
		}

		// Switching between the IDE and Agents is a focus change, not a handoff.
		// Both windows stay alive and paired, so every later switch is an instant
		// focus instead of building a window, an extension host and a set of
		// language servers from scratch. This is only safe because the Agent window
		// no longer writes the shared workspace editor state that leaving used to
		// have to discard by destroying the window.
		if (source && source.id !== target.id) {
			this.linkAgentWorkspaceWindows(
				options.agentWorkspace ? source : target,
				options.agentWorkspace ? target : source
			);
		}

		this.revealAgentWorkspaceWindow(target, source);
	}

	/**
	 * Show one half of an IDE/Agents pair and hide the other. Hiding keeps the
	 * window, its renderer, its extension host and its language servers alive, so
	 * switching back is a focus rather than a rebuild, while still showing only
	 * one of the two at a time.
	 */
	private revealAgentWorkspaceWindow(target: ICodeWindow, source: ICodeWindow | undefined): void {
		target.win?.show();
	...
	private agentWorkspaceCompanionState(source: ICodeWindow | undefined): IWindowState | undefined {
		if (!source) {
			return undefined;
		}

		const sourceState = source.serializeWindowState();
		const sourceBounds = source.getBounds();
		const workArea = screen.getDisplayMatching(sourceBounds).workArea;
		const preferred = defaultWindowState(WindowMode.Normal, true);
		const width = Math.min(workArea.width, Math.max(sourceBounds.width, preferred.width ?? 1440));
		const height = Math.min(workArea.height, Math.max(sourceBounds.height, preferred.height ?? 900));
		const centerX = sourceBounds.x + (sourceBounds.width / 2);
		const centerY = sourceBounds.y + (sourceBounds.height / 2);
		const x = Math.round(Math.min(Math.max(centerX - (width / 2), workArea.x), workArea.x + workArea.width - width));
		const y = Math.round(Math.min(Math.max(centerY - (height / 2), workArea.y), workArea.y + workArea.height - height));
	...
	private linkAgentWorkspaceWindows(ide: ICodeWindow, agent: ICodeWindow): void {
		this.unlinkAgentWorkspaceWindowPair(ide.id);
		this.unlinkAgentWorkspaceWindowPair(agent.id);
		this.agentWorkspaceWindowPairs.set(ide.id, agent.id);
		this.agentWorkspaceWindowPairs.set(agent.id, ide.id);
	}

	private pairedAgentWorkspaceWindow(windowId: number): ICodeWindow | undefined {
		const pairedWindowId = this.agentWorkspaceWindowPairs.get(windowId);
		if (pairedWindowId === undefined) {
			return undefined;
		}

		const pairedWindow = this.windowsMainService.getWindowById(pairedWindowId);
		if (!pairedWindow) {
			this.unlinkAgentWorkspaceWindowPair(windowId);
		}
		return pairedWindow;
	}

	private unlinkAgentWorkspaceWindowPair(windowId: number): void {
		const pairedWindowId = this.agentWorkspaceWindowPairs.get(windowId);
		this.agentWorkspaceWindowPairs.delete(windowId);
		if (pairedWindowId !== undefined && this.agentWorkspaceWindowPairs.get(pairedWindowId) === windowId) {
			this.agentWorkspaceWindowPairs.delete(pairedWindowId);
		}
	}
	...
	async saveWindowSplash(windowId: number | undefined, splash: IPartsSplash): Promise<void> {
		...
		// Agents glass: keep Electron's native background transparent and the window
		// material alive whenever splash refreshes from the renderer. Keyed off the
		// marker the window set at creation rather than re-deriving the platform
		// test, so this automatically covers whichever platforms actually took the
		// glass path (macOS vibrancy, Windows 11 DWM backdrop — see windowImpl).
		const glassWindow = window?.win as (BrowserWindow & { __v3AgentsGlass?: boolean }) | undefined;
		if (glassWindow?.__v3AgentsGlass) {
			splash = {
				...splash,
				colorInfo: {
					...splash.colorInfo,
			...
			try {
				glassWindow.setBackgroundColor('#00000000');
				if (isMacintosh) {
					glassWindow.setVibrancy('fullscreen-ui');
				} else {
					glassWindow.setBackgroundMaterial('acrylic');
				}
			} catch {
				// ignore — theme splash still proceeds with transparent bg
			}
		}
