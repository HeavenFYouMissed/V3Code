...
export class ExtensionManagementService extends AbstractExtensionManagementService implements INativeServerExtensionManagementService {
	...
	private async downloadExtension(extension: IGalleryExtension, operation: InstallOperation, verifySignature: boolean, clientTargetPlatform?: TargetPlatform): Promise<{ readonly location: URI; readonly verificationStatus: ExtensionSignatureVerificationCode | undefined }> {
		if (verifySignature) {
			...
			// The default for this key is registered by a WORKBENCH contribution, which the CLI
			// process never loads — so `code --install-extension` saw `undefined` here and the
			// hardcoded fallback decided it, demanding a signature no matter what the product
			// said. Against Open VSX, which does not sign extensions and reports "verification
			// was not executed" rather than the NotSigned status the tolerance below forgives,
			// that made every CLI install fail outright.
			//
			// product.json already declares this default; honour it before assuming true. This
			// is not a new weakening: it lowers the bar only where the product explicitly asked
			// for it, and a product that ships a signing gallery still gets verification.
			const declared = (this.productService as { configurationDefaults?: Record<string, unknown> }).configurationDefaults?.[VerifyExtensionSignatureConfigKey];
			verifySignature = isBoolean(value) ? value : isBoolean(declared) ? declared : true;
		}
