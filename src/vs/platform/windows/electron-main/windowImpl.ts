...
export interface IWindowCreationOptions {
	...
	readonly isAgentWorkspaceWindow?: boolean;
}
...
export abstract class BaseWindow extends Disposable implements IBaseWindow {
	...
	private doFocusWindow() {
		...
		// A hidden window must be shown before it can take focus: on Win32, focusing a
		// window without WS_VISIBLE is a no-op, so it simply never appears. Upstream
		// never hits this because it never hides a window — but the Agent Workspace
		// swap hides the other half of the pair instead of closing it, and both halves
		// are open on the SAME folder. Reopening that folder can match the hidden half
		// (windowsFinder does not filter on visibility), and without this the open
		// request "succeeds" while nothing comes up. The existing self-heal further
		// down is gated on !isBuilt, so it does not run in a shipped build.
		if (!win.isVisible()) {
			win.show();
		}
	...
	updateWindowControls(options: { height?: number; backgroundColor?: string; foregroundColor?: string; dimmed?: boolean }): void {
		...
		// macOS: update window controls via setWindowButtonPosition()
			...
			// A band shorter than the buttons themselves gives a negative offset, which
			// would place the traffic lights outside the window's top-left corner and
			// leave them half cut off and unclickable. A window shell that hides the
			// titlebar reports height 0, so treat anything non-positive as "no custom
			// position" and let the system place them.
			if (offset <= 0) {
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	constructor(
		...
		//#region create browser window
		{
			...
			if (config.isSessionsWindow || config.isAgentWorkspaceWindow) {
			...
			const isAgentsGlassWindow = !!(config.isSessionsWindow || config.isAgentWorkspaceWindow);
			// Windows 11 22H2 is the first build with DWM system backdrops. Below it
			// `backgroundMaterial` is silently a no-op, and a zero-alpha background
			// with nothing drawing behind it renders black — worse than the opaque
			// window — so the whole glass path stays off rather than degrading.
			const useWindowsAgentsGlass = isAgentsGlassWindow && isWindows && getWindowsBuildNumberSync() >= 22621;

			// Agents glass: let the desktop show through the frosted panels.
			// macOS composites per-pixel: `transparent` + vibrancy + a zero-alpha
			// background are all required; splash later tries to paint an opaque
			// theme bg (guarded elsewhere).
			if (isAgentsGlassWindow && isMacintosh) {
				options.transparent = true;
				options.backgroundColor = '#00000000';
				options.vibrancy = 'fullscreen-ui';
				// Always-on material avoids inactive/active vibrancy rebuild flash on Cmd-Tab.
				options.visualEffectState = 'active';
			} else if (useWindowsAgentsGlass) {
				// Windows draws its backdrop in DWM, behind the entire window including
				// the non-client area. Deliberately no `transparent: true` on this path:
				// there it switches the window to per-pixel alpha, which *replaces* the
				// system backdrop instead of layering over it, and additionally requires
				// a frameless window. The zero-alpha background is still required so the
				// workbench does not paint an opaque layer over the material.
			...
			if (isAgentsGlassWindow && (isMacintosh || useWindowsAgentsGlass)) {
				try {
					// This marker is the single signal the rest of the app uses to know
					// the window owns a native material, so splash and theme updates
					// leave its background alone (see themeMainServiceImpl).
					(this._win as electron.BrowserWindow & { __v3AgentsGlass?: boolean }).__v3AgentsGlass = true;
					const pinAgentsGlass = () => {
						try {
							this._win?.setBackgroundColor('#00000000');
							if (isMacintosh) {
								// Never setVibrancy(null) — that flashes dark then transparent.
								this._win?.setVibrancy('fullscreen-ui');
							} else {
								this._win?.setBackgroundMaterial('acrylic');
							}
						} catch (err) {
							this.logService.warn('window#agents-glass: failed to pin window material', err);
						}
					};
					pinAgentsGlass();
					this._win.once('ready-to-show', pinAgentsGlass);
					// Focus loss: pin clear bg only — do not tear down vibrancy.
					this._win.on('blur', () => {
						try {
							this._win?.setBackgroundColor('#00000000');
						} catch { /* ignore */ }
					});
					this._win.on('focus', pinAgentsGlass);
				} catch (err) {
					this.logService.warn('window#ctor: failed to apply Agents window material', err);
				}
			}
