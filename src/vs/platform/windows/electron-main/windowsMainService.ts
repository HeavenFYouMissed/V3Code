...
import { ICodeWindow, IWindowState as IWindowUIState, UnloadReason } from '../../window/electron-main/window.js';
...
interface IOpenBrowserWindowOptions {
	...
	readonly windowState?: IWindowUIState;
...
export class WindowsMainService extends Disposable implements IWindowsMainService {
	...
	private async doOpen(
		...
		if (allWorkspacesToOpen.length > 0) {
			...
			const windowsOnWorkspace = openConfig.forceDuplicateWorkspace
				? []
				: coalesce(allWorkspacesToOpen.map(workspaceToOpen => findWindowOnWorkspaceOrFolder(this.getWindows(), workspaceToOpen.workspace.configPath)));
		...
		if (allFoldersToOpen.length > 0) {
			...
			// Check for existing instances
			const windowsOnFolderPath = openConfig.forceDuplicateWorkspace
				? []
				: coalesce(allFoldersToOpen.map(folderToOpen => findWindowOnWorkspaceOrFolder(this.getWindows(), folderToOpen.workspace.uri)));
	...
	private doOpenEmpty(openConfig: IOpenConfiguration, forceNewWindow: boolean, remoteAuthority: string | undefined, filesToOpen: IFilesToOpen | undefined, emptyWindowBackupInfo?: IEmptyWindowBackupInfo): Promise<ICodeWindow> {
		...
		return this.openInBrowserWindow({
			userEnv: openConfig.userEnv,
			cli: openConfig.cli,
			windowState: openConfig.windowState,
			isAgentWorkspaceWindow: openConfig.isAgentWorkspaceWindow,
			initialStartup: openConfig.initialStartup,
			remoteAuthority,
	...
	private async openInBrowserWindow(options: IOpenBrowserWindowOptions): Promise<ICodeWindow> {
		...
		const configuration: INativeWindowConfiguration = {
			...
			isAgentWorkspaceWindow: options.isAgentWorkspaceWindow,
		};

		// New window
		if (!window) {
			const state = options.windowState ?? this.windowsStateHandler.getNewWindowState(configuration);
			...
			const createdWindow = window = this.instantiationService.createInstance(CodeWindow, {
				...
				isAgentWorkspaceWindow: configuration.isAgentWorkspaceWindow
			});
