...
export interface IBaseOpenConfiguration {
	...
	/**
	 * Create the window with the native Agent Workspace material from its first
	 * frame without turning it into the separate Sessions application surface.
	 */
	readonly isAgentWorkspaceWindow?: boolean;
	/**
	 * Seed a newly-created window with the exact state of an existing window.
	 * Used by companion and one-way handoff flows so the new window does not jump
	 * to the default size or display.
	 */
...
export interface IOpenConfiguration extends IBaseOpenConfiguration {
	...
	/**
	 * Open a second window even when the requested workspace is already open.
	 * This is reserved for companion and one-way handoff flows that must construct
	 * the new window before any existing window is retired.
	 */
	readonly forceDuplicateWorkspace?: boolean;
