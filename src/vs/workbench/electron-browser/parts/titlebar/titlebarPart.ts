...
export class NativeTitlebarPart extends BrowserTitlebarPart {
	...
	override layout(width: number, height: number): void {
		...
		if (useWindowControlsOverlay(this.configurationService)) {
			...
			// The Agent Workspace shell hides the titlebar, but the grid still lays this
			// part out - at height 0. Pushing that would ask macOS to centre the traffic
			// lights in a zero-height band, placing them off the window's corner. Leave
			// the last good height in place instead.
			if (isMacintosh && newHeight === 0) {
				return;
			}
