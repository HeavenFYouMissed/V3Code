...
export class DelegatedOutputChannelModel extends Disposable implements IOutputChannelModel {
	...
	private async createOutputChannelModel(id: string, modelUri: URI, language: ILanguageSelection, outputDir: URI, outputDirPromise: Promise<void>): Promise<IOutputChannelModel> {
		...
		// Output channels can be removed and registered again during the lifetime of
		// a window (for example, while an entitlement-backed channel is hidden and
		// restored). The previous model is disposed, but its backing file remains in
		// the window's output directory. Recreate the channel with a fresh file rather
		// than failing because that stale backing file already exists.
		await this.fileService.createFile(file, undefined, { overwrite: true });
