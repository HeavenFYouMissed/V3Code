...
suite('Workbench - MCP - Registry', () => {
	...
	// V3Code: the unresolved-placeholder gate. Definitions without variableReplacement
	// (the legacy ~/.v3code/mcp.json discovery path) skip variable resolution entirely,
	// and that path once sent "Bearer ${GITHUB_PERSONAL_ACCESS_TOKEN}" over the wire.
	suite('unresolved placeholder gate', () => {
		test('an HTTP launch with a bare placeholder header never becomes a connection', async () => {
			const definition: McpServerDefinition = {
				...baseDefinition,
				launch: {
			...
			assert.strictEqual(connection, undefined, 'placeholder residue must refuse to launch');
		});

		test('a legacy stdio launch (no variableReplacement) is NOT gated — it launched before', async () => {
			const definition: McpServerDefinition = {
				...baseDefinition,
				launch: {
					...(baseDefinition.launch as McpServerTransportStdio),
			...
			const connection = await registry.resolveConnection({ collectionRef: testCollection, definitionRef: definition, logger, trustNonceBearer, taskManager });
			assert.ok(connection, 'pre-existing legacy stdio behavior is preserved');
			connection.dispose();
		});

		test('a resolution-capable stdio launch with residue after resolution is refused', async () => {
			const definition: McpServerDefinition = {
				...baseDefinition,
				launch: {
					...(baseDefinition.launch as McpServerTransportStdio),
		...
		test('a clean launch without variableReplacement still connects', async () => {
			const definition: McpServerDefinition = {
				...baseDefinition,
				launch: {
					type: McpServerTransportType.HTTP,
			...
			const connection = await registry.resolveConnection({ collectionRef: testCollection, definitionRef: definition, logger, trustNonceBearer, taskManager });
			assert.ok(connection, 'clean legacy launches are untouched by the gate');
			connection.dispose();
		});
	});
