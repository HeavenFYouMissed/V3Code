...
export class McpRegistry extends Disposable implements IMcpRegistry {
	...
	private async _replaceVariablesInLaunch(delegate: IMcpHostDelegate, definition: McpServerDefinition, launch: McpServerLaunch, errorOnUserInteraction?: boolean) {
		...
		// V3Code: a cancelled input prompt leaves its ${input:...} replacement unresolved.
		// Treat that as a user cancel (like resolveServerLanch returning undefined), not a
		// broken configuration — otherwise the launch gate refuses with advice to edit a
		// config that is perfectly fine.
		for (const replacement of expr.unresolved()) {
			if (replacement.name === 'input') {
				return undefined;
			}
		}
	...
	public async resolveConnection(opts: IMcpResolveConnectionOptions): Promise<IMcpServerConnection | undefined> {
		...
		try {
			...
			if (!launch) {
				return undefined; // input prompt cancelled by the user
			}

			// V3Code: hard gate — placeholder residue must never reach the NETWORK (any HTTP
			// launch: exactly the path that once sent a literal "Bearer ${GITHUB_...}" header),
			// and for resolution-capable definitions residue after resolution is always fatal.
			// Legacy stdio definitions (no variableReplacement, the ~/.v3code/mcp.json adapter)
			// are deliberately NOT gated: they launched before this change and gating them
			// would refuse configs that previously worked.
			if (definition.variableReplacement || launch.type === McpServerTransportType.HTTP) {
				const residue = Array.from(ConfigurationResolverExpression.parse(launch).unresolved(), r => r.id);
				if (residue.length > 0) {
					throw new Error(localize('mcp.unresolvedPlaceholders', 'The configuration contains unresolved placeholder values: {0}. Edit the server configuration to fill them in, or reinstall the server from the catalog.', residue.join(', ')));
				}
			}
