...
export class RunTestTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, countTokens: CountTokensCallback, progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
		...
		if (!testCases.length) {
			if ([...this._testService.collection.rootItems].length === 0) {
				return {
					content: [{ kind: 'text', value: 'No tests are registered with the VS Code Testing API. Activate or install a test provider extension for this workspace, then retry, or use run_command to run the project\'s test command directly.' }],
					toolResultError: localize('runTestTool.noRegisteredTests', 'No tests are registered with the VS Code Testing API. Activate or install a test provider extension for this workspace, then retry, or use run_command to run the project\'s test command directly.'),
				};
			}
			return {
