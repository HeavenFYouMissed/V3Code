...
import { CoverageDetails, DetailType, IBranchCoverage, IDeclarationCoverage, IFileCoverage, IncrementalTestCollectionItem, IStatementCoverage, ResolvedTestRunRequest, TestMessageType, TestResultState, TestRunProfileBitset } from '../../common/testTypes.js';
...
suite('Workbench - RunTestTool', () => {
	...
	let registeredTestRoots: IncrementalTestCollectionItem[];
	...
	suite('invoke', () => {
		test('returns actionable error when no tests are registered with the VS Code Testing API', async () => {
			const result = await tool.invoke(
				upcastPartial<IToolInvocation>({ parameters: {} }),
				noopCountTokens, noopProgress, CancellationToken.None,
			);
			assert.strictEqual(result.toolResultError, 'No tests are registered with the VS Code Testing API. Activate or install a test provider extension for this workspace, then retry, or use run_command to run the project\'s test command directly.');
			assert.ok(result.content[0].kind === 'text');
			assert.strictEqual(result.content[0].value, 'No tests are registered with the VS Code Testing API. Activate or install a test provider extension for this workspace, then retry, or use run_command to run the project\'s test command directly.');
		});

		test('keeps the file-path guidance when a provider is registered but no tests match', async () => {
			registeredTestRoots.push(upcastPartial<IncrementalTestCollectionItem>({}));
			const result = await tool.invoke(
			...
			assert.ok(result.content[0].kind === 'text');
			assert.strictEqual(result.content[0].value, 'No tests found in the files. Ensure the correct absolute paths are passed to the tool.');
		});
	});
