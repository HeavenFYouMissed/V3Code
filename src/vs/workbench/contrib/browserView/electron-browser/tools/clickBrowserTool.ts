...
export class ClickBrowserTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, _token: CancellationToken): Promise<IToolResult> {
		...
		if (params.dblClick) {
			return playwrightInvoke(this.playwrightService, sessionId, pageId, (page, sel, btn) => page.locator(sel).dblclick({ button: btn }), selector, button);
		}

		return playwrightInvoke(this.playwrightService, sessionId, pageId, (page, sel, btn) => page.locator(sel).click({ button: btn }), selector, button);
	}
}
