...
import { createBrowserPageLink, errorResult, getSessionId, playwrightInvoke, resolveBrowserPageId } from './browserToolHelpers.js';
...
export class TypeBrowserTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, _token: CancellationToken): Promise<IToolResult> {
		...
		const rawPageId = browserPageId(params as unknown as Record<string, unknown>) ?? params.pageId;
		...
		if (selector) {
			...
			this.agentVisuals.beginActivity(pageId, invocation.context?.sessionResource);
		}

		// Press key
		if (params.key) {
			if (selector) {
				return playwrightInvoke(this.playwrightService, sessionId, pageId, (page, sel, key) => page.locator(sel).press(key), selector, params.key);
			}
			return playwrightInvoke(this.playwrightService, sessionId, pageId, (page, key) => page.keyboard.press(key), params.key);
		}

		// Type text
		if (selector) {
			return playwrightInvoke(this.playwrightService, sessionId, pageId, (page, sel, text) => page.locator(sel).fill(text), selector, params.text!);
		}
		return playwrightInvoke(this.playwrightService, sessionId, pageId, (page, text) => page.keyboard.type(text), params.text!);
	}
}
