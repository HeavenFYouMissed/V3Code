...
export class ScreenshotBrowserTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, _token: CancellationToken): Promise<IToolResult> {
		...
		this.agentVisuals.beginActivity(params.pageId, invocation.context?.sessionResource);
		await this.agentVisuals.hideForToolUse(params.pageId);
		const overlayHidden = await this.agentVisuals.checkOverlayHidden(params.pageId);
		if (!overlayHidden) {
			this.logService.error('[AgentVisuals] overlay still visible before screenshot capture');
		}
		const screenshot = await browserViewModel.captureScreenshot({ pageRect: bounds });
		this.agentVisuals.showAfterToolUse(params.pageId);
		this.agentVisuals.notifyToolCall(params.pageId);
