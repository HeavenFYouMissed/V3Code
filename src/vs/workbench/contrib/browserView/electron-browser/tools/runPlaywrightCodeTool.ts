...
export class RunPlaywrightCodeTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, _token: CancellationToken): Promise<IToolResult> {
		...
		if (/\b(document|window)\b/.test(trimmed) && !/page\.evaluate/.test(trimmed)) {
			return errorResult(
				'Code references document/window directly — Playwright runs outside the page. Wrap in page.evaluate, e.g. `return await page.evaluate(() => document.title);`'
			);
		}
		...
		try {
			...
			result = await this.playwrightService.invokeFunction(sessionId, params.pageId, `async (page) => { ${params.code} }`, undefined, timeout);
		} catch (e) {
