...
import { ExtractPageDataBrowserTool, ExtractPageDataBrowserToolData } from './extractPageDataBrowserTool.js';
import { GetBrowserConsoleLogsBrowserTool, GetBrowserConsoleLogsBrowserToolData } from './getBrowserConsoleLogsTool.js';
import { ReconstructPageSourcesBrowserTool, ReconstructPageSourcesBrowserToolData } from './reconstructPageSourcesBrowserTool.js';
import { SaveBrowserSessionBrowserTool, SaveBrowserSessionBrowserToolData, RestoreBrowserSessionBrowserTool, RestoreBrowserSessionBrowserToolData } from './browserSessionBrowserTools.js';
import { FillFormBrowserTool, FillFormBrowserToolData } from './fillFormBrowserTool.js';
import { GetComputedStylesBrowserTool, GetComputedStylesBrowserToolData } from './getComputedStylesBrowserTool.js';
import { InterceptNetworkBrowserTool, InterceptNetworkBrowserToolData, GetBrowserNetworkLogBrowserTool, GetBrowserNetworkLogBrowserToolData } from './networkBrowserTools.js';
import { WatchPageBrowserTool, WatchPageBrowserToolData } from './watchPageBrowserTool.js';
...
class BrowserChatAgentToolsContribution extends Disposable implements IWorkbenchContribution {
	...
	private _updateToolRegistrations(): void {
		...
		this._toolsStore.add(this.toolsService.registerTool({
			id: 'list_pages',
			displayName: localize('browser.listPages', "List Browser Pages"),
			modelDescription: 'List currently shared browser pages with their exact page IDs. Use this to recover after a page closes; never reuse a stale ID.',
			source: ToolDataSource.Internal,
			inputSchema: { type: 'object', properties: {} },
		}, {
			invoke: async () => ({
				content: [{
					kind: 'text', value: formatBrowserEditorList(this.editorService,
						[...this.browserViewService.getKnownBrowserViews().values()].filter(page => page.model?.sharingState === BrowserViewSharingState.Shared),
						{ agentNetworkFilterService: this.agentNetworkFilterService }) || 'No shared browser pages.'
				}]
			}),
		}));
		...
		this._toolsStore.add(this.toolsService.registerTool({
			id: 'close_page',
			displayName: localize('browser.closePage', "Close Browser Page"),
			modelDescription: 'Close an unused shared browser page by its exact pageId. Never close a page the user is working in without their instruction.',
			source: ToolDataSource.Internal,
			inputSchema: { type: 'object', properties: { pageId: { type: 'string' } }, required: ['pageId'] },
		}, {
			invoke: async invocation => {
				const page = this.browserViewService.getKnownBrowserViews().get(invocation.parameters.pageId);
				if (!page || page.model?.sharingState !== BrowserViewSharingState.Shared) {
					return { content: [{ kind: 'text', value: 'Page not found or not shared. Use list_pages for current page IDs.' }], toolResultError: 'Page not found or not shared' };
				}
				await this.editorService.closeEditors(this.editorService.findEditors(page.resource));
				return { content: [{ kind: 'text', value: `Closed page ${page.id}.` }] };
			},
		}));
		...
		this._toolsStore.add(this.toolsService.registerTool(ExtractPageDataBrowserToolData, this.instantiationService.createInstance(ExtractPageDataBrowserTool)));
		this._toolsStore.add(this.toolsService.registerTool(GetBrowserConsoleLogsBrowserToolData, this.instantiationService.createInstance(GetBrowserConsoleLogsBrowserTool)));
		this._toolsStore.add(this.toolsService.registerTool(ReconstructPageSourcesBrowserToolData, this.instantiationService.createInstance(ReconstructPageSourcesBrowserTool)));
		this._toolsStore.add(this.toolsService.registerTool(GetComputedStylesBrowserToolData, this.instantiationService.createInstance(GetComputedStylesBrowserTool)));
		this._toolsStore.add(this.toolsService.registerTool(WatchPageBrowserToolData, this.instantiationService.createInstance(WatchPageBrowserTool)));
		this._toolsStore.add(this.toolsService.registerTool(SaveBrowserSessionBrowserToolData, this.instantiationService.createInstance(SaveBrowserSessionBrowserTool)));
		this._toolsStore.add(this.toolsService.registerTool(RestoreBrowserSessionBrowserToolData, this.instantiationService.createInstance(RestoreBrowserSessionBrowserTool)));
		this._toolsStore.add(this.toolsService.registerTool(FillFormBrowserToolData, this.instantiationService.createInstance(FillFormBrowserTool)));
		this._toolsStore.add(this.toolsService.registerTool(InterceptNetworkBrowserToolData, this.instantiationService.createInstance(InterceptNetworkBrowserTool)));
		this._toolsStore.add(this.toolsService.registerTool(GetBrowserNetworkLogBrowserToolData, this.instantiationService.createInstance(GetBrowserNetworkLogBrowserTool)));
		...
		this._toolsStore.add(this._browserToolSet.addTool(ExtractPageDataBrowserToolData));
		this._toolsStore.add(this._browserToolSet.addTool(GetBrowserConsoleLogsBrowserToolData));
		this._toolsStore.add(this._browserToolSet.addTool(ReconstructPageSourcesBrowserToolData));
		this._toolsStore.add(this._browserToolSet.addTool(GetComputedStylesBrowserToolData));
		this._toolsStore.add(this._browserToolSet.addTool(WatchPageBrowserToolData));
		this._toolsStore.add(this._browserToolSet.addTool(SaveBrowserSessionBrowserToolData));
		this._toolsStore.add(this._browserToolSet.addTool(RestoreBrowserSessionBrowserToolData));
		this._toolsStore.add(this._browserToolSet.addTool(FillFormBrowserToolData));
		this._toolsStore.add(this._browserToolSet.addTool(InterceptNetworkBrowserToolData));
		this._toolsStore.add(this._browserToolSet.addTool(GetBrowserNetworkLogBrowserToolData));
