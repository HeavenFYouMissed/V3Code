...
const OVERLAY_DEFINITIONS: ReadonlyArray<{ className: string; type: BrowserOverlayType }> = [
	...
	// The account panel uses the hover service for positioning, but it is a sticky menu.
	// Treat it as a hard overlay so the native WebContentsView is hidden immediately.
	{ className: 'sessions-account-titlebar-panel-hover', type: BrowserOverlayType.Menu },
	// V3Code's own body-mounted menus. Neither goes through the context-view or menu services, so
	// nothing above matches them: v3ProfileButton and unifiedSidebarAgentsContribution each append a
	// bare div to document.body. Without an entry here the overlay manager never sees them, the
	// WebContentsView is never hidden, and the menu is drawn UNDERNEATH the native view and clipped
	// away — reported as the profile menu being "eaten by the browser". Menu, not Hover, so the view
	// hides immediately rather than after the transient debounce.
	{ className: 'v3-profile-menu', type: BrowserOverlayType.Menu },
	{ className: 'unified-agents-account-menu', type: BrowserOverlayType.Menu },
...
function getOverlappingRectangleCenterPoint(rect1: IDomNodePagePosition, rect2: IDomNodePagePosition): { x: number; y: number } | null {
	...
	// Require a real overlap (>4px each axis), not a 1px graze — sub-pixel/edge touches
	// from rounding shouldn't count as an overlay covering the browser and pause it.
	const MIN_OVERLAP_PX = 4;
	if (overlapRight - overlapLeft > MIN_OVERLAP_PX && overlapBottom - overlapTop > MIN_OVERLAP_PX) {
		return {
