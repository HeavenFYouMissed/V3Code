...
export class BrowserEditorChatIntegration extends BrowserEditorContribution {
	...
	private async _toggleShareWithAgent(): Promise<void> {
		...
		const wantShared = model.sharingState !== BrowserViewSharingState.Shared;
		const ok = await model.setSharedWithAgent(wantShared);
		if (!ok && wantShared) {
			this.logService.warn('[BrowserEditorChatIntegration] Share with Agent was not enabled (user declined or policy blocked).');
		}
	}
	...
	private async _attachElementDataToChat(elementData: IElementData, model: IBrowserViewModel) {
		...
		if (attachImages) {
			...
			toAttach.push({
				...
				kind: 'image',
				// captureScreenshot ALWAYS returns JPEG (image.toJPEG). Labeling it correctly here
				// is what keeps Anthropic from 400-rejecting the whole turn (it validates declared
				// media_type against the actual JPEG magic bytes, unlike OpenAI-compatible endpoints).
...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	...
	properties: {
		...
		'workbench.browser.agentVisuals': {
			type: 'boolean',
			default: true,
			tags: ['experimental'],
			markdownDescription: localize(
				'browser.agentVisuals',
				'When enabled, show a phantom cursor, viewport glow, and status badge while the chat agent drives the integrated browser. Overlays are hidden during screenshots.'
			),
		}
	}
});
