...
export class BrowserEditorEmulationSupport extends BrowserEditorContribution {
	...
	/** One-press phone toggle: snap straight to a real phone (viewport + touch + DPR + UA), or back
	 *  to desktop if already on a phone. No device-picker fiddling — pressing the button just makes
	 *  the browser a phone. */
	toggleMobile(): void {
		const model = this.editor.model;
		if (!model) {
		...
		const device = model.device;
		if (device?.mobile) {
			// Already a phone -> back to desktop (clears the device + hides the emulation toolbar).
			// Gate on `mobile` alone: a phone device without an explicit width still needs the
			// titlebar button to toggle OFF cleanly.
			this.setVisible(false);
		} else {
			// Jump to a real phone (iPhone-class), then show the device frame.
			void model.setDevice({
				width: 393,
				height: 852,
				mobile: true,
...
MenuRegistry.appendMenuItem(MenuId.TitleBar, {
	command: {
		id: ToggleBrowserMobileEmulationAction.ID,
		title: localize('browser.titlebar.mobile', "Toggle Mobile View"),
		icon: Codicon.deviceMobile,
		toggled: CONTEXT_BROWSER_EMULATION_IS_MOBILE,
	},
	group: 'navigation',
	...
	when: ContextKeyExpr.and(
		...
		ContextKeyExpr.or(
			ContextKeyExpr.has('browserEditorOpen'),
