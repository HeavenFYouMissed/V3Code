...
import { IBrowserAutomationConsentService } from './browserAutomationConsent.js';
...
export interface IBrowserViewModel extends IDisposable {
	...
	/** Run JS in the page's isolated world (shared DOM) and return its result. Powers Visual Edit. */
...
export class BrowserViewModel extends Disposable implements IBrowserViewModel {
	...
	constructor(
		...
		@IBrowserAutomationConsentService private readonly automationConsent: IBrowserAutomationConsentService,
	) {
		super();
	...
	async executeScript(code: string): Promise<unknown> {
		return this.browserViewService.executeScript(this.id, code);
	}
	...
	async setSharedWithAgent(shared: boolean): Promise<boolean> {
		if (shared) {
			if (!(await this.automationConsent.ensureAccepted())) {
				return false;
			}
