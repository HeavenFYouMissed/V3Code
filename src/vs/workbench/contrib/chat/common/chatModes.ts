...
import { problemsErrorIconForeground } from '../../../../platform/theme/common/colors/editorColors.js';
...
class ChatModes extends Disposable implements IChatModes {
	...
	private getBuiltinModes(): IChatMode[] {
		...
		const hasAgentMode = this.chatAgentService.hasToolsAgent || this.isAgentModeDisabledByPolicy();
		...
		if (hasAgentMode) {
			// Debug and Multitask both run on the tools agent — hidden with it, like Agent.
			builtinModes.push(ChatMode.Debug);
			builtinModes.push(ChatMode.Multitask);
		}
...
export class BuiltinChatMode implements IChatMode {
	...
	constructor(
		...
		description: string,
		icon: ThemeIcon,
		private readonly stableId: string = kind,
		name: string = kind,
	) {
		this.name = constObservable(name);
...
export namespace ChatMode {
	export const Ask = new BuiltinChatMode(ChatModeKind.Ask, 'Chat', localize('chatMode.chatDescription', "Talk to the agent — no file edits"), Codicon.comment);
	export const Edit = new BuiltinChatMode(ChatModeKind.Edit, 'Read', localize('chatMode.readDescription', "Read the codebase — tools only, no edits"), Codicon.search);
	export const Agent = new BuiltinChatMode(ChatModeKind.Agent, 'Agent', localize('agentDescription', "Plan, build, agent — / for commands, @ for context"), Codicon.agent);
	export const Plan = new BuiltinChatMode(ChatModeKind.Plan, 'Plan', localize('planDescription', "Plan before building"), Codicon.tasklist);
	export const Multitask = new BuiltinChatMode(ChatModeKind.Agent, 'Multitask', localize('multitaskDescription', "Foreman: plan, dispatch parallel work agents, reconcile at the board"), Codicon.organization, 'multitask', 'multitask');
	// Theme-aware red accent: the icon carries a registered theme color, which the picker
	// list and the composer control both resolve through the current theme (incl. high contrast).
	export const Debug = new BuiltinChatMode(ChatModeKind.Agent, 'Debug', localize('debugDescription', "Reproduce, localise, prove the root cause, minimal fix, guard"), { id: Codicon.bug.id, color: themeColorFromId(problemsErrorIconForeground) }, 'debug', 'debug');
}
