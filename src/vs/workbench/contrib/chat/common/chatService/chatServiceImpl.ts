...
export class ChatService extends Disposable implements IChatService {
	...
	async sendRequest(sessionResource: URI, request: string, options?: IChatSendRequestOptions): Promise<ChatSendResult> {
		...
		try {
			...
			// No agent is registered that can handle this request (e.g. a window that
			// registers no default chat participant, like the Agents window) and it is
			// not a standalone slash command. Without this guard _sendRequestAsync
			// throws 'Cannot handle request' AFTER this method already returned
			// { kind: 'sent' }, so nothing ever renders. Mirrors the guard in
			// processNextPendingRequest.
			if (!agent && !parsedRequest.parts.some(r => r instanceof ChatRequestSlashCommandPart)) {
				this.trace('sendRequest', `No agent available for session ${sessionResource} at location ${location}`);
				return { kind: 'rejected', reason: localize('chatService.noAgentAvailable', "No chat agent is available to handle this request.") };
			}
		...
		const sendRequestInternal = async () => {
			...
			try {
				...
				if (agentPart || (defaultAgent && !commandPart)) {
					...
					// Defense in depth for paths that reach here without an agent
					// (e.g. resendRequest, which also assumes a default agent):
					// create the request model FIRST so the error renders in the
					// transcript — the catch below only surfaces errors when a
					// request exists.
					request = model.addRequest(parsedRequest, { variables: [] }, attempt, options?.modeInfo);
					completeResponseCreated();
					throw new Error(localize('chatService.cannotHandleRequest', "No chat agent or command is available to handle this request."));
				}
