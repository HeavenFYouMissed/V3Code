...
export interface IRunSubagentToolInputParams {
	...
	agentName?: string;
	model?: string;
	/** V3Code runtime-only containment flag; never model-controlled. */
	readOnly?: boolean;
}

// Conservative cap, aligned with V3Code's SUBAGENT_MAX_NESTING_DEPTH: a session may
// nest subagent invocations two levels deep before further delegation is disabled.
export const RUN_SUBAGENT_MAX_NESTING_DEPTH = 2;
...
export class RunSubagentTool extends Disposable implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
		...
		try {
			...
			if (subAgentName && !isGeneralPurpose) {
				...
				if (subagent) {
					...
					if (modeCustomTools) {
						...
						// ROBUST FALLBACK (V3Code): the enablement map above leaves every tool not
						// matching the agent's declared names as `false`. With native BYOK tools, the
						// fullReferenceName/alias matching can fail at runtime (toolset-namespaced names
						// not aliasing to the bare name), leaving EVERY tool false → a subagent with no
						// tools at all. To guard that, directly enable each declared tool by matching
						// registered tools' bare reference name. Additive: only ever enables tools the
						// agent explicitly declared; never enables anything undeclared.
						const declared = new Set<string>();
						for (const n of modeCustomTools) {
							declared.add(n);
							const slash = n.lastIndexOf('/');
							if (slash !== -1) { declared.add(n.substring(slash + 1)); }
						}
						for (const tool of this.languageModelToolsService.getTools(undefined)) {
							const refName = tool.toolReferenceName ?? tool.displayName;
							// Declared names arrive bare (read_file), slash-qualified (set/read_file), or
							// V3Code-qualified (v3code_read_file — what .agent.md resolves to at runtime).
							// The v3code_ form must match the REFERENCE NAME, not the id: builtin ids are
							// v3code_read_file but MCP ids are v3code_mcp_<server>_<name>, yet the agent
							// declares v3code_<name> for both.
							const matches =
								declared.has(tool.id) ||
								(!!refName && declared.has(refName)) ||
								(!!refName && declared.has(`v3code_${refName}`));
							if (matches) { modeTools[tool.id] = true; }
						}
					}
			...
			// V3Code invokes this native runner with a profile-filtered selection (read-only
			// for research children, the parent's allowed surface for work children). A named
			// custom agent may replace modeTools above, so intersect it with the invocation
			// boundary here as the final enforcement point — for EVERY profile: a writable
			// child must not exceed its parent's enabled tools any more than a read-only one.
			// Unknown or newly registered tools fail closed rather than inheriting capability.
			// An absent/empty boundary (plain upstream chat with no tool customization) keeps
			// the runner's default behavior.
			const boundary = invocation.userSelectedTools;
			if (boundary && Object.keys(boundary).length > 0) {
				for (const toolId of Object.keys(modeTools)) {
					modeTools[toolId] = modeTools[toolId] === true && boundary[toolId] === true;
				}
				for (const [toolId, enabled] of Object.entries(boundary)) {
					if (!Object.hasOwn(modeTools, toolId)) {
						modeTools[toolId] = enabled === true;
					}
				}
			}
			...
			if (existingRunSubagentEnablement !== false) {
				modeTools[RunSubagentTool.Id] = !args.readOnly && depthAllowed; // read-only children never recurse
			}
	...
	private resolveSubagentModel(subagent: ICustomAgent | undefined, mainModelId: string | undefined, explicitModelQualifiedName?: string): { modeModelId: string | undefined; resolvedModelName: string | undefined } {
		...
		if (explicitModelQualifiedName) {
			...
			if (lm?.identifier) {
				...
				// The lookup wants the fully qualified `Name (vendor)` form, but the model list in
				// the system prompt is built from an entirely different catalog and advertises a
				// different shape — so every id a model naturally reaches for failed, and the error
				// said "not found" about a model that was sitting in its own available list one line
				// below. Accept a bare name when exactly one registered model answers to it.
				const byBareName = this.languageModelsService.getLanguageModelIds()
					.map(id => ({ id, metadata: this.languageModelsService.lookupLanguageModel(id) }))
					.filter((m): m is { id: string; metadata: ILanguageModelChatMetadata } => !!m.metadata)
					.filter(m => m.metadata.name === explicitModelQualifiedName);

				if (byBareName.length === 1) {
					modeModelId = byBareName[0].id;
					explicitModelResolved = true;
				} else if (byBareName.length > 1) {
					// Deliberately still an error: display names repeat across vendors, and silently
					// picking one would send the work to a different model than was asked for.
					throw new Error(`Requested model '${explicitModelQualifiedName}' is ambiguous — ${byBareName.length} registered models share that name. Ask for one of these exactly: ${byBareName.map(m => `'${ILanguageModelChatMetadata.asQualifiedName(m.metadata)}'`).join(', ')}.`);
				} else {
					throw new Error(`Requested model '${explicitModelQualifiedName}' not found. Model names are matched exactly, either bare (e.g. 'V3Pro') or fully qualified as 'Name (vendor)'. ${this.getAvailableModelsInfo(mainModelId)}`);
				}
			}
		}
