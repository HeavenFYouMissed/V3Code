...
export class ComputeAutomaticInstructions {
	...
	private _getSkillRelevanceQuery(variables: ChatRequestVariableSet, context: { files: ResourceSet }): string {
		const parts: string[] = [];
		...
		for (const file of context.files) {
			parts.push(file.fsPath);
			parts.push(this._labelService.getUriLabel(file, { relative: true }));
		}
		return parts.join(' ').toLowerCase();
	}

	private _isSkillRelevant(skill: IAgentSkill, query: string, context: { files: ResourceSet }): boolean {
		...
		const hasGlobs = !!globs && globs.length > 0;
		// Legacy: skills without triggers remain discoverable in the index
		if (!hasKeywords && !hasGlobs) {
			return true;
		}

		if (hasGlobs) {
			for (const glob of globs) {
				if (this._matches(context.files, glob.trim())) {
					return true;
				}
			}
		}
	...
	private async _getCustomizationsIndex(instructionFiles: readonly IInstructionFile[], _existingVariables: ChatRequestVariableSet, telemetryEvent: InstructionsCollectionEvent, debugInfo: InstructionsCollectionDebugInfo, token: CancellationToken): Promise<IPromptTextVariableEntry | undefined> {
		...
		if (fileReadTool) {
			...
			const context = this._getContext(_existingVariables);
			const skillRelevanceQuery = this._getSkillRelevanceQuery(_existingVariables, context);
			const relevantSkills = modelInvocableSkills?.filter(skill => {
				const relevant = this._isSkillRelevant(skill, skillRelevanceQuery, context);
				if (!relevant) {
					debugInfo.debugDetails.push({ category: 'skipped', name: skill.name, uri: skill.uri, reason: localize('debugDetail.skillNotRelevant', 'keywords/globs did not match current context') });
				}
				return relevant;
			}) ?? [];
			if (relevantSkills.length > 0) {
				// Log per-skill telemetry for each skill loaded into context
				this._logSkillLoadedTelemetry(relevantSkills);
				...
				let truncatedAtIndex = relevantSkills.length;
				for (let i = 0; i < relevantSkills.length; i++) {
					const skill = relevantSkills[i];
				...
				if (truncatedAtIndex < relevantSkills.length) {
					const truncatedSkills = relevantSkills.slice(truncatedAtIndex);
					const names: string[] = [];
