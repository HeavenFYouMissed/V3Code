			...
			try {
				...
				if (isAbsolute(configuredLocation)) {
					...
					for (const folder of rootFolders) {
						...
						if (type === PromptsType.skill && configuredLocation === '.v3code/skills') {
							try {
								...
								for (const child of stat.children ?? []) {
									...
									const nestedSkillsDir = joinPath(child.resource, '.v3code', 'skills');
									if (!seen.has(nestedSkillsDir)) {
										seen.add(nestedSkillsDir);
										const { searchRoot, filePattern } = resolveSearchLocation(type, nestedSkillsDir);
										result.push({ uri: nestedSkillsDir, searchRoot, filePattern, source: sourceFolder.source, storage: sourceFolder.storage, displayPath: `${basename(child.resource)}/.v3code/skills`, isDefault });
									}
								}
							} catch {
								// ignore missing or inaccessible child folders
							}
						}
					}
				}
