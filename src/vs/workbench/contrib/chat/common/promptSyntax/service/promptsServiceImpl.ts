	...
	private async computeSkillDiscoveryInfo(token: CancellationToken): Promise<IPromptFileDiscoveryResult[]> {
		...
		for (const skill of allSkills) {
			...
			try {
				...
				const keywords = parsedFile.header?.keywords;
				const globs = parsedFile.header?.globs;
				const alwaysApply = parsedFile.header?.alwaysApply === true;

				files.push({ status: 'loaded', promptPath: this.withPromptPathMetadata(promptPath, sanitizedName, description), disableModelInvocation, userInvocable, keywords, globs, alwaysApply });
			} catch (e) {
