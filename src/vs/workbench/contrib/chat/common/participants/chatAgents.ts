...
export interface IChatAgentRequest {
	...
	/** True only for a request delegated by the V Voice runtime. */
	isV3VoiceRelay?: boolean;
	/**
...
export class ChatAgentService extends Disposable implements IChatAgentService {
	...
	registerDynamicAgent(data: IChatAgentData, agentImpl: IChatAgentImplementation): IDisposable {
		...
		// Dynamic agents bundle data + implementation in one call and never pass through
		// registerAgent()/registerAgentImplementation(), so the chat context keys must be
		// refreshed here. Without this a default dynamic agent (e.g. the V3Code chat agent)
		// leaves ChatContextKeys.enabled / panelParticipantRegistered false, which disables
		// every chat action gated on those keys (the "all chat features greyed out" bug).
		if (data.isDefault) {
			this._hasDefaultAgent.set(true);
		}
		this._updateAgentsContextKeys();
	...
	private _preferExtensionAgent<T extends IChatAgentData>(agents: T[]): T | undefined {
		// V3Code registers v3code.agent as the core default — prefer it over Copilot's
		// extension participants (github.copilot.default), which require GitHub auth.
		if (this.productService.defaultChatAgent?.extensionId === 'v3code.v3code') {
			return findLast(agents, agent => agent.isCore) ?? agents.at(-1);
		}
		// Upstream: prefer extension-provided agents over core fallbacks.
		return findLast(agents, agent => !agent.isCore) ?? agents.at(-1);
	}
