...
import { badgeBackground, badgeForeground, contrastBorder, descriptionForeground, editorBackground, editorSelectionBackground, editorWidgetBackground, foreground, registerColor, transparent } from '../../../../../platform/theme/common/colorRegistry.js';
...
/**
 * Colour of the agent's rolling work stream — the running commentary of tool calls and
 * progress under a response.
 *
 * Registered separately from descriptionForeground on purpose. That token is shared with every
 * muted label in the workbench, so dimming the work stream by editing it moved settings
 * descriptions, placeholders and list subtitles at the same time. This gives the one surface
 * people actually want to turn down its own control, and it falls back to descriptionForeground
 * when a theme says nothing, so the default look is unchanged.
 */
export const chatRollingWorkForeground = registerColor('chat.rollingWorkForeground',
	{ dark: descriptionForeground, light: descriptionForeground, hcDark: descriptionForeground, hcLight: descriptionForeground },
	localize('chat.rollingWorkForeground', "Foreground colour of the agent's rolling work stream (tool calls and progress text)."));
