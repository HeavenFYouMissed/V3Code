...
// V3Code: the upstream precondition gated the Agents door on GitHub Copilot's
// entitlement "Setup" context keys (Setup.hidden / disabledInWorkspace). V3Code
// hides that Copilot onboarding entirely, so those keys are always unfavorable and
// the door never showed. Gate only on what actually applies: agent mode enabled,
// and we're in the main (non-sessions, non-auxiliary) window.
