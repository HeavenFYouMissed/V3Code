...
import { isV3StickyRequestActive } from '../../common/widget/v3codeStickyRequest.js';
...
import { deactivateV3VoiceSurfaceForSession } from '../../../void/browser/v3codeVoiceOrbSurface.js';
...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private listRendererWidthOffset = 0;
	...
	private updateWorkingProgressBorder(): void {
		...
		// Capsule ownership follows the durable active-request lifecycle, not the
		// optional animated-border setting or only the streaming phase. The latter
		// briefly drops during tool confirmations and elicitation, which made the
		// pinned request disappear while the same turn was still active.
		const model = this.viewModel?.model;
		const hasActiveRequest = !!model && isV3StickyRequestActive(
			model.hasActiveRequest.get(),
			model.requestInProgress.get(),
			!!model.requestNeedsInput.get(),
		);
		this.listWidget.setStickyRequestActive(hasActiveRequest);
	}
	...
	private onDidChangeItems(skipDynamicLayout?: boolean) {
		const items = this.viewModel?.getItems() ?? [];
		// The empty-session treatment absolutely positions the composer at the top.
		// Keep the class synchronized even while this widget is hidden so reopening a
		// session cannot restore a stale class and pin the input over its transcript.
		this.container.classList.toggle('chat-session-empty', items.length === 0);
	...
	private updateChatViewVisibility(): void {
		...
		// Upstream hides welcome chrome once setup/BYOK is done (room for recent sessions).
		// V3 Agent blank state keeps the branded welcome + tips visible.
		const setupDone = this.chatEntitlementService.sentiment.completed || this.chatEntitlementService.hasByokModels;
		const keepV3AgentWelcome = this.input?.currentModeKind === ChatModeKind.Agent;
		this.container.classList.toggle(
			'chat-view-getting-started-disabled',
			setupDone && !keepV3AgentWelcome);
	...
	private renderWelcomeViewContentIfNeeded() {
		...
		try {
			// Mark an empty session on the container so styling can treat "nothing said yet" as its
			// own layout rather than an ordinary transcript that happens to have no rows. Set before
			// the renderStyle guard below so compact and minimal hosts stay in sync too — they skip
			// the welcome CONTENT, not the state.
			this.container.classList.toggle('chat-session-empty', (this.viewModel?.getItems().length ?? 0) === 0);
	...
	private getWelcomeViewContent(additionalMessage: string | IMarkdownString | undefined): IChatViewWelcomeContent {
		...
		if (this.input.currentModeKind === ChatModeKind.Edit) {
			return {
		...
		// V3 Agent welcome — skip additionalMessage so Generate Instructions can't collide with tips.
		const brandIcon = FileAccess.asBrowserUri('vs/workbench/contrib/void/browser/media/v3-welcome-mark.png' as AppResourcePath);
		return {
			title: localize('v3AgentWelcomeTitle', "V3Code"),
			message: new MarkdownString(localize(
				'v3AgentWelcomeMessage',
				"Deep understanding of your codebase — plan, build, and ship from here."
			)),
			icon: brandIcon,
			useLargeIcon: true,
			tips: new MarkdownString([
				'$(check)  LSP + local semantic index — real project context',
				'$(check)  Plans tasks and drives edits with real tools',
				'$(check)  Remembers what you teach it across sessions',
	...
	private async _acceptInput(query: { query: string } | undefined, options: IChatAcceptInputOptions = {}): Promise<IChatResponseModel | undefined> {
		...
		// Voice is a hidden runtime overlay, not a sticky replacement for the normal
		// composer. A manual composer submit is the explicit mode-exit gesture; V's
		// own delegated submissions carry isVoiceInput and keep the overlay alive.
		if (!query && !options.isVoiceInput) {
			deactivateV3VoiceSurfaceForSession(this.viewModel.sessionResource.toString());
		}
		...
		const result = await this.chatService.sendRequest(this.viewModel.sessionResource, requestInputs.input, {
			...
			isV3VoiceRelay: options?.isV3VoiceRelay,
	...
	/**
	 * Keeps pixel-sized response parts inside hosts whose message rows use
	 * additional horizontal chrome. The list and input retain their full width;
	 * only the response renderer receives the adjusted measurement.
	 */
	setListRendererWidthOffset(offset: number): void {
		this.listRendererWidthOffset = Math.max(0, offset);
	}

	layout(height: number, width: number): void {
		// V3: widened 950 -> 980 to match the CSS column caps (void.css
		// --v3-chat-column-max, agentWorkspaceShell --agent-workspace-chat-content-width).
		// This number and those MUST move together: the list renderer sizes code
		// fences from the width handed in here, so a JS value wider than the CSS
		// box paints fences past the column edge.
		width = Math.min(width, this.viewOptions.renderStyle === 'minimal' ? width : 980); // no min width of inline chat
	...
	private _layoutListForInputHeight(): void {
		...
		this.listWidget.layout(contentHeight, width, Math.max(0, width - this.listRendererWidthOffset));

		this.welcomeMessageContainer.style.height = `${contentHeight}px`;
		this.welcomeMessageContainer.style.setProperty('--v3-welcome-input-height', `${inputHeight}px`);
