...
export class ModePickerActionItem extends ChatInputPickerActionViewItem {
		...
		const actionProvider: IActionWidgetDropdownActionProvider = {
			getActions: () => {
				...
				const otherBuiltinModes = modes.builtin.filter(mode => {
					return mode.id !== ChatMode.Agent.id && shouldShowBuiltInMode(mode, assignments.get(), agentModeDisabledViaPolicy, this._productService);
				});
				const filteredCustomModes = modes.custom.filter(mode => {
					if (isModeConsideredBuiltIn(mode, this._productService)) {
						return shouldShowBuiltInMode(mode, assignments.get(), agentModeDisabledViaPolicy, this._productService);
					}
					return true;
				});
	...
	protected override renderLabel(element: HTMLElement): IDisposable | null {
		...
		if (icon) {
			const iconElements = renderLabelWithIcons(`$(${icon.id})`);
			if (icon.color) {
				// Mirror actionList's group icon coloring so the selected composer control shows the
				// same theme-resolved accent as the picker entry (e.g. the red Debug bug).
				for (const iconElement of iconElements) {
					if (dom.isHTMLElement(iconElement)) {
						iconElement.style.color = asCssVariable(icon.color.id);
					}
				}
			}
			labelElements.push(...iconElements);
		}
		if (!collapsed || !icon) {
...
function isV3CodeProduct(productService: IProductService): boolean {
	return productService.defaultChatAgent?.extensionId === 'v3code.v3code';
}

function shouldShowBuiltInMode(mode: IChatMode, assignments: { showOldAskMode: boolean }, agentModeDisabledViaPolicy: boolean, productService: IProductService): boolean {
	// V3Code ships Chat (Ask) and Read (Edit) as first-class modes — upstream hides them when Agent is enabled.
	if (isV3CodeProduct(productService) && (mode.id === ChatMode.Ask.id || mode.id === ChatMode.Edit.id)) {
		return true;
	}
