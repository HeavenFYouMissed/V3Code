...
import { applyCacheClockEvent, cacheCountdownRemainingMs, formatCacheCountdown, subscribeV3CacheClock } from '../../../../void/common/v3CacheClock.js';
...
import { ChatMessageRole, ILanguageModelChatMetadata, ILanguageModelChatMetadataAndIdentifier, ILanguageModelsService } from '../../../common/languageModels.js';
...
import { ChatSessionPrimaryPickerAction, ChatSubmitAction, IChatExecuteActionContext, OpenComposerPlusAction, OpenDelegationPickerAction, OpenModelPickerAction, OpenModePickerAction, OpenPermissionPickerAction, OpenSessionTargetPickerAction, OpenWorkspacePickerAction } from '../../actions/chatExecuteActions.js';
...
import { ChatInputNotificationSeverity, IChatInputNotificationService } from './chatInputNotificationService.js';
...
import { EXTERNAL_AGENT_PICKER_ID, ExternalAgentPicker } from '../../../../void/browser/externalAgentPicker.js';
...
import { ComposerPlusActionItem } from './composerPlusActionItem.js';
import { buildPromptPolishRequest, promptExpansionNeedsRetry, sanitizePromptPolishOutput } from './promptPolish.js';

const $ = dom.$;

const INPUT_EDITOR_MAX_HEIGHT = 250;
const INPUT_EDITOR_LINE_HEIGHT = 20;
/* V3Code: one row shorter than upstream (was 12/12) — Cursor-tight composer */
const INPUT_EDITOR_PADDING = { compact: { top: 2, bottom: 2 }, default: { top: 6, bottom: 6 } };
...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	/* V3Code: prompt-cache countdown ghost text in the composer's top-right corner. Armed when
	 * a reply ends, counts the 5-minute cache window down, cleared the moment a prompt is sent. */
	private _cacheCountdownEndedAt: number | null = null;
	private _cacheCountdownSession: string | undefined;
	private readonly _cacheCountdownTicker = this._register(new MutableDisposable<IDisposable>());
	...
	/** V3Code: subscribe the countdown element to the prompt-cache clock. */
	private _installCacheCountdown(el: HTMLElement): void {
		const render = () => {
			const current = this._widget?.viewModel?.model.sessionResource?.toString();
			const remaining = this._cacheCountdownEndedAt === null || current !== this._cacheCountdownSession
				? 0
				: cacheCountdownRemainingMs(this._cacheCountdownEndedAt, Date.now());
			if (remaining <= 0) {
				this._cacheCountdownEndedAt = null;
				this._cacheCountdownTicker.clear();
				el.textContent = '';
				el.classList.remove('visible');
				return;
			}
			el.textContent = formatCacheCountdown(remaining);
			el.classList.add('visible');
		};
		this._register(subscribeV3CacheClock(event => {
			const mine = this._widget?.viewModel?.model.sessionResource?.toString();
			if (!mine) { return; }
			const next = applyCacheClockEvent(this._cacheCountdownEndedAt, event, mine);
			if (next === this._cacheCountdownEndedAt) { return; }
			this._cacheCountdownEndedAt = next;
			this._cacheCountdownSession = mine;
			if (next !== null && !this._cacheCountdownTicker.value) {
				this._cacheCountdownTicker.value = dom.disposableWindowInterval(dom.getWindow(el), render, 1000);
			}
			render();
		}));
	}

	/** V3Code: one-line hint that a background agent reported while this session was idle. */
	showV3BackgroundNotice(message: string): void {
		this.chatInputNotificationService.setNotification({
			id: 'v3code.backgroundNotice',
	...
	private composerPlusWidget: ComposerPlusActionItem | undefined;
	private permissionWidget: PermissionPickerActionItem | undefined;
	private _promptPolishRequest: CancellationTokenSource | undefined;
	...
	public get currentModeInfo(): IChatRequestModeInfo {
		const mode = this._currentModeObservable.get();
		const modeId: 'ask' | 'agent' | 'edit' | 'custom' | undefined = mode.isBuiltin ? (this.currentModeKind as 'ask' | 'agent' | 'edit' | 'custom') : 'custom';
	...
	constructor(
		...
		this._register(toDisposable(() => {
			this._promptPolishRequest?.dispose(true);
			this._promptPolishRequest = undefined;
		}));
	...
	/**
	 * V3Code: the permission (approval) picker now lives on the PRIMARY input
	 * toolbar (next to Agent) instead of the secondary toolbar, so both toolbar
	 * providers build it from the same delegate. Extracted here to avoid duplicating
	 * the ~35-line delegate in two places.
	 */
	...
	/** V3Code: wire a freshly-created permission picker as the active permissionWidget. */
	private _attachPermissionWidget(widget: PermissionPickerActionItem): PermissionPickerActionItem {
	...
	public openComposerPlus(): void {
		this.composerPlusWidget?.show();
	}

	/**
	 * Expand the current draft with the selected model without entering the agent
	 * loop. The replacement is race-safe and remains a single undoable editor edit.
	 */
	public async polishPrompt(): Promise<void> {
		const editor = this.inputEditor;
		const model = editor.getModel();
		const modelId = this.currentLanguageModel;
		const original = editor.getValue();
		if (!model || !original.trim()) {
			this.chatInputNotificationService.setNotification({
				id: 'v3code.promptPolish',
				severity: ChatInputNotificationSeverity.Info,
				message: localize('v3code.promptPolish.empty', "Write an idea first, then choose Expand prompt."),
				description: undefined,
				actions: [],
				dismissible: true,
				autoDismissOnMessage: true,
		...
		if (!modelId) {
			this.chatInputNotificationService.setNotification({
				...
				message: localize('v3code.promptPolish.noModel', "Choose a model before expanding this prompt."),
				description: undefined,
				actions: [],
				dismissible: true,
				autoDismissOnMessage: true,
		...
		if (original.length > 24_000) {
			this.chatInputNotificationService.setNotification({
				id: 'v3code.promptPolish',
				severity: ChatInputNotificationSeverity.Warning,
				message: localize('v3code.promptPolish.tooLong', "This draft is too long to expand safely. Shorten it below 24,000 characters."),
				description: undefined,
				actions: [],
				dismissible: true,
				autoDismissOnMessage: true,
		...
		this._promptPolishRequest?.dispose(true);
		this._promptPolishRequest = request;
		this.inputContainer.classList.add('v3-prompt-polishing');
		this.inputContainer.setAttribute('aria-busy', 'true');
		aria.status(localize('v3code.promptPolish.working', "Expanding prompt with the selected model"));
		...
		try {
			...
			for (let attempt = 0; attempt < 2; attempt++) {
				try {
					const response = await this.languageModelsService.sendChatRequest(
						modelId,
						undefined,
						[{ role: ChatMessageRole.User, content: [{ type: 'text', value: buildPromptPolishRequest(original, attempt > 0) }] }],
						{},
						request.token,
					);
					...
					const candidate = sanitizePromptPolishOutput(text);
					if (!candidate) {
						throw new Error('The selected model returned an empty expansion.');
					}
					if (promptExpansionNeedsRetry(original, candidate)) {
						throw new Error('The selected model only lightly rewrote the idea instead of expanding it.');
					}
					polished = candidate;
					break;
				} catch (error) {
					...
					if (attempt === 0) {
						this.logService.warn('[ChatInputPart] prompt expansion attempt failed; retrying once', error);
						continue;
					}
				}
			}
			if (!polished) {
				throw lastError ?? new Error('The selected model could not expand this prompt.');
			}
			if (editor.getModel() !== model || model.getVersionId() !== version || editor.getValue() !== original) {
				this.chatInputNotificationService.setNotification({
					id: 'v3code.promptPolish',
					severity: ChatInputNotificationSeverity.Info,
					message: localize('v3code.promptPolish.changed', "Your draft changed while the rewrite was running, so V3Code left it untouched."),
					description: undefined,
					actions: [],
					dismissible: true,
					autoDismissOnMessage: true,
			...
			editor.pushUndoStop();
			editor.executeEdits('v3code.promptPolish', [{ range: model.getFullModelRange(), text: polished, forceMoveMarkers: true }]);
			editor.pushUndoStop();
			editor.setPosition(getLastPosition(model));
			this._widget?.refreshParsedInput();
			this.chatInputNotificationService.deleteNotification('v3code.promptPolish');
			this.focus();
			aria.status(localize('v3code.promptPolish.done', "Prompt expanded. Undo to restore the original draft."));
		} catch (error) {
			if (!request.token.isCancellationRequested) {
				this.logService.error('[ChatInputPart] prompt polish failed', error);
				this.chatInputNotificationService.setNotification({
					id: 'v3code.promptPolish',
					severity: ChatInputNotificationSeverity.Error,
					message: localize('v3code.promptPolish.failed', "V3Code could not expand this prompt. Your draft was not changed."),
			...
			if (this._promptPolishRequest === request) {
				request.dispose();
				this._promptPolishRequest = undefined;
				this.inputContainer.classList.remove('v3-prompt-polishing');
				this.inputContainer.removeAttribute('aria-busy');
			}
		}
	}
	...
	*/
	...
	private getModels(): ILanguageModelChatMetadataAndIdentifier[] {
		...
		// V3Code: the chat model picker must surface ONLY the V3Code model system —
		// V3Pro / V3Fast and the user's BYOK models, which all register under the
		// `v3code` vendor. Other vendors (notably the bundled GitHub Copilot extension's
		// `copilot` / `copilotcli` models, including stale cached rows) are hidden so the
		// picker mirrors the V3Code model settings. Fall back to the unfiltered list only
		// if no v3code model is present (e.g. the provider hasn't registered yet) so the
		// picker never renders empty.
		const v3codeModels = visible.filter(m => m.metadata.vendor === 'v3code');
		return v3codeModels.length > 0 ? v3codeModels : visible;
	}
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
				...
				dom.h('.interactive-input-and-edit-session', [
					...
					dom.h('.interactive-input-and-side-toolbar@inputAndSideToolbar', [
						dom.h('.chat-input-container@inputContainer', [
							...
							dom.h('.v3code-cache-countdown@cacheCountdown'),
						]),
					]),
		...
		this._installCacheCountdown(elements.cacheCountdown);
		...
		// Chat owns its drop surface through ChatDragAndDrop. Enabling Monaco's generic
		// drop controller here lets the same image drop run twice: chat attaches the
		// image while the editor inserts macOS' temporary screenshot path as text.
		// Keep the chat-aware paste controller, but leave generic editor drops disabled.
		...
		// V3Code: Approve and Agent are STATE MARKS, not reading material — they are
		// glanced at, never read, and their words ("Approve", "Agent") are the two
		// widest things on the toolbar. Pinning them compact at every width keeps the
		// composer to one readable label (the model), which is the only control whose
		// value you actually need to read. This also removes the width-dependent
		// reflow where labels appeared/vanished as the panel was resized.
		// Note: `compact` means LETTER for the permission picker (B/D/A) and
		// ICON-ONLY for the mode picker — see their renderLabel implementations.
		const alwaysCompactPickerOptions: IChatInputPickerOptions = {
			...pickerOptions,
		...
		// Shrink priority: Approve / Mode / Agent collapse FIRST (low min-width);
		// Model picker is protected (high min-width) because users resize chat
		// specifically to pick a model, not to approve or switch agents.
		const shrinkFirstActionIds = new Set<string>([
			OpenComposerPlusAction.ID,
		...
		this.inputActionsToolbar = this._register(this.instantiationService.createInstance(MenuWorkbenchToolBar, this.options.renderInputToolbarBelowInput ? this.attachmentsContainer : toolbarsContainer, MenuId.ChatInput, {
			...
			responsiveBehavior: {
				...
				minItems: 1,
				actionMinWidth: 48,
				getActionMinWidth: action => shrinkFirstActionIds.has(action.id) ? 22 : (action.id === OpenModelPickerAction.ID ? 72 : undefined),
			},
			actionViewItemProvider: (action, options) => {
				if (action.id === EXTERNAL_AGENT_PICKER_ID && action instanceof MenuItemAction) {
					return this.instantiationService.createInstance(ExternalAgentPicker, action, widget, location === ChatWidgetLocation.Editor ? 'editor' : 'sidebar', alwaysCompactPickerOptions);
				}
				if (action.id === OpenComposerPlusAction.ID && action instanceof MenuItemAction) {
					return this.composerPlusWidget = this.instantiationService.createInstance(
						ComposerPlusActionItem,
						action,
						{ polishPrompt: () => this.polishPrompt() },
						pickerOptions,
					);
				}
				...
				if (action.id === OpenModelPickerAction.ID && action instanceof MenuItemAction) {
					...
					return this.modeWidget = this.instantiationService.createInstance(ModePickerActionItem, action, delegate, alwaysCompactPickerOptions);
					...
					// V3Code: the D/B/A approval pill now lives on the primary toolbar (next
					// to Agent, where the unused "+" add-context button used to be).
					return this._attachPermissionWidget(
						this.instantiationService.createInstance(PermissionPickerActionItem, action, this._createPermissionPickerDelegate(), alwaysCompactPickerOptions)
					);
				}
				return undefined;
			}
		...
		this.secondaryToolbar = this._register(this.instantiationService.createInstance(MenuWorkbenchToolBar, this.secondaryToolbarContainer, MenuId.ChatInputSecondary, {
			...
			actionViewItemProvider: (action, options) => {
				if ((action.id === OpenSessionTargetPickerAction.ID || action.id === OpenDelegationPickerAction.ID) && action instanceof MenuItemAction) {
					...
					// V3Code: kept for backwards compat if the picker is ever shown in the
					// secondary toolbar; normally it now renders on the primary toolbar.
					return this._attachPermissionWidget(
						this.instantiationService.createInstance(PermissionPickerActionItem, action, this._createPermissionPickerDelegate(), secondaryPickerOptions)
					);
				} else if (
	...
	private _layout(width: number, allowRecurse = true): void {
		...
		const fallbackWidth = width - data.inputPartHorizontalPadding - data.editorBorder - data.inputPartHorizontalPaddingInside - data.toolbarsWidth - data.sideToolbarWidth;
		/*
		 * Measure the editor host's real content box. Guessing from widget width
		 * drifts from void.css capsule padding / left-only chat-editor padding, so
		 * wrap ends past the right edge and overflow:hidden eats characters.
		 */
		let newEditorWidth = fallbackWidth;
		if (this._editorContainer) {
			const win = dom.getWindow(this._editorContainer);
			const style = win.getComputedStyle(this._editorContainer);
			const padL = parseFloat(style.paddingLeft) || 0;
			const padR = parseFloat(style.paddingRight) || 0;
			const contentW = this._editorContainer.clientWidth - padL - padR;
			if (contentW > 40) {
				// Small reserve for vertical scrollbar / glyph edge (eased back from 10).
				newEditorWidth = Math.max(50, contentW - 4);
			}
		}
	...
	private getLayoutData() {
		...
		return {
			...
			/* Fallback only — live width comes from _editorContainer measurement in _layout. */
