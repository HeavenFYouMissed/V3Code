...
import { buildPhrasePool, createThinkingIcon, createThinkingSnake, getToolInvocationIcon } from './chatThinkingContentPart.js';
...
export class ChatSubagentContentPart extends ChatCollapsibleContentPart implements IChatContentPart {
	...
	// Travelling green-dot "snake" shown left of the title while the subagent is active. Subagents run
	// collapsed (their working-spinner content isn't rendered until expanded), so this is the visible
	// "working now" motion on the agent card. Reuses the thinking row's snake markup/CSS.
	private titleSnake: HTMLElement | undefined;
	...
	constructor(
		...
		// Apply shimmer to the initial title when still active, with the travelling snake to its left.
		if (!this.element.isComplete && this._collapseButton) {
			const labelElement = this._collapseButton.labelElement;
			labelElement.textContent = '';
			this.titleSnake = this.buildTitleSnake();
			labelElement.appendChild(this.titleSnake);
	...
	/**
	 * Builds the travelling green-dot snake (same markup the thinking row uses). Each dot's
	 * animation-delay is staggered by index so the lit crest moves left-to-right. Styles live in
	 * chatThinkingContent.css and apply because the subagent domNode carries `chat-thinking-box`.
	 */
	private buildTitleSnake(): HTMLElement {
		return createThinkingSnake('v3-subagent-title-snake');
	}
	...
	private createWorkingSpinner(): void {
		...
		// Snake: same travelling green-dot wave as the thinking row's active spinner (the motion
		// Daniel asked the subagent card to match). Pure CSS — each dot's animation-delay is staggered
		// by index so the lit crest moves left-to-right. Styles live in chatThinkingContent.css and are
		// disabled under prefers-reduced-motion; they apply here because the subagent domNode carries
		// the `chat-thinking-box` class.
		this.workingSpinnerElement.appendChild(createThinkingSnake());
	...
	public markAsInactive(): void {
		...
		this.domNode.classList.add('chat-subagent-completed');
		if (this._collapseButton) {
			this._collapseButton.icon = Codicon.check;
		}
	...
	private updateTitle(): void {
		...
		// Ensure the persistent shimmer span exists, with the travelling snake to its left.
