...
export class ChatToolProgressSubPart extends BaseChatToolInvocationSubPart {
	...
	private renderProgressContent(content: IMarkdownString | string, shouldAnnounce: boolean) {
		...
		return this.instantiationService.createInstance(ChatProgressContentPart, progressMessage, this.renderer, this.context, undefined, true, this.getIcon(isV3TranscriptResponse(this.context.element)), this.toolInvocation, shouldShimmerForTool(this.toolInvocation));
	}
