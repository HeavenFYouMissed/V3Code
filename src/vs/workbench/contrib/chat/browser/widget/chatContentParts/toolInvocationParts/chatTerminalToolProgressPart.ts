...
export class ChatTerminalToolProgressPart extends BaseChatToolInvocationSubPart implements IChatTerminalToolProgressPart {
	...
	constructor(
		...
		// supportHtml has to carry over with the value: presenters put colored <span>s in this
		// message (exit codes, +N/-M), and rebuilding without the flag renders them as literal tags.
		const markdownContent = new MarkdownString(pastTenseMessage, {
			supportThemeIcons: true,
			supportHtml: isMarkdownString(toolInvocation.pastTenseMessage) ? toolInvocation.pastTenseMessage.supportHtml : false,
		...
		if (this._usesCollapsibleWrapper) {
			// The wrapper's own header already shows the command, so the inner title block
			// would be the same text a second time, boxed. Flag it for the stylesheet so the
			// card reads as one piece: header, then output.
			elements.container.classList.add('chat-terminal-command-shown-in-header');
	...
	public async focusTerminal(): Promise<void> {
		...
		if (instance) {
			...
			} else if (this._shouldRevealInEditorArea()) {
				// In agent surfaces (agent mode / Agent window) the bottom panel is deliberately
				// kept hidden — revealing a tool terminal with showPanel() pops the panel out of
				// place and deforms the workbench. Terminals live in the editor area there, so move
				// this one into the editor (its stable home), matching a normal terminal open. Only
				// panel-located terminals that belong to a group can move; if the move can't happen
				// (e.g. a hidden background terminal), fall back to showing the panel.
				this._terminalService.moveToEditor(instance);
				// moveToEditor mutates instance.target synchronously. The check is written as
				// "did it stay in the panel" rather than "is it now in the editor" because the
				// compiler already narrowed target to Panel|undefined in this branch, so an
				// Editor comparison is statically dead even though it is true at runtime.
		...
		// Neither an instance nor a command URI. Returning here is what made the card's terminal
		// button look broken: the click was handled and then nothing happened. Reveal the panel so
		// the button always lands somewhere, and say why, since the tool session should have
		// registered an instance and the id not resolving is the actual bug to chase.
		this._logService.warn(`[chat] terminal card found no instance for tool session ${this._terminalData.terminalToolSessionId ?? '<none>'} — revealing the panel instead`);
		if (!this._shouldRevealInEditorArea()) {
			// In agent surfaces the panel is hidden on purpose; with no instance to reveal there is
			// nothing to move to the editor, so avoid popping the bottom panel and deforming the layout.
			await this._terminalGroupService.showPanel(true);
		}
	}

	/**
	 * True when the chat terminal card should reveal a tool terminal in the editor area instead of
	 * the bottom panel. Agent surfaces (in-window agent mode or the dedicated Agent window) keep the
	 * panel hidden by design; forcing it open from this card is what dropped the terminal at the
	 * bottom out of place. Mirrors TerminalToolService._shouldRevealTerminalForAgent.
	 */
	private _shouldRevealInEditorArea(): boolean {
		const agentMode = this._contextKeyService.getContextKeyValue<boolean>('v3code.agentMode') === true;
		if (!agentMode) {
			return false;
		}
...
export class ChatTerminalThinkingCollapsibleWrapper extends ChatCollapsibleContentPart {
	...
	constructor(
		...
		this._addOpenTerminalButton();
		this.setExpanded(initialExpanded);
	}

	/**
	 * Glyph on the right of the header that jumps to the terminal this command ran in. The
	 * existing "Show" link only appears for background commands; this is the always-there
	 * equivalent, and it swallows the click so it does not also toggle the card.
	 */
	private _addOpenTerminalButton(): void {
		if (!this._onFocusTerminal || !this._collapseButton) {
			return;
		}
		const button = dom.$('a.chat-terminal-open-button');
		button.classList.add(...ThemeIcon.asClassNameArray(Codicon.terminal));
		button.role = 'button';
		button.tabIndex = 0;
		const label = localize('chat.terminal.openTerminal', "Open terminal");
		...
		this._register(dom.addDisposableListener(button, dom.EventType.KEY_DOWN, e => {
			...
			if (keyboardEvent.equals(KeyCode.Enter) || keyboardEvent.equals(KeyCode.Space)) { activate(e); }
		}));
		this._collapseButton.element.appendChild(button);
	}
