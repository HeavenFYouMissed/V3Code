...
import { formatStreamedToolInput } from '../../../../../void/common/v3DebugTranscript.js';
...
export class ChatToolStreamingSubPart extends BaseChatToolInvocationSubPart {
	...
	private createStreamingPart(): HTMLElement {
		...
		this._register(autorun(reader => {
			...
			// The transcript shows the arguments as they stream in — plain text, never rendered
			// as markdown — so "preparing" is visibly distinct from "running" everywhere.
			if (isV3TranscriptResponse(this.context.element)) {
				const streamed = formatStreamedToolInput(currentState.partialInput.read(reader));
				if (streamed) {
					container.appendChild(dom.$('.v3-debug-streamed-input', undefined, streamed));
				}
			}
		}));
