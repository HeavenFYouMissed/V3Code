...
import { buildPhrasePool, createThinkingSnake, defaultThinkingMessages, maybePickFunWorkingMessage } from './chatThinkingContentPart.js';
...
export class ChatProgressContentPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		this.isStreaming = isResponseVM(context.element) && !context.element.isComplete;
		...
		// An explicit `shimmer: true` from the caller is authoritative for as long as the response is
		// still streaming. It used to be ANDed with `showSpinner`, which is only true when this part is
		// the LAST piece of content -- so the moment anything followed it (a second queued
		// progressMessage, a retry/nudge notice) the flag was silently dropped, `.shimmer-progress` was
		// never added, and the label fell back to flat `descriptionForeground` grey with a hidden check
		// icon. When no flag is passed we keep the old spinner-scoped default.
		const useShimmer = shimmer === true
			? (forceShowSpinner ?? this.isStreaming)
			: ((shimmer ?? (!icon || isLoadingIcon)) && this.showSpinner);
		...
		if (useShimmer) {
			...
			// The shimmer hides the codicon slot, so the row has no motion of its own if the text
			// gradient is flattened by a theme override. Lead with the same travelling green dots the
			// thinking row uses: transform/opacity motion that cannot be defeated by a color rule.
			this.domNode.insertBefore(createThinkingSnake(), this.domNode.firstChild);
		}
	...
	hasSameContent(other: IChatRendererContent, followingContent: IChatRendererContent[], element: ChatTreeItem): boolean {
		...
		// The shimmer now tracks the streaming state rather than the spinner, so a response that
		// finishes while this row is not the last part must still re-render to drop the animation.
		if (this.isStreaming !== (isResponseVM(element) && !element.isComplete)) {
			return false;
		}
