...
export abstract class AbstractToolConfirmationSubPart extends BaseChatToolInvocationSubPart {
	...
	protected render(config: IToolConfirmationConfig) {
		...
		if (customOptions && customOptions.length > 2) {
			// V3Code: 3+ options is a real multiple-choice question (ask_user), not an
			// allow/deny pair — render ONE row per option instead of collapsing the tail
			// into a dropdown where the choices are invisible. Styled as a vertical list
			// via .chat-multi-choice-question (see void.css).
			buttons = customOptions.map((option): IChatConfirmationButton<(() => void)> => ({
				label: option.label,
				data: () => {
		...
		if (customOptions && customOptions.length > 2) {
			confirmWidget.domNode.classList.add('chat-multi-choice-question');
		}
	...
	private buildCustomOptionButtons(toolInvocation: IChatToolInvocation, options: readonly ConfirmationOption[]): IChatConfirmationButton<(() => void)>[] {
		// V3Code ask_user — each option is a distinct answer; never collapse into Approve + dropdown.
		if (toolInvocation.toolId === 'v3code_ask_user') {
