...
import { isV3TranscriptResponse } from '../../../../../void/browser/v3TranscriptResponse.js';
import { categorizeDebugTool, describeToolLifecycle, observeToolInvocation, V3_ASK_USER_TOOL_ID, V3ToolCategory, V3ToolLifecycle } from '../../../../../void/common/v3DebugTranscript.js';
...
export class ChatToolInvocationPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		this._isV3TranscriptResponse = isV3TranscriptResponse(context.element);
		this.domNode = dom.$('.chat-tool-invocation-part');
		if (toolInvocation.presentation === 'hidden') {
			return;
		}

		// Push any todoList data into the todo service so the widget above the chat
		// input renders it. This must run not only at construction but also when the
		// data arrives *after* a streaming card was created (the agent does
		// beginToolCall({ force: true }) -> transitionFromStreaming, so toolSpecificData
		// is undefined at construction and is filled in later). Running it only in the
		// constructor silently drops the agent's update_plan todos.
		const syncTodoList = () => {
		...
		if (toolInvocation.kind === 'toolInvocation') {
			...
			this._register(autorun(reader => {
				...
				if (stateChanged || dataKindChanged) {
					// A presenter can replace partial todo data with the final merged list
					// while keeping the same `todoList` kind. The state transition is the
					// observable signal that the non-observable payload changed.
					syncTodoList();
					render();
				}
			}));
		...
		const render = () => {
			...
			// Surface the live tool lifecycle as a data-status attribute so the stylesheet can
			// show pending/running/done/cancelled. Read-only — never affects control flow.
			this._applyStatus();
	...
	/**
	 * Reflect the tool's lifecycle on the DOM node as `data-status` so the stylesheet can render
	 * pending -> running -> completed -> cancelled. Driven by the live tool state (not by scroll
	 * position), so it is safe on the virtualized chat list. Read-only.
	 */
	private _applyStatus(): void {
		const ti = this.toolInvocation;
		let status: 'streaming' | 'pending' | 'running' | 'completed' | 'cancelled' | 'skipped';
		if (ti.kind === 'toolInvocation') {
			const stateType = ti.state.get().type;
			if (stateType === IChatToolInvocation.StateKind.Streaming) {
				status = 'streaming';
			} else if (stateType === IChatToolInvocation.StateKind.WaitingForConfirmation || stateType === IChatToolInvocation.StateKind.WaitingForPostApproval) {
				status = 'pending';
			} else if (!IChatToolInvocation.isComplete(ti)) {
				status = 'running';
			} else {
				status = this._terminalStatus(ti);
			}
		} else {
			status = this._terminalStatus(ti);
		}
		this.domNode.dataset.status = status;

		// Every V3Code transcript: the observed lifecycle (preparing / awaiting-approval / running /
		// succeeded / failed / cancelled / skipped) and the sheen, which runs only during observed
		// active work. Rows this editor did not render get neither attribute nor class.
		if (this._isV3TranscriptResponse) {
			const description = describeToolLifecycle(observeToolInvocation(ti));
			this.domNode.dataset.v3Lifecycle = description.lifecycle;
			this.domNode.classList.toggle('v3-sheen', description.isActive);
		}
	}

	private readonly _isV3TranscriptResponse: boolean;

	/** Observed lifecycle of the underlying call (the transcript grouping and live status read this). */
	get v3Lifecycle(): V3ToolLifecycle {
		return describeToolLifecycle(observeToolInvocation(this.toolInvocation)).lifecycle;
	}

	get v3Category(): V3ToolCategory {
		return categorizeDebugTool(this.toolInvocation.toolId, this.toolInvocation.toolSpecificData?.kind);
	}

	get isQuestionTool(): boolean {
		return this.toolInvocation.toolId === V3_ASK_USER_TOOL_ID;
	}

	/** Hidden presentations render an empty node — they neither join nor break a Debug group. */
	get isVisuallyHidden(): boolean {
		return this.domNode.style.display === 'none' || !this.domNode.hasChildNodes();
	}

	private _terminalStatus(ti: IChatToolInvocation | IChatToolInvocationSerialized): 'completed' | 'cancelled' | 'skipped' {
		const confirmState = IChatToolInvocation.executionConfirmedOrDenied(ti);
		if (confirmState?.type === ToolConfirmKind.Denied) {
			return 'cancelled';
		}
		if (confirmState?.type === ToolConfirmKind.Skipped) {
			return 'skipped';
		}
		return 'completed';
	}
