...
export class CodeBlockPart extends Disposable {
	...
	constructor(
		...
		// The first layout() runs while this element is still off-DOM, where clientWidth is 0, so
		// the clamp in layout() cannot apply and the block paints at the caller's over-wide value.
		// It then looked correct as soon as ANYTHING triggered a relayout -- hovering the block was
		// enough, which is exactly the "it fixes itself the moment I touch a border" symptom.
		// Watch the element so the real width is applied the moment it has one, and again whenever
		// the column resizes (panel drag, sidebar toggle).
		//
		// This cannot loop: layout() sizes the editor INSIDE this element, and the guard only acts
		// on a WIDTH that differs from the last one laid out, so it converges after one pass.
		const widthObserver = this._register(new dom.DisposableResizeObserver('CodeBlockPart.width', () => {
			const observedWidth = this.element.clientWidth;
			if (observedWidth > 0 && observedWidth !== this.lastLayoutWidth) {
				this.layout();
			}
		}));
		this._register(widthObserver.observe(this.element));
	...
	layout(width = this.lastLayoutWidth): void {
		...
		// The width handed down here is the LIST ROW width minus a hard-coded inset
		// (chatListRenderer.layout: `width - 40`). That constant stopped matching reality when the
		// chat gutter was widened to the Cursor-style inset: the row is 584, the real content
		// column is 488, but the editor was told it had 544. Monaco then laid itself out 542 wide
		// inside a 486 container whose overflow-x is `visible`, so a code block spilled 56px past
		// the right edge of the message instead of scrolling inside it -- borders cut off, lines
		// running under the edge of the pane. The diff variant below never had this bug because it
		// sets element.style.width explicitly; this one does not.
		//
		// Clamp to the width this block ACTUALLY has. Self-correcting: if the gutter changes
		// again, or another surface insets differently, this follows it without a second magic
		// number. clientWidth is 0 while the element is still off-DOM (render() notes the caller
		// attaches it afterwards) -- keep the caller's value then, and onDidRemount re-runs this
		// once the element is connected and measurable.
	...
	onDidRemount(): void {
		if (this.currentCodeBlockData) {
			...
			// !!!!
			// Re-layout FIRST: off-DOM the element measures 0, so layout() could not clamp to the
			// real column width and fell back to the caller's over-wide value. Now that it is
			// connected the measurement works, which is what stops the block overflowing.
			this.layout();
			this.editor.renderAsync(true);
		}
	}
