...
import { getV3CollapsedToolsDisplayModeForResponse } from '../../../void/browser/v3codeConversationDensity.js';
import { isV3DebugResponse } from '../../../void/browser/v3DebugResponse.js';
import { removeV3DebugLiveStatus, updateV3DebugLiveStatus, V3DebugToolGroupPart } from '../../../void/browser/v3DebugTranscriptView.js';
import { isV3CodeAgentIdentity, isV3TranscriptResponse, V3_TRANSCRIPT_RESPONSE_CLASS } from '../../../void/browser/v3TranscriptResponse.js';
import { debugLiveStatusIsActive, debugLiveStatusText, parseTranscriptDensity, planDebugToolGroups, v3DebugIdleWorkingText, V3_TRANSCRIPT_DENSITY_SETTING_ID, V3DebugGroupExpansionStore, V3LiveStatusEntry, V3TranscriptItem } from '../../../void/common/v3DebugTranscript.js';
...
import { v3AgentDevilUri } from '../../../void/browser/v3BrandAssets.js';
import { updateV3VoiceSurfaceForResponse } from '../../../void/browser/v3codeVoiceOrbSurface.js';
import { AccessibilityWorkbenchSettingId } from '../../../accessibility/browser/accessibilityConfiguration.js';
import { isMcpToolInvocation, shouldKeepMcpToolStandalone } from './chatContentParts/toolInvocationParts/chatToolPartUtilities.js';
...
/**
 * Whether a live tool invocation belongs in the rolling working group.
 *
 * Confirmation metadata deliberately survives into Executing and Completed
 * states, so eligibility must be based on the current state rather than the
 * presence of that metadata.
 */
export function isToolInvocationStatePinnable(state: IChatToolInvocation.StateKind): boolean {
	return state === IChatToolInvocation.StateKind.Streaming
		|| state === IChatToolInvocation.StateKind.Executing
		|| state === IChatToolInvocation.StateKind.Completed;
}

export type ToolInvocationGroupingTransition = 'moveOut' | 'rejoin' | undefined;

/** Returns the rolling-group ownership transition for a tool state change. */
export function getToolInvocationGroupingTransition(
	state: IChatToolInvocation.StateKind,
	hasMovedOutPart: boolean,
	isAttachedToThinking = false,
): ToolInvocationGroupingTransition {
	if (state === IChatToolInvocation.StateKind.WaitingForConfirmation || state === IChatToolInvocation.StateKind.WaitingForPostApproval) {
		return 'moveOut';
	}

	// A collapsed group keeps ownership in movedOutToolParts until its lazy child
	// materializes. `isAttachedToThinking` is the in-flight ownership bit: without
	// this guard Executing and then Completed would enqueue the same tool twice.
	return hasMovedOutPart && !isAttachedToThinking && isToolInvocationStatePinnable(state) ? 'rejoin' : undefined;
}

/**
 * Preview-style groups close as soon as narration resumes. Fixed-scrolling
 * groups defer that close until later work actually starts so partially
 * streamed narration cannot fragment one live work chapter.
 */
export function shouldFinalizeThinkingPhase(
	hasNarrationBoundary: boolean,
	responseFinished: boolean,
	displayMode: ThinkingDisplayMode,
): boolean {
	return responseFinished || (hasNarrationBoundary && displayMode !== ThinkingDisplayMode.FixedScrolling);
}

/** Returns whether deferred fixed-scrolling narration should start a new work chapter. */
export function shouldRolloverThinkingPhase(
	displayMode: ThinkingDisplayMode,
	isNextWorkPart: boolean,
	activeThinkingMatchesBoundary: boolean,
	activeThinkingIndex: number,
	narrationContentIndex: number,
	nextContentIndex: number,
): boolean {
	return displayMode === ThinkingDisplayMode.FixedScrolling
		&& isNextWorkPart
		&& activeThinkingMatchesBoundary
		&& activeThinkingIndex >= 0
		&& activeThinkingIndex < narrationContentIndex
		&& narrationContentIndex < nextContentIndex;
}
...
export interface IChatListItemTemplate {
	...
	/**
	 * Debug transcripts only: disclosure wrappers around runs of completed successful
	 * operations, plus the single live status region. Same lifecycle as `renderedParts`.
	 */
	v3DebugGroups?: DisposableMap<string, V3DebugToolGroupPart>;
	v3DebugLiveStatus?: HTMLElement;
	/**
	 * Fixed-scrolling narration does not close a rolling group immediately: the
	 * markdown may still be streaming. Keep the exact group that narration
	 * followed, then close it only if a later work part arrives.
	 */
	pendingThinkingPhaseBoundary?: {
		readonly narrationContentIndex: number;
		readonly thinkingPart: ChatThinkingContentPart;
	};
	/**
...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
	...
	/** Debug transcript group expansion, keyed by response identity plus member identity. */
	private readonly v3DebugExpansion = new V3DebugGroupExpansionStore();
	private v3DebugGroupSequence = 0;
	// Element ids that have already played their entry animation. Tracked so the virtualized list
	// recycling a template on scroll never re-fires the slide-in for the same element.
	private readonly _animatedElementIds = new Set<string>();
	...
	private clearRenderedParts(templateData: IChatListItemTemplate): void {
		...
		templateData.v3DebugGroups?.dispose();
		templateData.v3DebugGroups = undefined;
		templateData.v3DebugLiveStatus = removeV3DebugLiveStatus(templateData.v3DebugLiveStatus);
		templateData.pendingThinkingPhaseBoundary = undefined;
	...
	private renderChatTreeItem(element: ChatTreeItem, index: number, templateData: IChatListItemTemplate): void {
		...
		templateData.rowContainer.classList.remove('pending-item', 'pending-divider', 'pending-request', 'chat-pending-dragging', 'animate-in');
		...
		// Entry animation: slide a row in the first time its element renders. Keyed by element id
		// so scroll recycling never replays it. Existing history animates once as it first scrolls
		// into view, reading as the chat materializing rather than popping.
		if (!this._animatedElementIds.has(element.id)) {
			this._animatedElementIds.add(element.id);
			templateData.rowContainer.classList.add('animate-in');
		}
		...
		// Two hooks on purpose. The transcript class carries the shared craft and is set for every
		// V3Code mode; the Debug class stays for behaviour that is genuinely Debug-only. The
		// density listener follows the craft rather than the mode, so changing the setting can
		// regroup a row that is already streaming.
		const isV3Transcript = isResponseVM(element) && isV3TranscriptResponse(element);
		templateData.rowContainer.classList.toggle(V3_TRANSCRIPT_RESPONSE_CLASS, isV3Transcript);
		templateData.rowContainer.classList.toggle('v3-debug-response', isV3Transcript && isV3DebugResponse(element));
		if (isV3Transcript) {
			templateData.elementDisposables.add(this.configService.onDidChangeConfiguration(e => {
				if (e.affectsConfiguration(V3_TRANSCRIPT_DENSITY_SETTING_ID)) {
					this.scheduleV3DebugReconcile(templateData);
				}
			}));
		}
		if (isResponseVM(element) && this.isV3CodeAgent(element)) {
			updateV3VoiceSurfaceForResponse(templateData.rowContainer, element.id, !element.isComplete, () => element.response.value
		...
		// V3Code: compact identity on sent user turns — hub display
		// name + account avatar (circular), not the generic "User" placeholder.
		// Agent turns always show "V" even when older sessions snapshotted "V3Code Agent".
		if (isRequestVM(element) && !isSystemInitiatedRequest) {
			const accountName = this.v3codeAccountService.state.displayName?.trim();
			templateData.username.textContent = accountName || localize('v3ChatUserFallback', "You");
			templateData.username.classList.toggle('hidden', this.environmentService.isSessionsWindow);
			templateData.avatarContainer.classList.toggle('hidden', this.environmentService.isSessionsWindow);
		} else if (isResponseVM(element) && this.isV3CodeAgent(element)) {
			templateData.username.textContent = 'V';
			templateData.username.classList.toggle('hidden', this.environmentService.isSessionsWindow);
			templateData.avatarContainer.classList.toggle('hidden', this.environmentService.isSessionsWindow);
		} else {
		...
		// V3Code: show header on user requests (avatar + name above the bubble)
		// as well as agent responses.
		const shouldShowHeader = ((isRequestVM(element) || isResponseVM(element)) && !this.rendererOptions.noHeader) && !isSystemInitiatedRequest;
	...
	private renderDetail(element: IChatResponseViewModel, templateData: IChatListItemTemplate): void {
		...
		} else if (this.rendererOptions.renderStyle !== 'minimal' && element.isComplete && typeof element.result?.metadata?.usedModel === 'string') {
			// V3Code: per-turn model attribution — always show which model served this turn.
			const usedModel = element.result.metadata.usedModel;
			const usedProvider = typeof element.result.metadata.usedProvider === 'string' ? element.result.metadata.usedProvider : undefined;
			templateData.detail.textContent = usedModel;
			templateData.detail.title = usedProvider ? `${usedProvider} / ${usedModel}` : usedModel;
		}
	}
	...
	private isV3CodeAgent(element: ChatTreeItem): boolean {
		// Delegates to the shared identity predicate so this row check and the transcript gate
		// can never drift apart about which responses belong to V3Code.
		return isResponseVM(element) && isV3CodeAgentIdentity(element.agent);
	}
	...
	private renderAvatar(element: ChatTreeItem, templateData: IChatListItemTemplate): void {
		...
		// Hub photo or chrome-V fallback (account service always supplies a URL).
		if (isRequestVM(element)) {
			const avatarUrl = this.v3codeAccountService.state.avatarUrl;
			if (avatarUrl) {
				const avatarIcon = dom.$<HTMLImageElement>('img.icon');
				avatarIcon.src = avatarUrl;
				avatarIcon.alt = '';
				avatarIcon.referrerPolicy = 'no-referrer';
				templateData.avatarContainer.replaceChildren(dom.$('.avatar.v3-user-avatar', undefined, avatarIcon));
				return;
			}
		}
		// Force grey-devil "V" mark for our agent — old sessions still snapshot sparkle + "V3Code Agent".
		if (isResponseVM(element) && this.isV3CodeAgent(element)) {
			const avatarIcon = dom.$<HTMLImageElement>('img.icon');
			avatarIcon.src = v3AgentDevilUri().toString(true);
			avatarIcon.alt = 'V';
			templateData.avatarContainer.replaceChildren(dom.$('.avatar.v3-agent-avatar', undefined, avatarIcon));
			return;
		}
	...
	private getAgentIcon(agent: IChatAgentMetadata | undefined): URI | ThemeIcon {
		// Prefer custom image icons over themeIcon (sparkle) so brand marks win.
		if (agent?.iconDark && isDark(this.themeService.getColorTheme().type)) {
			return agent.iconDark;
		} else if (agent?.icon) {
			return agent.icon;
		} else if (agent?.themeIcon) {
	...
	private finalizeCompletedResponseParts(element: IChatResponseViewModel, templateData: IChatListItemTemplate): void {
		const displayMode = this.configService.getValue<ThinkingDisplayMode>('chat.agent.thinkingStyle');
		if (!shouldFinalizeThinkingPhase(false, element.isComplete || element.isCanceled, displayMode)) {
			return;
		}
		templateData.pendingThinkingPhaseBoundary = undefined;
	...
	private shouldShowWorkingProgress(element: IChatResponseViewModel, partsToRender: IChatRendererContent[], moreContentAvailable: boolean, templateData: IChatListItemTemplate): IChatWorkingProgress | undefined {
		...
		if (lastPart && (lastPart.kind === 'toolInvocation' || lastPart.kind === 'toolInvocationSerialized')) {
			...
			const collapsedToolsMode = this.getCollapsedToolsMode(isResponseVM(element) ? element : undefined);
	...
	private renderChatContentDiff(partsToRender: ReadonlyArray<IChatRendererContent | null>, contentForThisTurn: ReadonlyArray<IChatRendererContent>, element: IChatResponseViewModel, elementIndex: number, templateData: IChatListItemTemplate): void {
		...
		partsToRender.forEach((partToRender, contentIndex) => {
			...
			// Narration seals the preceding fixed-scrolling chapter only when the
			// next real work item arrives. Do this before the in-place thinking/tool
			// reuse paths below so the new item creates or joins the next chapter.
			this.rolloverPendingThinkingPhaseBoundary(partToRender, context, templateData);
		...
		if (isV3TranscriptResponse(element)) {
			this.reconcileV3DebugTranscript(templateData, element);
		}
	}

	private scheduleV3DebugReconcile(templateData: IChatListItemTemplate): void {
		// Tool autoruns fire synchronously inside renderChatContentDiff; defer like
		// updateWorkingProgressForPendingConfirmations does and bail if the template was recycled.
		const originalElement = templateData.currentElement;
		queueMicrotask(() => {
			if (templateData.currentElement !== originalElement || !isResponseVM(originalElement) || !isV3TranscriptResponse(originalElement)) {
				return;
			}
			this.reconcileV3DebugTranscript(templateData, originalElement);
		});
	}

	/**
	 * Debug transcript ownership pass. Runs after every content diff and on every tool state
	 * change: wraps runs of consecutive successful operations (per the density setting) in
	 * disclosure groups by MOVING the existing tool cards, keeps the one live status region
	 * last, and never touches other modes. No second state machine — every decision reads the
	 * cards' observed lifecycle.
	 */
	private reconcileV3DebugTranscript(templateData: IChatListItemTemplate, element: IChatResponseViewModel): void {
		const renderedParts = templateData.renderedParts;
		if (!renderedParts) {
			return;
		}
		const value = templateData.value;
		const items: V3TranscriptItem[] = [];
		const statusEntries: V3LiveStatusEntry[] = [];
		const toolParts = new Map<string, ChatToolInvocationPart>();
		const indexOfNode = new Map<HTMLElement, number>();
		for (let i = 0; i < renderedParts.length; i++) {
			const part = renderedParts[i];
			if (!part?.domNode) {
				items.push({ kind: 'transparent' });
				continue;
			}
			indexOfNode.set(part.domNode, i);
			if (part instanceof ChatToolInvocationPart) {
				const lifecycle = part.v3Lifecycle;
				statusEntries.push({ lifecycle, isQuestion: part.isQuestionTool });
				if (part.isVisuallyHidden) {
					items.push({ kind: 'transparent' });
					continue;
				}
				items.push({ kind: 'tool', id: part.toolCallId, lifecycle, category: part.v3Category });
		...
		let changed = false;
		const density = parseTranscriptDensity(this.configService.getValue(V3_TRANSCRIPT_DENSITY_SETTING_ID));
		const plans = planDebugToolGroups(items, density);
		const groups = templateData.v3DebugGroups ??= new DisposableMap<string, V3DebugToolGroupPart>();
		const matched = new Set<string>();
		for (const plan of plans) {
			let wrapper: V3DebugToolGroupPart | undefined;
			for (const [key, existing] of groups) {
				if (!matched.has(key) && existing.memberIds.some(id => plan.memberIds.includes(id))) {
					wrapper = existing;
					matched.add(key);
					break;
				}
			...
			if (!wrapper) {
				const key = `v3debug-${++this.v3DebugGroupSequence}`;
				// Default OPEN while the response is still streaming, and closed only once it is
				// finished. A group that appears already-collapsed mid-run hides the very output
				// the user is watching — an operation succeeds, and the line it printed folds
				// away before it can be read. A settled transcript from history is tidy instead.
				// An explicit user toggle always wins; the store only ever holds their choice.
				const expanded = this.v3DebugExpansion.get(element.id, plan.memberIds) ?? !element.isComplete;
				const created = this.instantiationService.createInstance(V3DebugToolGroupPart, plan.memberIds, expanded, this.v3DebugRenderContext(element, templateData));
				created.addDisposable(created.onDidChangeExpansion(isExpanded => {
					this.v3DebugExpansion.set(element.id, created.memberIds, isExpanded);
					this.fireItemHeightChange(templateData);
				}));
			...
			const list = wrapper.listElement;
			const memberNodes = plan.memberIds.map(id => toolParts.get(id)!.domNode);
			// Position the wrapper where the first member currently sits (or where the group
			// that still holds it sits), then eject ex-members to the correct side.
			const first = memberNodes[0];
			if (first.parentElement !== list) {
				const anchor = first.parentElement === value ? first : (first.closest('.v3-debug-tool-group') ?? first);
				if (anchor !== wrapper.domNode && anchor.parentElement === value) {
					value.insertBefore(wrapper.domNode, anchor);
					changed = true;
				}
			}
			for (const child of Array.from(list.children)) {
				if (dom.isHTMLElement(child) && !memberNodes.includes(child)) {
					const index = indexOfNode.get(child) ?? Number.MAX_SAFE_INTEGER;
					value.insertBefore(child, index < plan.start ? wrapper.domNode : wrapper.domNode.nextSibling);
					changed = true;
				}
			}
			for (const node of memberNodes) {
				if (node.parentElement !== list || node !== list.lastElementChild) {
					list.appendChild(node);
					changed = true;
				}
			}
			if (wrapper.memberIds.length !== plan.memberIds.length || wrapper.memberIds.some((id, i) => id !== plan.memberIds[i])) {
				wrapper.setMembers(plan.memberIds);
				changed = true;
			}
		}
		for (const [key, wrapper] of Array.from(groups)) {
			if (!matched.has(key)) {
				// Dissolve in place: members go back to where the wrapper stood.
				wrapper.domNode.replaceWith(...Array.from(wrapper.listElement.children));
				groups.deleteAndDispose(key);
				changed = true;
			}
		}

		if (element.isComplete) {
			if (templateData.v3DebugLiveStatus) {
				templateData.v3DebugLiveStatus = removeV3DebugLiveStatus(templateData.v3DebugLiveStatus);
				changed = true;
			}
		} else {
			const observed = debugLiveStatusText(statusEntries);
			// Debug is the one mode that suppresses the native working indicator, so it needs this
			// idle line or the panel reads as dead between tool calls. Every other mode still has
			// that indicator, and a second "Working" here would only duplicate it. The word is
			// fixed and true for exactly as long as this response is streaming — never a timer.
			const text = observed || (isV3DebugResponse(element) ? v3DebugIdleWorkingText() : '');
			const active = debugLiveStatusIsActive(statusEntries) || (observed.length === 0 && text.length > 0);
			const previousText = templateData.v3DebugLiveStatus?.textContent ?? undefined;
			templateData.v3DebugLiveStatus = updateV3DebugLiveStatus(value, templateData.v3DebugLiveStatus, text, active);
	...
	private v3DebugRenderContext(element: IChatResponseViewModel, templateData: IChatListItemTemplate): IChatContentPartRenderContext {
		return {
			element,
			elementIndex: 0,
			content: [],
	...
	/** Tool-card density from V3Code settings (panel vs editor surface). */
	private getCollapsedToolsMode(element?: IChatResponseViewModel): CollapsedToolsDisplayMode {
		return getV3CollapsedToolsDisplayModeForResponse(this.configService, element);
	}
	...
	private shouldPinPart(part: IChatRendererContent, element?: IChatResponseViewModel): boolean {
		const collapsedToolsMode = this.getCollapsedToolsMode(element);
		...
		// Rich MCP Apps keep their own interactive surface. Plain/headless MCP calls are
		// allowed into the rolling group only for compact-all-grouped; normal confirmation
		// and streaming checks below still decide whether an individual invocation can pin.
		const isStandaloneMcpTool = (part.kind === 'toolInvocation' || part.kind === 'toolInvocationSerialized')
			&& shouldKeepMcpToolStandalone(part, collapsedToolsMode);
		if (isStandaloneMcpTool) {
			return false;
		}
		...
		if (isTerminalTool && !isContributedTerminalToolInvocation) {
			// A live terminal must leave the rolling group only while it is actually
			// blocked on the user (or was cancelled). Confirmation metadata remains on
			// the Executing/Completed states and is not evidence that it is still blocked.
			if (part.kind === 'toolInvocation' && !isToolInvocationStatePinnable(part.state.get().type)) {
				return false;
			}
		...
		if (part.kind === 'toolInvocation') {
			return isToolInvocationStatePinnable(part.state.get().type);
		}
	...
	private isThinkingPhaseWorkPart(part: IChatRendererContent, element: IChatResponseViewModel): boolean {
		if (part.kind === 'thinking') {
			return Array.isArray(part.value) ? part.value.length > 0 : !!part.value;
		}

		const canJoinRollingChapter = part.kind === 'toolInvocation'
			|| part.kind === 'toolInvocationSerialized'
			|| part.kind === 'markdownContent'
			|| part.kind === 'textEditGroup'
			|| part.kind === 'externalEdit'
			|| part.kind === 'hook';

		return canJoinRollingChapter && this.shouldPinPart(part, element);
	}

	private rolloverPendingThinkingPhaseBoundary(
		part: IChatRendererContent | null,
		context: IChatContentPartRenderContext,
		templateData: IChatListItemTemplate,
	): void {
		const pendingBoundary = templateData.pendingThinkingPhaseBoundary;
		if (!pendingBoundary || !part || !isResponseVM(context.element)) {
			return;
		}

		const displayMode = this.configService.getValue<ThinkingDisplayMode>('chat.agent.thinkingStyle');
		const activeThinking = this.getLastThinkingPart(templateData.renderedParts);
		const activeThinkingIndex = activeThinking ? templateData.renderedParts?.indexOf(activeThinking) ?? -1 : -1;
		if (!shouldRolloverThinkingPhase(
			displayMode,
			this.isThinkingPhaseWorkPart(part, context.element),
			activeThinking === pendingBoundary.thinkingPart,
			activeThinkingIndex,
			pendingBoundary.narrationContentIndex,
			context.contentIndex,
		)) {
			return;
		}

		// Consume the marker only after proving this is genuinely later work. An
		// older tool/thinking rerender must leave it available for the next chapter.
		templateData.pendingThinkingPhaseBoundary = undefined;
	...
	private finalizeCurrentThinkingPart(context: IChatContentPartRenderContext, templateData: IChatListItemTemplate): void {
		...
		if (templateData.pendingThinkingPhaseBoundary?.thinkingPart === lastThinking) {
			templateData.pendingThinkingPhaseBoundary = undefined;
		}
	...
	private renderChatContentPart(content: IChatRendererContent, templateData: IChatListItemTemplate, context: IChatContentPartRenderContext): IChatContentPart | undefined {
		try {
			...
			if (content.kind === 'treeData') {
				...
				if (isV3DebugResponse(context.element)) {
					// Debug: the live status region reports observed activity; no randomized label.
					return this.renderNoContent(other => other.kind === 'working');
				}
	...
	private renderToolInvocation(toolInvocation: IChatToolInvocation | IChatToolInvocationSerialized, context: IChatContentPartRenderContext, templateData: IChatListItemTemplate): IChatContentPart | undefined {
		...
		if (this.getCollapsedToolsMode(isResponseVM(context.element) ? context.element : undefined) === CollapsedToolsDisplayMode.Off) {
			this.finalizeCurrentThinkingPart(context, templateData);
		}
		...
		const collapsedToolsMode = this.getCollapsedToolsMode(isResponseVM(context.element) ? context.element : undefined);
		...
		if (toolInvocation.kind === 'toolInvocation' && isV3TranscriptResponse(context.element)) {
			// Every observed state change re-reconciles the transcript (grouping + live status).
	...
	private setupConfirmationTransitionWatcher(
		...
		let didFinalizeThinkingPart = false;
		const rejoinMovedOutTool = (part: ChatToolInvocationPart): void => {
			if (!isResponseVM(context.element)
				|| !this.shouldPinPart(toolInvocation, context.element)
				|| IChatToolInvocation.isEffectivelyHidden(toolInvocation)) {
				return;
			}

			// The map is the owner while the confirmation card is standalone. If it
			// no longer owns this exact part, the row was cleared/recycled and there is
			// nothing safe to reattach.
			if (templateData.movedOutToolParts?.get(toolInvocation.toolCallId) !== part) {
				return;
			}

			toolInvocation.isAttachedToThinking = true;

			// Empty groups are temporarily removed while the confirmation UI is
			// standalone. Put the original group back at the card's position before
			// moving the card into it so chat order stays stable.
			if (!thinkingPart.domNode.parentElement && part.domNode.parentElement === templateData.value) {
				templateData.value.insertBefore(thinkingPart.domNode, part.domNode);
			}

			// Keep ownership in movedOutToolParts until appendItem actually
			// materializes the child. This matters for collapsed groups, whose factory
			// may not run until the user expands them.
			dom.hide(part.domNode);
			thinkingPart.appendItem(
				() => {
					templateData.movedOutToolParts?.deleteAndLeak(toolInvocation.toolCallId);
					dom.show(part.domNode);
		...
		const moveConfirmationWidgetOutOfThinking = (): ChatToolInvocationPart => {
			const alreadyMovedPart = templateData.movedOutToolParts?.get(toolInvocation.toolCallId);
			if (alreadyMovedPart instanceof ChatToolInvocationPart) {
				// A collapsed group can defer the rejoin factory. If the tool reaches a
				// post-execution approval before that factory materializes, cancel the
				// pending grouped item and reveal the still-owned standalone card again.
				if (toolInvocation.isAttachedToThinking && thinkingPart.removeLazyItem(toolInvocation.toolId)) {
					toolInvocation.isAttachedToThinking = false;
					dom.show(alreadyMovedPart.domNode);
					if (thinkingPart.isEffectivelyEmpty()) {
						thinkingPart.domNode.remove();
					}
				}
				return alreadyMovedPart;
			}
			...
			if (!didFinalizeThinkingPart) {
				this.finalizeCurrentThinkingPart(context, templateData);
				didFinalizeThinkingPart = true;
			}

			// if the thinking part is now completely empty (no tools, no thinking text)
			if (thinkingPart.isEffectivelyEmpty()) {
				thinkingPart.domNode?.remove();
				// Keep the instance alive in renderedParts so an approved tool can
				// return to the same rolling group without recreating or double-owning
				// its content part. clearRenderedParts still owns final disposal.
			}
		...
		// Keep observing for the full invocation lifecycle. A tool may require both
		// pre-execution confirmation and post-execution approval, so a one-shot
		// watcher would leave the second approval card trapped inside the bundle.
		const disposable = autorun(reader => {
			const state = toolInvocation.state.read(reader);
			const movedPart = templateData.movedOutToolParts?.get(toolInvocation.toolCallId);
			const transition = getToolInvocationGroupingTransition(
				state.type,
				movedPart instanceof ChatToolInvocationPart,
				toolInvocation.isAttachedToThinking,
			);
			if (transition === 'moveOut') {
			...
			if (transition === 'rejoin' && movedPart instanceof ChatToolInvocationPart) {
				rejoinMovedOutTool(movedPart);
			}
		});
	...
	private renderTextEdit(context: IChatContentPartRenderContext, chatTextEdit: IChatTextEditGroup, templateData: IChatListItemTemplate): IChatContentPart {
		...
		const collapsedToolsMode = this.getCollapsedToolsMode(isResponseVM(context.element) ? context.element : undefined);
		if (isResponseVM(context.element) && collapsedToolsMode !== CollapsedToolsDisplayMode.Off && this.shouldPinPart(chatTextEdit, context.element)) {
			const partId = `textEdit-${context.contentIndex}-${chatTextEdit.uri.toString()}`;
			// ChatThinkingContentPart does not have a separate textEditGroup metadata
			// shape. Reuse its external-edit metadata contract so the grouped diff gets
			// the correct pencil icon and an "Edited <file>" title.
			const editMetadata: IChatExternalEdit = {
				kind: 'externalEdit',
				uri: chatTextEdit.uri,
				editKind: 'edit',
			};
			const appendToThinking = (thinkingPart: ChatThinkingContentPart) => thinkingPart.appendItem(
				() => ({ domNode: textEditPart.domNode, disposable: textEditPart }),
				partId,
				editMetadata,
				templateData.value,
				undefined,
			...
			if (!lastThinking && collapsedToolsMode === CollapsedToolsDisplayMode.Always) {
				...
				if (thinkingPart instanceof ChatThinkingContentPart) {
					appendToThinking(thinkingPart);
				}
				return thinkingPart;
			}

			if (lastThinking) {
				appendToThinking(lastThinking);
				return this.renderNoContent(other => other.kind === 'textEditGroup' && isEqual(other.uri, chatTextEdit.uri));
			}
		}
	...
	private renderExternalEdit(content: IChatExternalEdit, context: IChatContentPartRenderContext, templateData: IChatListItemTemplate): IChatContentPart {
		// An edit made by a tool is already reported by that tool's card, which carries the
		// file name, the +/- stats and the diff itself. The pill directly beneath it is the
		// same edit stated twice. Suppressing the DOM does not touch the progress part in the
		// model, so the undo stop this edit carries survives.
		const precedingPart = context.content[context.contentIndex - 1];
		if (precedingPart?.kind === 'toolInvocation' || precedingPart?.kind === 'toolInvocationSerialized') {
			return this.renderNoContent(other => other.kind === content.kind);
		}
	...
	private renderMarkdown(markdown: IChatMarkdownContent, templateData: IChatListItemTemplate, context: IChatContentPartRenderContext): IChatContentPart {
		...
		const hasNarrationBoundary = !this.hasEditCodeblockUri(markdown) && !isBlankMarkdown && !hasPendingEditCodeblock;
		const displayMode = this.configService.getValue<ThinkingDisplayMode>('chat.agent.thinkingStyle');
		if (shouldFinalizeThinkingPhase(hasNarrationBoundary, false, displayMode)) {
			templateData.pendingThinkingPhaseBoundary = undefined;
			// Preview-style narration is a phase boundary immediately.
			this.finalizeCurrentThinkingPart(context, templateData);
		} else if (hasNarrationBoundary && displayMode === ThinkingDisplayMode.FixedScrolling) {
			const activeThinking = this.getLastThinkingPart(templateData.renderedParts);
			const activeThinkingIndex = activeThinking ? templateData.renderedParts?.indexOf(activeThinking) ?? -1 : -1;
			if (activeThinking && activeThinkingIndex >= 0 && activeThinkingIndex < context.contentIndex) {
				const existingBoundary = templateData.pendingThinkingPhaseBoundary;
				templateData.pendingThinkingPhaseBoundary = {
					narrationContentIndex: existingBoundary?.thinkingPart === activeThinking
						? Math.max(existingBoundary.narrationContentIndex, context.contentIndex)
						: context.contentIndex,
					thinkingPart: activeThinking,
				};
			}
		}
