...
export class ChatDragAndDrop extends Themable {
	...
	private guessDropType(e: DragEvent): ChatDragAndDropType | undefined {
		...
		if (containsDragType(e, CodeDataTransfers.NOTEBOOK_CELL_OUTPUT)) {
			...
			// Upstream only accepts an image when some extension declares it can receive
			// binary references. V3Code carries images itself - the chat converter turns
			// them into base64 image parts - so gating on that proposal made a working
			// feature refuse the drop, and the image fell through to the editor instead.
