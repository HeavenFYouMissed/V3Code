...
// V3Code: the chat compacts (Claude-style) when history approaches this many tokens, NOT the model's
// full advertised window. The context-usage bar must fill relative to THIS point so it visibly "grows
// to compaction" — at the model window denominator a 1M-window model barely moved the ring. Mirrors
// DEFAULT_HISTORY_BUDGET.effectiveCeilingTokens + headroomFraction in
// src/vs/workbench/contrib/void/common/memory/contextBudget.ts (kept as a local const to avoid a
// chat -> void layer dependency). Keep these two in sync.
const V3CODE_COMPACTION_CEILING_TOKENS = 250_000;
const V3CODE_HEADROOM_FRACTION = 0.30;
const V3CODE_MIN_HISTORY_TOKENS = 6_000;

/**
 * The token count at which the session compacts: a headroom-reserved fraction of the model's input
 * window, bounded above by the compaction ceiling and below by the floor. The usage bar fills toward
 * this, so 100% == "compaction is about to happen", not "the 1M window is full".
 */
function v3codeCompactionPoint(maxInputTokens: number, totalContextWindow: number): number {
	const inputBudget = Math.max(V3CODE_MIN_HISTORY_TOKENS, (maxInputTokens || totalContextWindow));
	const cap = Math.min(Math.round(inputBudget * (1 - V3CODE_HEADROOM_FRACTION)), V3CODE_COMPACTION_CEILING_TOKENS);
	return Math.max(V3CODE_MIN_HISTORY_TOKENS, cap);
}
...
export class ChatContextUsageWidget extends Disposable {
	...
	private updateFromResponse(response: IChatResponseModel, modelId: string): void {
		...
		const modelContextWindow = (maxInputTokens ?? 0) + (maxOutputTokens ?? 0);
		...
		// V3Code: fill the bar toward the COMPACTION point, not the model's full window. The session
		// compacts near v3codeCompactionPoint tokens (Claude-style), so that is what "100%" means here.
		const totalContextWindow = v3codeCompactionPoint(maxInputTokens ?? 0, modelContextWindow);
