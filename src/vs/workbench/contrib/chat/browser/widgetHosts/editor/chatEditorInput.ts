...
export class ChatEditorInput extends EditorInput implements IEditorCloseHandler {
	...
	private resolveIcon(): ThemeIcon | URI | undefined {
		...
		const sessionType = this.getSessionType();
		// V3Code branding follows the owner in native editor tabs too.
		const brand = getExternalAgentSessionIcon(sessionType);
		if (brand) { return brand; }
		if (sessionType === localChatSessionType) { return v3AgentMark; }
		if (sessionType === 'agent-host-claude' || sessionType === 'claude-code') { return Codicon.claude; }
		if (sessionType !== localChatSessionType) {
...
export class ChatEditorInputSerializer implements IEditorSerializer {
	canSerialize(input: EditorInput): input is ChatEditorInput {
		// An Agent Workspace chat belongs to that window's chat slot, not to the
		// workspace. Suppressing the write in EditorPart.saveState covers the window
		// swap; refusing it here also covers the exits that never reach it, such as
		// closing the Agent window outright or losing it to a crash.
		return input instanceof ChatEditorInput && !!input.sessionResource && input.options.agentWorkspace !== true;
	}
	...
	serialize(input: EditorInput): string | undefined {
		...
		// Agent Workspace is a property of the current top-level window. Never
		// persist it with the session: the native Agent-window IPC explicitly
		// supplies it when opening the session, while an IDE restore must remain a
		// normal ChatEditor.
		const { agentWorkspace: _agentWorkspace, ...persistedOptions } = input.options;
	...
	deserialize(instantiationService: IInstantiationService, serializedEditor: string): EditorInput | undefined {
		try {
			...
			// Strip stale state written by Agent Workspace builds that serialized
			// this transient window flag with the editor input.
			const { agentWorkspace: _agentWorkspace, ...options } = parsed.options ?? {};
			...
			if (parsed.sessionResource) {
				...
				return instantiationService.createInstance(ChatEditorInput, sessionResource, options);
			}
			...
			return instantiationService.createInstance(ChatEditorInput, resource, options);
		} catch (err) {
			return undefined;
		}
	}
