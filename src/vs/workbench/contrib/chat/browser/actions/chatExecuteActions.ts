...
export interface IChatExecuteActionContext {
	...
	/** Set by V3Code press-to-talk mic; skips focus moves that cancel the hold. */
	v3VoicePointerId?: number;
}
...
/** V3Code: opens the command-backed composer + sheet. */
export class OpenComposerPlusAction extends Action2 {
	static readonly ID = 'workbench.action.chat.v3code.openComposerPlus';

	constructor() {
		super({
			id: OpenComposerPlusAction.ID,
			title: localize2('v3code.chat.openComposerPlus', "Add to prompt"),
			tooltip: localize('v3code.chat.openComposerPlus.tooltip', "Add files, context, or create an agent resource"),
			icon: Codicon.add,
			category: CHAT_CATEGORY,
			f1: false,
			precondition: ChatContextKeys.enabled,
	...
	override run(accessor: ServicesAccessor): void {
		accessor.get(IChatWidgetService).lastFocusedWidget?.input.openComposerPlus();
	}
}
...
export class OpenPermissionPickerAction extends Action2 {
	...
	constructor() {
		super({
			...
			precondition: ChatContextKeys.enabled,
			menu: {
				// V3Code: the approval pill follows the composer + sheet on the primary row.
				id: MenuId.ChatInput,
				order: -1,
				group: 'navigation',
				// V3Code local chat always needs Bypass / Autopilot / Default Approvals.
				when: ContextKeyExpr.and(
					ChatContextKeys.enabled,
					ChatContextKeys.location.isEqualTo(ChatAgentLocation.Chat),
			...
			menu: [
				{
					id: MenuId.ChatInput,
					order: 1,
					// V3Code uses v3code.agent for every mode — always expose the picker in
					// sidebar chat. (Copilot cloud/coding sessions hide this when locked and
					// route mode selection elsewhere; that UX breaks local BYOK chat.)
					when: ContextKeyExpr.and(
						ChatContextKeys.enabled,
						ChatContextKeys.location.isEqualTo(ChatAgentLocation.Chat),
						ChatContextKeys.inQuickChat.negate()),
...
export class OpenSessionTargetPickerAction extends Action2 {
	...
	constructor() {
		super({
			...
			menu: [
				// V3Code is local-only (BYOK). The Local / Copilot CLI / Cloud session-
				// target picker is hidden entirely — the session target is permanently
				// "Local" and must never change. (Hiding both menu entries also empties the
				// secondary toolbar, so the footer reclaims that row.)
				{
					id: MenuId.ChatInput,
					order: 0,
...
/**
 * V3Code's command-backed native file/image picker. The permanent paperclip was
 * removed from the footer; the composer + sheet invokes this same command.
 */
export const V3_ATTACH_TO_COMPOSER_COMMAND_ID = 'workbench.action.chat.v3code.attachToComposer';

class AttachToComposerAction extends Action2 {
	static readonly ID = V3_ATTACH_TO_COMPOSER_COMMAND_ID;
	constructor() {
		super({
			id: AttachToComposerAction.ID,
			title: localize2('v3code.chat.attachToComposer', "Attach files & images"),
			icon: Codicon.attach,
			f1: false,
			category: CHAT_CATEGORY,
			precondition: ChatContextKeys.enabled,
			menu: [{
				...
				group: 'navigation',
				order: 3,
				// The native file picker now lives in the composer + sheet. Keep the
				// command registered without duplicating a paperclip on the footer.
				when: ContextKeyExpr.false(),
			}],
		});
	}
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		// Open the real OS file picker (multi-select). The user explicitly wants the
		// machine's file system here, not V3Code's internal Add-Context quick pick.
		const uris = await fileDialogService.showOpenDialog({
			canSelectFiles: true,
			canSelectFolders: false,
			canSelectMany: true,
			title: localize('v3code.chat.attachToComposer.title', "Attach files & images"),
			openLabel: localize('v3code.chat.attachToComposer.open', "Attach"),
		});
		if (!uris || uris.length === 0) {
			return;
		}
		for (const uri of uris) {
			widget.input.attachmentModel.addFile(uri);
		}
	}
}

/** V3Code: turn a rough idea into an implementation-ready brief with the selected model. */
class V3PromptPolishAction extends Action2 {
	static readonly ID = 'workbench.action.chat.v3code.polishPrompt';

	constructor() {
		super({
			id: V3PromptPolishAction.ID,
			title: localize2('v3code.chat.polishPrompt', "Expand Prompt"),
			tooltip: localize('v3code.chat.polishPrompt.tooltip', "Turn this idea into an implementation-ready brief"),
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		await accessor.get(IChatWidgetService).lastFocusedWidget?.input.polishPrompt();
	}
}

export const V3_VOICE_INPUT_START_COMMAND_ID = 'workbench.action.chat.v3code.voiceInput.start';
export const V3_VOICE_INPUT_STOP_COMMAND_ID = 'workbench.action.chat.v3code.voiceInput.stop';

/**
 * V3Code: opens V, the independent live voice controller. The custom view item
 * renders this as a pill between Agent mode and the model picker.
 */
export class V3VoiceInputAction extends Action2 {
	static readonly ID = 'workbench.action.chat.v3code.voiceInput';
	constructor() {
		super({
			id: V3VoiceInputAction.ID,
			title: localize2('v3code.chat.voiceInput', "V Voice"),
			tooltip: localize('v3code.chat.voiceInput.tooltip', "Talk with V while the main agent keeps working"),
			icon: Codicon.mic,
			f1: false,
			category: CHAT_CATEGORY,
			precondition: ChatContextKeys.enabled,
	...
	override async run(_accessor: ServicesAccessor, _context?: IChatExecuteActionContext): Promise<void> {
		// Press-to-talk is handled by the custom action view item.
	}
}

export function registerChatExecuteActions(): DisposableStore {
	const store = new DisposableStore();
	store.add(registerAction2(OpenComposerPlusAction));
	store.add(registerAction2(V3PromptPolishAction));
	store.add(registerAction2(V3VoiceInputAction));
	store.add(registerAction2(AttachToComposerAction));
