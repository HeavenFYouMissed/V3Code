...
export function registerNewChatActions() {
	...
	// V3Code: the "New Chat" control in the chat view title is a plain "+" command that
	// directly starts a new chat. (Previously a split-button with a dropdown caret to the
	// ChatNewMenu submenu — the caret was redundant with "+" and its popup pushed the panel
	// content under the top bar, so it was removed.)
...
async function runNewChatAction(
	...
	sessionType: AgentSessionProviders = AgentSessionProviders.Local
) {
	...
	// General New Chat returns to V3Code. External-agent chips explicitly select their provider.
