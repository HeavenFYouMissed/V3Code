...
import { matchV3VoiceQuestionOption, registerV3VoiceQuestionAnswerHandler } from '../../../void/browser/v3codeVoiceAgentEvents.js';
...
const toolIdsThatCannotBeAutoApproved = new Set([
	...
	// V3Code: multiple-choice questions must always block for a button click — never YOLO through.
	'v3code_ask_user',
]);
...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
		...
		try {
			if (dto.context) {
				...
				if (preparedInvocation?.confirmationMessages?.title) {
					...
					const voiceAnswerBinding = tool.data.id === 'v3code_ask_user' && dto.context?.sessionResource
						? registerV3VoiceQuestionAnswerHandler(dto.context.sessionResource.toString(), rawChoice => {
							const options = preparedInvocation?.confirmationMessages?.customOptions ?? [];
							const normalized = rawChoice.trim().toLocaleLowerCase();
							const exact = options.find(option => option.id.toLocaleLowerCase() === normalized);
							const matchedIndex = exact ? options.indexOf(exact) : matchV3VoiceQuestionOption(rawChoice, options.map(option => option.label));
							const fuzzy = matchedIndex === undefined ? undefined : options[matchedIndex];
							if (!fuzzy) {
								return { accepted: false, message: `That did not match an available option: ${options.map(option => option.label).join(', ')}` };
							}
							const accepted = IChatToolInvocation.confirmWith(toolInvocation, {
								type: ToolConfirmKind.UserAction,
								selectedButton: fuzzy.id,
								selectedButtonKind: fuzzy.kind,
							});
							return accepted
								? { accepted: true, message: `The main agent received: ${fuzzy.label}` }
								: { accepted: false, message: 'The main agent is no longer waiting for that decision.' };
						})
						: undefined;
					let userConfirmed: ConfirmedReason;
					try {
						userConfirmed = await IChatToolInvocation.awaitConfirmation(toolInvocation, token);
					} finally {
						voiceAnswerBinding?.dispose();
					}
	...
	private async resolveAutoConfirmFromHook(
		...
		// Tools with custom option buttons are the confirmation UI — never auto-confirm past them.
		if (preparedInvocation?.confirmationMessages?.customOptions?.length) {
			return { autoConfirmed: undefined, preparedInvocation };
		}
