...
import { V3CODE_FAST_MODEL_ID, selectV3CodeUtilityModel } from '../v3codeUtilityModels.js';
...
export class ChatToolRiskAssessmentService implements IChatToolRiskAssessmentService {
	...
	private async _invokeModel(tool: IToolData, parameters: unknown, token: CancellationToken): Promise<IToolRiskAssessment | undefined> {
		const modelId = this._configurationService.getValue<string>(ChatConfiguration.ToolRiskAssessmentModel) || V3CODE_FAST_MODEL_ID;

		const models = await selectV3CodeUtilityModel(this._languageModelsService, modelId);
