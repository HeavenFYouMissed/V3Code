...
import { mcpAccessConfig, McpAccessValue, mcpGalleryServiceUrlConfig } from '../../../../../platform/mcp/common/mcpManagement.js';
...
export class McpListWidget extends Disposable {
	...
	private emptyActionLink!: HTMLAnchorElement;
	private readonly emptyActionListener = this._register(new MutableDisposable());
	...
	private create(): void {
		...
		// V3Code: optional one-click action inside the empty state (registry enablement).
		this.emptyActionLink = DOM.append(this.emptyContainer, $('a.empty-subtext')) as HTMLAnchorElement;
		this.emptyActionLink.href = '#';
		this.emptyActionLink.setAttribute('role', 'button');
		this.emptyActionLink.style.display = 'none';
	...
	/**
	 * The gallery manifest rebuilds asynchronously after the setting is written, so an
	 * immediate single query races it and dead-ends on "No MCP servers available" with
	 * the enable affordance gone. Write, then retry briefly until servers appear; a
	 * refused settings write (policy lock) is surfaced instead of swallowed.
	 */
	private async _enableRegistryAndQuery(): Promise<void> {
		try {
			await this.configurationService.updateValue(mcpGalleryServiceUrlConfig, 'https://registry.modelcontextprotocol.io');
		} catch (err) {
			this.emptySubtext.textContent = localize('registryEnableFailed', "Could not save the registry setting: {0}", String(err instanceof Error ? err.message : err));
			return;
		}
		this.emptyText.textContent = localize('registryConnecting', "Connecting to the registry...");
		for (let attempt = 0; attempt < 6; attempt++) {
			await this.queryGallery();
			if (this.galleryServers.length > 0 || !this.browseMode) {
				return;
			}
	...
	private async queryGallery(): Promise<void> {
		...
		this.emptyActionLink.style.display = 'none';
		this.emptyActionListener.clear();
	...
	private _hasRegistryConfigured(): boolean {
	...
	private updateGalleryList(): void {
		...
		if (this.galleryServers.length === 0) {
			...
			} else if (!this._hasRegistryConfigured()) {
				// V3Code: an empty marketplace because no registry is configured is worse
				// than hidden. Offer the one-click enable — no network until the user asks.
				this.emptyText.textContent = localize('registryOff', "Registry browsing is off");
				this.emptySubtext.textContent = localize('registryOffDetail', "Browse the public MCP registry's servers here. Nothing is contacted until you turn it on; you can change or clear the registry URL in Settings at any time.");
				this.emptyActionLink.textContent = localize('enableRegistry', "Enable registry browsing");
				this.emptyActionLink.style.display = '';
				this.emptyActionListener.value = DOM.addDisposableListener(this.emptyActionLink, 'click', (e) => {
					e.preventDefault();
					void this._enableRegistryAndQuery();
				});
			} else {
