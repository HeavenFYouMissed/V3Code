...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerSetupAgents(context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): void {
		...
		// V3Code is the default chat agent and handles every mode/location itself.
		// The Copilot "setup" agents must never register, otherwise they win the
		// default-agent slot for Agent/Ask/Edit modes and show a bogus
		// "sign in to GitHub Copilot" message (Plan mode has no setup agent, which
		// is why it works while Agent mode does not).
		const v3codeIsDefault = product.defaultChatAgent?.extensionId === 'v3code.v3code';

		const updateRegistration = () => {

			// Agent + Tools
			{
				if (!context.state.hidden && !context.state.disabledInWorkspace && !v3codeIsDefault) {
