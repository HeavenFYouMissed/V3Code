...
import { AgentSessionsViewContainerId, AgentSessionsViewId, AgentSessionsViewPane } from './agentSessions/agentSessionsViewPane.js';
...
const agentSessionsViewIcon = registerIcon('agent-sessions-view-icon', Codicon.agent, localize('agentSessionsViewIcon', 'View icon of the Agents sessions view.'));
...
// Agents live in Parts.UNIFIED_SIDEBAR_PART (Cursor-faithful). Keep the view
// container id registered for legacy commands, but never surface it in Explorer.
const agentSessionsViewContainer: ViewContainer = Registry.as<IViewContainersRegistry>(ViewExtensions.ViewContainersRegistry).registerViewContainer({
	...
	title: localize2('chat.sessionsContainer.label', "Agents"),
...
const agentSessionsViewDescriptor: IViewDescriptor = {
	...
	name: localize2('chat.sessionsView.label', "Agents"),
	canToggleVisibility: false,
	canMoveView: false,
	order: 0,
	weight: 30,
...
viewsRegistry.registerViews([agentSessionsViewDescriptor], agentSessionsViewContainer);
viewsRegistry.registerViews([chatViewDescriptor], chatViewContainer);
