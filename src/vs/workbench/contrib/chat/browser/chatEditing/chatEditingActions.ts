...
registerAction2(class RestoreCheckpointAction extends Action2 {
	constructor() {
		super({
			...
			menu: [
				...
				/* Compact: sit next to copy / read-aloud on the agent response footer */
				{
	...
	async run(accessor: ServicesAccessor, ...args: unknown[]) {
		...
		const checkpointId = isRequestVM(item) ? item.id : isResponseVM(item) ? item.requestId : item.id;
		widget?.viewModel?.model.setCheckpoint(checkpointId);
		const confirmed = await restoreSnapshotWithConfirmation(accessor, item);
