...
import { allDiscoverySources, DiscoverySource, discoverySourceSettingsLabel, McpCollisionBehavior, mcpDiscoverySection, mcpServerCollisionBehaviorSection, mcpServerSamplingSection } from '../../mcp/common/mcpConfiguration.js';
...
import { AgentWorkspaceFilesEditor, AgentWorkspaceFilesEditorInput } from './widgetHosts/editor/agentWorkspaceFilesEditor.js';
import { AgentWorkspaceTerminalEditor, AgentWorkspaceTerminalEditorInput } from './widgetHosts/editor/agentWorkspaceTerminalEditor.js';
...
import { V3CODE_FAST_MODEL_ID } from './v3codeUtilityModels.js';
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[AgentHostEnabledSettingId]: {
			...
			// V3Code: enabled on desktop regardless of the (stable) quality channel so the
			// Agents Window + worktree-isolated agent sessions are available (Cmd/Ctrl+Shift+A).
			// The agent host process spawns lazily on first session, so this is cheap when idle.
			default: !isWeb,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostClaudeAgentSdkPathSettingId]: {
			type: 'string',
			description: nls.localize('chat.agentHost.claudeAgent.path', "Override the path to the `@anthropic-ai/claude-agent-sdk` package, for local SDK testing. V3Code bundles the SDK, so the Claude agent works without this setting; when set, the SDK is loaded from this path instead of the bundled copy. Requires `#chat.agentHost.enabled#`. The agent host process must be restarted for changes to take effect."),
			default: '',
			tags: ['experimental', 'advanced'],
			// V3Code: the bundled @anthropic-ai/claude-agent-sdk is the default;
			// this setting stays visible in stable as the local-testing override.
			included: true,
		},
		...
		[mcpDiscoverySection]: {
			...
			default: Object.fromEntries(allDiscoverySources.map(k => [k, k === DiscoverySource.V3Code])),
		...
		[ChatConfiguration.SubagentsAllowInvocationsFromSubagents]: {
			...
			markdownDescription: nls.localize('chat.subagents.allowInvocationsFromSubagents.md', "Controls whether subagents can invoke other subagents. When enabled, nesting is limited to a maximum depth of 2."),
			default: true,
			experiment: {
				mode: 'auto'
			}
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(
		AgentWorkspaceFilesEditor,
		AgentWorkspaceFilesEditor.ID,
		nls.localize('agentWorkspace.filesEditor', "Files")
	),
	[
		new SyncDescriptor(AgentWorkspaceFilesEditorInput)
	]
);
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(
		AgentWorkspaceTerminalEditor,
		AgentWorkspaceTerminalEditor.ID,
		nls.localize('agentWorkspace.terminalEditor', "Terminal")
	),
	[
		new SyncDescriptor(AgentWorkspaceTerminalEditorInput)
	]
);
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(
