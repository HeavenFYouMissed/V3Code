...
function sameWelcomeIcon(first: ThemeIcon | URI | undefined, second: ThemeIcon | URI | undefined): boolean {
	if (URI.isUri(first) && URI.isUri(second)) {
		return first.toString() === second.toString();
	}
	if (ThemeIcon.isThemeIcon(first) && ThemeIcon.isThemeIcon(second)) {
		return ThemeIcon.isEqual(first, second);
	}
	return first === second;
}
...
export class ChatViewWelcomePart extends Disposable {
	...
	constructor(
		...
		try {
			...
			if (content.useLargeIcon) {
				...
				this.element.classList.add('v3-branded-welcome');
			}
			let isV3AgentWelcome = false;
			if (content.icon) {
				if (ThemeIcon.isThemeIcon(content.icon)) {
					...
					if (content.useLargeIcon) {
						/* Use a real image element for full-color art. A CSS background can fail
						 * silently when an asset is missing or its resource URI is rejected. */
						const image = dom.append(icon, $<HTMLImageElement>('img.chat-welcome-view-icon-image'));
						image.alt = '';
						image.decoding = 'async';
						image.addEventListener('error', () => {
							this.logService.error(`[ChatWelcome] Failed to load welcome icon: ${content.icon?.toString()}`);
							image.remove();
							icon.classList.remove('custom-icon', 'v3-welcome-img-icon');
							icon.appendChild(renderIcon(Codicon.chatSparkle));
						}, { once: true });
						image.src = content.icon.toString(true);
						icon.classList.add('custom-icon', 'v3-welcome-img-icon');
						isV3AgentWelcome = true;
					} else {
			...
			if (isV3AgentWelcome) {
				this.element.classList.add('v3-agent-welcome');
			}
	...
	public needsRerender(content: IChatViewWelcomeContent): boolean {
		...
		return !!(
			...
			this.content.useLargeIcon !== content.useLargeIcon ||
			!sameWelcomeIcon(this.content.icon, content.icon));
	}
