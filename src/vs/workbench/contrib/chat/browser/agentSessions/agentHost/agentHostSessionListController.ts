...
import { getAgentHostIcon, getExternalAgentSessionIcon } from '../agentSessions.js';
...
export class AgentHostSessionListController extends Disposable implements IChatSessionItemController {
	...
	async refresh(token: CancellationToken): Promise<void> {
		...
		// The agent-host connection is a machine-wide session catalogue. Keep
		// every session for this provider and carry its working directory as
		// metadata; the Agents rail owns repository/workspace grouping. Filtering
		// here made a workspace switch look as if other agents had disappeared.
	...
	private _makeItem(rawId: string, opts: {
		...
		return {
			...
			description,
			iconPath: getExternalAgentSessionIcon(this._provider) ?? getAgentHostIcon(this._productService),
