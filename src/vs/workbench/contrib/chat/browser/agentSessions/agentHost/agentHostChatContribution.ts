...
import { getExternalAgentSessionIcon } from '../agentSessions.js';
	...
	private _registerAgent(agent: AgentInfo): void {
		...
		store.add(this._chatSessionsService.registerChatSessionContribution({
			...
			...(agent.provider.startsWith('acp-') ? {
				icon: (getExternalAgentSessionIcon(agent.provider) ?? Codicon.plug).id,
				welcomeTitle: agent.displayName,
				welcomeMessage: localize('externalAgent.welcome', "Start a new chat with {0}.", agent.displayName),
			} : {}),
			name: agentId,
			displayName,
		...
		// V3Code: the hub bearer comes from IV3CodeAccountService, not the VS Code
		// authentication service — re-push it on account state changes (sign-in and
		// the ~10-min refreshFromHub reconcile; NOT the 45-min token-renew instant,
		// but a renewed bearer reaches the host within <=10 min, safely inside the
		// 60-min token life). Without this, users who sign in after the agent host
		// starts keep an empty model picker, and long sessions materialize with a
		// stale bearer (Anthropic-side 401 -> error-subtype result). Sign-out is
		// deliberately not swept: the stale bearer just expires (parity with today).
		store.add(this._v3codeAccountService.onDidChangeState(() => {
			const agents = this._getRootAgents();
			this._authenticateWithServer(agents).catch(() => { /* best-effort */ });
		}));
	}
	...
	private async _authenticateWithServer(agents: readonly AgentInfo[]): Promise<void> {
		...
		try {
			await authenticateProtectedResources(agents, {
				...
				hubToken: this._v3codeAccountService.getAccessToken(),
			});
		} catch (err) {
