...
import { V3CODE_HUB_PROTECTED_RESOURCE } from '../../../../../../platform/agentHost/common/agentService.js';
...
export interface IAgentHostAuthenticationOptions {
	...
	/**
	 * V3Code hub bearer token (the signed-in account token). Used for the
	 * V3Code hub protected resource, which has no VS Code OAuth provider — the
	 * token comes straight from the account service instead of resolveTokenForResource.
	 */
...
export async function authenticateProtectedResources(
	...
	for (const agent of agents) {
		for (const resource of agent.protectedResources ?? []) {
			// V3Code hub resource has no VS Code OAuth provider — its bearer is the
			// signed-in account token supplied directly via options.hubToken.
			const isHubResource = resource.resource === V3CODE_HUB_PROTECTED_RESOURCE.resource;
			const resourceUri = URI.parse(resource.resource);
			const token = isHubResource
				? options.hubToken
				: await resolveTokenForResource(
					resourceUri,
...
export async function resolveAuthenticationInteractively(
	...
	for (const resource of protectedResources) {
		// V3Code hub resource has no VS Code OAuth provider — authenticate directly with the
		// signed-in account token (same special-case as authenticateProtectedResources). Without
		// this, a brand-new Agents session hits this interactive path and always fails auth for a
		// signed-in V3Code user (no provider for app.v3code.dev/login).
		if (resource.resource === V3CODE_HUB_PROTECTED_RESOURCE.resource) {
			if (options.hubToken) {
				await options.authenticate({ resource: resource.resource, token: options.hubToken });
				options.authTokenCache?.updateAndIsChanged(resource.resource, options.hubToken);
				options.logService.info(`${options.logPrefix} V3Code hub authenticated (interactive path)`);
				return true;
			}
			continue;
		}
