...
export function usageInfoToChatUsage(usage: UsageInfo | undefined): IChatUsage | undefined {
	if (typeof usage?.inputTokens !== 'number' && typeof usage?.outputTokens !== 'number' && typeof usage?.cacheReadTokens !== 'number') {
		return undefined;
	}
	return {
		kind: 'usage',
		// inputTokens is the NON-cached portion only (Anthropic semantics) — fold the cache-read
		// subset back in so promptTokens reflects the true prompt size. Dropping cacheReadTokens
		// made the context ring read ~0% on cache-hot sessions, where nearly the whole prompt is
		// served from cache.
