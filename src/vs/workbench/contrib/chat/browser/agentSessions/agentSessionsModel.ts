...
export class AgentSessionsModel extends Disposable implements IAgentSessionsModel {
	...
	private resolveProvider(provider: string, options: { refreshProvider: boolean }): Promise<void> {
		// NOTE: Do NOT gate on `sentiment.hidden`. V3Code calls setForceHidden(true) to
		// suppress Copilot sign-in chrome, which sets sentiment.hidden — that emptied the
		// Agents rail forever even while native chat sessions were live. Truly disabled AI
		// still yields empty provider results; force-hidden is branding, not "no agents".
