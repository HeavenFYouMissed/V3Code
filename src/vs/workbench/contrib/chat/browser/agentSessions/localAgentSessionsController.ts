...
export class LocalAgentsSessionsController extends Disposable implements IChatSessionItemController, IWorkbenchContribution {
	...
	private registerListeners(): void {
		...
		this._register(this.chatService.onDidDisposeSession(e => {
			...
			if (removedSessionResources.length) {
				// willDisposeModel persists the session before this event fires. Rebuild
				// from live + history so completed chats stay in the Agents rail after
				// the pane releases its model ref (New Chat / switch). Deleting from
				// `_items` alone left a gap until window focus or Cmd+R refreshed.
	...
	private async tryUpdateLiveSessionItem(model: IChatModel): Promise<void> {
		...
		if (!existing) {
			// Empty sessions used to be omitted from `_items`. When the first request
			// lands, tryUpdate previously no-oped and the new chat never appeared
			// until a full refresh (focus / Cmd+R). Rebuild once it becomes listable.
		...
		const updated = new LocalChatSessionItem(await chatModelToChatDetail(model), model, this.getDefaultWorkingDirectory());
	...
	private toChatSessionItem(chat: IChatDetail): LocalChatSessionItem | undefined {
		...
		// Include empty live sessions so New Chat / New Agent shows in the Agents
		// rail immediately (previously filtered until the first user message).
		return new LocalChatSessionItem(chat, model, this.getDefaultWorkingDirectory());
	}

	private getDefaultWorkingDirectory(): URI | undefined {
		// Local chat history is stored per workspace. Older entries predate the
		// working-directory field, so attributing them to that store's current
		// workspace is the faithful fallback and lets the Agents rail group them.
...
class LocalChatSessionItem implements IChatSessionItem {
	...
	constructor(chatDetail: IChatDetail, model: IChatModel | undefined, defaultWorkingDirectory: URI | undefined) {
		...
		const workingDirectory = chatDetail.workingDirectory ?? model?.workingDirectory ?? defaultWorkingDirectory;
		const workingDirectoryPath = workingDirectory?.scheme === Schemas.file ? workingDirectory.fsPath : undefined;
