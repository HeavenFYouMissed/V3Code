...
interface IAgentSessionItemTemplate {
	...
	readonly workingDots: HTMLElement;
	readonly liveStatus: HTMLElement;
	readonly liveStatusLabel: HTMLElement;
...
export interface IAgentSessionRendererOptions {
	...
	readonly preferProviderIcons?: boolean;
...
export class AgentSessionRenderer extends Disposable implements ICompressibleTreeRenderer<IAgentSession, FuzzyScore, IAgentSessionItemTemplate> {
	...
	/**
	 * Live working indicator — prefer ChatModel.hasActiveRequest so tool-call /
	 * confirmation gaps still count as working. Falls back to session.status for
	 * non-local providers without a live model.
	 */
	private applyWorkingIndicator(template: IAgentSessionItemTemplate, session: IAgentSession): void {
		const setWorking = (isWorking: boolean) => {
			const preserveProviderIcon = this.options.preferProviderIcons === true || session.providerType.startsWith('agent-host-acp-');
			template.element.classList.toggle('working', isWorking && !preserveProviderIcon);
			template.workingDots.classList.toggle('visible', isWorking && !preserveProviderIcon);
			template.icon.style.display = isWorking && !preserveProviderIcon ? 'none' : '';
			this.renderLiveStatus(template, session, isWorking);
		};

		const chatModel = this.chatService.getSession(session.resource);
		if (chatModel) {
			template.elementDisposable.add(autorun(reader => {
				// hasActiveRequest stays true across tool/elicitation gaps; requestInProgress does not.
				const isWorking = chatModel.hasActiveRequest.read(reader)
					|| chatModel.requestInProgress.read(reader)
					|| !!chatModel.requestNeedsInput.read(reader);
				setWorking(isWorking);
			}));
			return;
		}

		setWorking(isSessionInProgressStatus(session.status));
	}

	private renderLiveStatus(template: IAgentSessionItemTemplate, session: IAgentSession, isWorking: boolean): void {
		template.liveStatus.className = 'agent-session-live-status';
		if (!this.options.showActivityIndicator) {
			template.liveStatus.classList.add('hidden');
			template.liveStatusLabel.textContent = '';
			return;
		}

		if (isWorking) {
			template.liveStatus.classList.add('visible', 'running');
			template.liveStatusLabel.textContent = localize('agentSessionLive', "Live");
			template.liveStatus.title = localize('agentSessionLiveDescription', "Agent is actively working");
		} else if (session.status === AgentSessionStatus.NeedsInput) {
			template.liveStatus.classList.add('visible', 'needs-input');
			template.liveStatusLabel.textContent = localize('agentSessionInput', "Input");
			template.liveStatus.title = localize('agentSessionInputDescription', "Agent needs input");
		} else if (session.status === AgentSessionStatus.Failed) {
			template.liveStatus.classList.add('visible', 'failed');
			template.liveStatusLabel.textContent = localize('agentSessionFailedShort', "Failed");
			template.liveStatus.title = localize('agentSessionFailedDescription', "Agent run failed");
		} else {
			template.liveStatus.classList.add('hidden');
			template.liveStatusLabel.textContent = '';
			template.liveStatus.removeAttribute('title');
		}
	}
	...
	renderTemplate(container: HTMLElement): IAgentSessionItemTemplate {
		...
		const elements = h(
			...
			[
				h('div.agent-session-icon-col', [
					...
					h('div.agent-session-working-dots@workingDots', [
				...
				h('div.agent-session-main-col', [
					h('div.agent-session-title-row', [
						...
						h('div.agent-session-live-status@liveStatus', [
							h('span.agent-session-live-dot'),
							h('span.agent-session-live-label@liveStatusLabel'),
						]),
		...
		return {
			...
			icon: elements.icon,
			workingDots: elements.workingDots,
			liveStatus: elements.liveStatus,
			liveStatusLabel: elements.liveStatusLabel,
	...
	renderElement(session: ITreeNode<IAgentSession, FuzzyScore>, index: number, template: IAgentSessionItemTemplate, details?: ITreeElementRenderDetails): void {
		...
		// Archived / working (multi-row safe — each session carries its own live status)
		template.element.classList.toggle('archived', session.element.isArchived());
		this.applyWorkingIndicator(template, session.element);
		...
		if (this.options.preferProviderIcons || session.element.providerType.startsWith('agent-host-acp-')) {
			const providerIcon = session.element.icon;
			template.icon.className = `agent-session-icon ${ThemeIcon.asClassName(providerIcon)}`;
			template.icon.style.color = providerIcon.color ? asCssVariable(providerIcon.color.id) : '';
			template.icon.title = session.element.providerLabel;
			template.detailsIcon.className = 'agent-session-details-icon';
		} else if (this.options.useStatusOnlyIcons) {
	...
	private getIcon(session: IAgentSession, statusOnly?: boolean): ThemeIcon {
		if (session.status === AgentSessionStatus.InProgress) {
			// Venom green — pairs with the animated 6-dot working indicator.
			return { ...Codicon.sessionInProgress, color: themeColorFromId('charts.green') };
		}
...
export interface IAgentSessionSectionRendererOptions {
	...
	readonly showSectionIcons?: boolean;
}
...
export class AgentSessionSectionRenderer implements ICompressibleTreeRenderer<IAgentSessionSection, FuzzyScore, IAgentSessionSectionTemplate> {
	...
	renderTemplate(container: HTMLElement): IAgentSessionSectionTemplate {
		...
		const elements = h(
			...
			[
				h('span.agent-session-section-icon@icon'),
	...
	renderElement(element: ITreeNode<IAgentSessionSection, FuzzyScore>, index: number, template: IAgentSessionSectionTemplate, details?: ITreeElementRenderDetails): void {
		const sectionIcon = element.element.section === AgentSessionSection.Pinned
			? Codicon.pinned
			: element.element.section === AgentSessionSection.Repository
				? Codicon.repo
				: element.element.section === AgentSessionSection.Archived
					? Codicon.archive
					: Codicon.history;
		template.icon.className = `agent-session-section-icon ${ThemeIcon.asClassName(sectionIcon)}`;
		template.icon.classList.toggle('visible', this.sectionOptions.showSectionIcons === true);
...
export class AgentSessionsListDelegate implements IListVirtualDelegate<AgentSessionListItem> {
	...
	/** Compact density — keep in sync with `.agent-session-item` padding/fonts in agentsessionsviewer.css */
	static readonly ITEM_HEIGHT = 40;
	static readonly SECTION_HEIGHT = 22;
	...
	constructor(private readonly _approvalModel?: AgentSessionApprovalModel,
		...
		private readonly _sectionHeight?: number,
	) { }

	getHeight(element: AgentSessionListItem): number {
		if (isAgentSessionSection(element)) {
			// Rows are measured in JS and the header is absolutely positioned inside
			// that box, so any CSS margin a host adds around the header has to be
			// paid for here too - otherwise the label renders below the 22px row and
			// gets visibly clipped in half.
			return this._sectionHeight ?? AgentSessionsListDelegate.SECTION_HEIGHT;
		}
		...
		let height = this._itemHeight ?? AgentSessionsListDelegate.ITEM_HEIGHT;
...
export class AgentSessionsDataSource extends Disposable implements IAsyncDataSource<IAgentSessionsModel, AgentSessionListItem> {
	...
	constructor(
		...
		private readonly groupPinnedSessions?: boolean,
	) {
		super();
	...
	private groupSessionsByRepository(sortedSessions: IAgentSession[]): AgentSessionListItem[] {
		...
		if (pinnedSessions.length > 0 && this.groupPinnedSessions) {
			result.push({
