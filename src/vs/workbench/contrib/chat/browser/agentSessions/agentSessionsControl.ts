...
export interface IAgentSessionsControlOptions {
	...
	readonly groupPinnedSessions?: boolean;
	readonly itemHeight?: number;
	/** Virtual row height for a section header. The list measures rows in JS, so a
		host that adds vertical margin around the header in CSS MUST raise this to
		match or the label is clipped by the fixed 22px row box. */
...
export class AgentSessionsControl extends Disposable implements IAgentSessionsControl {
	...
	private createList(container: HTMLElement): void {
		...
		const sessionDataSource = this.sessionsDataSource = this._register(new AgentSessionsDataSource(
			this.options.filter,
			sorter,
			this.options.repositoryGroupLimit,
			this.options.groupPinnedSessions,
		));
		const list = this.sessionsList = this._register(this.instantiationService.createInstance(WorkbenchCompressibleAsyncDataTree,
			'AgentSessionsView',
			container,
			new AgentSessionsListDelegate(approvalModel, this.options.compactShowMore, this.options.itemHeight, this.options.sectionHeight),
			...
			[
				...
				this.instantiationService.createInstance(AgentSessionSectionRenderer, {
					...
					showSectionIcons: this.options.showSectionIcons,
				}),
