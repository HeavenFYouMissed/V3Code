...
suite('ChatRequestParser', () => {
	...
	setup(async () => {
		...
		instantiationService.stub(IProductService, { ...product, _serviceBrand: undefined });
	...
	test('external agent slash commands are not intercepted by the editor', () => {
		const slashCommandService = mockObject<IChatSlashCommandService>()({ _serviceBrand: undefined });
		slashCommandService.getCommands.returns([{ command: 'clear' }]);
		instantiationService.stub(IChatSlashCommandService, slashCommandService);
		parser = instantiationService.createInstance(ChatRequestParser);
		const result = parser.parseChatRequest(testSessionUri, '/clear keep-notes', ChatAgentLocation.Chat, { sessionType: 'agent-host-acp-fixture' });
		assert.strictEqual(getPromptText(result).message, '/clear keep-notes');
		assert.strictEqual(result.parts.length, 1);
		assert.strictEqual(result.parts[0].kind, 'text');
	});
