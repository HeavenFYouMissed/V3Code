...
suite('ChatModeService', () => {
	...
	test('should return builtin modes', async () => {
		...
		assert.strictEqual(modes.builtin.length, 6);
		...
		assert.ok(askMode);
		assert.strictEqual(askMode.label.get(), 'Chat');
		assert.strictEqual(askMode.name.get(), 'ask');
		assert.strictEqual(askMode.kind, ChatModeKind.Ask);

		const multitaskMode = modes.builtin.find(mode => mode.id === ChatMode.Multitask.id);
		assert.ok(multitaskMode);
		assert.strictEqual(multitaskMode.label.get(), 'Multitask');
		assert.strictEqual(multitaskMode.name.get(), 'multitask');
		assert.strictEqual(multitaskMode.kind, ChatModeKind.Agent);
		assert.strictEqual(multitaskMode.isBuiltin, true);

		const debugMode = modes.builtin.find(mode => mode.id === ChatMode.Debug.id);
		assert.ok(debugMode);
		assert.strictEqual(debugMode.label.get(), 'Debug');
		assert.strictEqual(debugMode.name.get(), 'debug');
		assert.strictEqual(debugMode.kind, ChatModeKind.Agent);
		assert.strictEqual(debugMode.isBuiltin, true);
		const debugIcon = debugMode.icon.get();
		assert.ok(debugIcon);
		assert.strictEqual(debugIcon.id, 'bug');
		assert.strictEqual(debugIcon.color?.id, 'problemsErrorIcon.foreground', 'theme-aware red accent');
	});

	test('Debug sits immediately after Plan, above Multitask and the custom-agent divider', async () => {
		const builtin = (await chatModeService.getLocalModes()).builtin;
		const planIndex = builtin.indexOf(ChatMode.Plan);
		const debugIndex = builtin.indexOf(ChatMode.Debug);
		assert.ok(planIndex >= 0 && debugIndex >= 0);
		assert.strictEqual(debugIndex, planIndex + 1);
		assert.ok(builtin.indexOf(ChatMode.Multitask) > debugIndex);
	});
	...
	test('should adjust builtin modes based on tools agent availability', async () => {
		...
		assert.strictEqual(agents.builtin.find(agent => agent.id === ChatMode.Multitask.id), undefined);
		assert.strictEqual(agents.builtin.find(agent => agent.id === ChatMode.Debug.id), undefined);
	...
	test('should find Multitask as a permanent builtin mode', async () => {
		const multitaskMode = (await chatModeService.getLocalModes()).findModeById('multitask');
		assert.strictEqual(multitaskMode, ChatMode.Multitask);
		assert.strictEqual(multitaskMode?.isBuiltin, true);
	});

	test('should find Debug as a permanent builtin mode (saved-mode restore)', async () => {
		const debugMode = (await chatModeService.getLocalModes()).findModeById('debug');
		assert.strictEqual(debugMode, ChatMode.Debug);
		assert.strictEqual(debugMode?.isBuiltin, true);
	});
