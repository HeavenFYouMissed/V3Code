...
suite('Response', () => {
	...
	test('file edits keep narration in chronological chapters', () => {
		const response = store.add(new Response([]));
		response.updateContent({ content: new MarkdownString('Before edit'), kind: 'markdownContent' });
		response.updateContent({ kind: 'textEditGroup', uri: URI.parse('file:///file1.ts'), edits: [], state: undefined, done: true });
		response.updateContent({ content: new MarkdownString('After edit'), kind: 'markdownContent' });
