...
suite('LocalAgentsSessionsController', () => {
	...
	suite('Session Status', () => {
		test('should track multiple in-progress sessions independently', async () => {
			return runWithFakedTimers({}, async () => {
				const controller = createController();
				const firstResource = LocalChatSessionUri.forSession('concurrent-first');
				const secondResource = LocalChatSessionUri.forSession('concurrent-second');
				const firstModel = createMockChatModel({ sessionResource: firstResource, requestInProgress: true });
				const secondModel = createMockChatModel({ sessionResource: secondResource, requestInProgress: true });

				mockChatService.addSession(firstModel);
				mockChatService.addSession(secondModel);
				mockChatService.setLiveSessionItems([
					{
						sessionResource: firstResource,
						title: 'First Concurrent Session',
						lastMessageDate: Date.now(),
						isActive: true,
						lastResponseState: ResponseModelState.Complete,
						timing: createTestTiming()
					...
					{
						sessionResource: secondResource,
						title: 'Second Concurrent Session',
						lastMessageDate: Date.now(),
						isActive: true,
						lastResponseState: ResponseModelState.Complete,
						timing: createTestTiming()
				...
				await controller.refresh(CancellationToken.None);
				assert.deepStrictEqual(controller.items.map(item => item.status), [
					ChatSessionStatus.InProgress,
					ChatSessionStatus.InProgress
				]);

				firstModel.setRequestInProgress(false);
				await Event.toPromise(Event.filter(controller.onDidChangeChatSessionItems, delta => !!delta.addedOrUpdated?.some(item => item.resource.toString() === firstResource.toString())));

				assert.strictEqual(controller.items.find(item => item.resource.toString() === firstResource.toString())?.status, ChatSessionStatus.Completed);
				assert.strictEqual(controller.items.find(item => item.resource.toString() === secondResource.toString())?.status, ChatSessionStatus.InProgress);
			});
		});
	...
	suite('Events', () => {
		...
		test('should remove session from items when onDidDisposeSession fires and history is empty', async () => {
			return runWithFakedTimers({}, async () => {
				const controller = createController();
				...
				// Dispose now triggers refresh — clear live+history like a deleted entry.
				const removedResources: URI[] = [];
				const changed = Event.toPromise(controller.onDidChangeChatSessionItems);
				...
				assert.strictEqual(controller.items.length, 0, 'items should be empty after dispose with no history');
		...
		test('should keep disposed session visible via history after model ref release', async () => {
			return runWithFakedTimers({}, async () => {
				const controller = createController();

				const sessionResource = LocalChatSessionUri.forSession('dispose-to-history-session');
				const mockModel = createMockChatModel({
					sessionResource,
					hasRequests: true,
					customTitle: 'Background Agent'
				});
				...
				const changed = Event.toPromise(controller.onDidChangeChatSessionItems);
				// Switching away: live model gone, willDisposeModel already wrote history.
				mockChatService.setLiveSessionItems([]);
				mockChatService.setHistorySessionItems([{
					...detail,
					isActive: false,
				...
				assert.strictEqual(controller.items.length, 1, 'history item should replace the live session');
				assert.strictEqual(controller.items[0].label, 'Background Agent');
				assert.strictEqual(controller.items[0].resource.toString(), sessionResource.toString());
			});
		});
		...
		test('should not re-add disposed session to items on refresh', async () => {
			return runWithFakedTimers({}, async () => {
				...
				// Dispose the session — clear live+history like a deleted entry
				mockChatService.setLiveSessionItems([]);
				mockChatService.setHistorySessionItems([]);
				const changed = Event.toPromise(controller.onDidChangeChatSessionItems);
				mockChatService.fireDidDisposeSession([sessionResource]);
		...
		test('should include empty live sessions so New Chat appears immediately', async () => {
			return runWithFakedTimers({}, async () => {
				const controller = createController();

				const sessionResource = LocalChatSessionUri.forSession('empty-new-chat');
				const mockModel = createMockChatModel({
					sessionResource,
					hasRequests: false,
