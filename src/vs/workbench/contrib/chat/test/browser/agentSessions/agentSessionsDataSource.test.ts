...
suite('AgentSessionsDataSource', () => {
	...
	suite('groupSessionsByRepository', () => {
		...
		test('can render pinned sessions as a dedicated section for the unified sidebar', () => {
			const pinnedSession = createMockSession({ id: 'pinned-section', isPinned: true, metadata: { repositoryPath: '/path/zebra' } });
			const filter = createMockFilter({ groupBy: AgentSessionsGrouping.Repository });
			const dataSource = disposables.add(new AgentSessionsDataSource(filter, createMockSorter(), undefined, true));
			const result = Array.from(dataSource.getChildren(createMockModel([pinnedSession])));

			assert.strictEqual(result.length, 1);
			assert.ok(isAgentSessionSection(result[0]));
			assert.strictEqual(result[0].section, AgentSessionSection.Pinned);
			assert.strictEqual(result[0].label, 'Pinned');
			assert.strictEqual(result[0].sessions[0].resource.toString(), pinnedSession.resource.toString());
		});
	});
