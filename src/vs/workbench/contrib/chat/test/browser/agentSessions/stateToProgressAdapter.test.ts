...
suite('stateToProgressAdapter', () => {
	...
	suite('turnsToHistory', () => {
		...
		test('folds cacheReadTokens into promptTokens (cache-hot sessions must not read ~0%)', () => {
			// inputTokens is the NON-cached portion only; on a cache-hot turn nearly the whole
			// prompt arrives as cacheReadTokens. Dropping it made the context ring show ~0%.
			const turn = createTurn({
				usage: { inputTokens: 12, outputTokens: 300, cacheReadTokens: 326_000, model: 'opus-4.7' },
				responseParts: [{ kind: ResponsePartKind.Markdown, id: 'md-1', content: 'Done' }],
			});
			...
			const usagePart = response.parts.find((part): part is IChatUsage => part.kind === 'usage');
			assert.ok(usagePart, 'expected a usage part');
			assert.strictEqual(usagePart.promptTokens, 326_012);
			assert.strictEqual(usagePart.completionTokens, 300);
		});
