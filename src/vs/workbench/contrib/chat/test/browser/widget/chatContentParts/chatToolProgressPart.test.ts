...
import { isMcpToolInvocation, shouldKeepMcpToolStandalone } from '../../../../browser/widget/chatContentParts/toolInvocationParts/chatToolPartUtilities.js';
...
suite('ChatToolProgressSubPart', () => {
	...
	test('groups only plain MCP calls in compact-all-grouped mode', () => {
		const plainMcp = createSerializedToolInvocation({ source: undefined, toolId: 'mcp__weather' });
		const richMcp = {
			...plainMcp,
			toolSpecificData: {
				kind: 'input' as const,
				...
				mcpAppData: {
					...
					serverDefinitionId: 'weather-server',
					collectionId: 'weather-collection',
				},
			},
		};

		assert.strictEqual(shouldKeepMcpToolStandalone(plainMcp, CollapsedToolsDisplayMode.Always), false);
		assert.strictEqual(shouldKeepMcpToolStandalone(plainMcp, CollapsedToolsDisplayMode.WithThinking), true);
		assert.strictEqual(shouldKeepMcpToolStandalone(richMcp, CollapsedToolsDisplayMode.Always), true);
		assert.strictEqual(
			shouldKeepMcpToolStandalone(createSerializedToolInvocation({ toolId: 'read_file' }), CollapsedToolsDisplayMode.Always),
			false,
		);
	});
