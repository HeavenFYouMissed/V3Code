...
suite('ChatThinkingContentPart', () => {
	...
	suite('ThinkingDisplayMode.FixedScrolling', () => {
		...
		test('should collapse the rolling body when finalized and remain expandable', () => {
			const content = createThinkingPart('**Inspecting files**\nReviewed package configuration.');
			const context = createMockRenderContext(false);
			...
			const contentList = part.domNode.querySelector('.chat-used-context-list');
			assert.ok(contentList, 'The active phase should eagerly create its rolling body');
			assert.ok(contentList.classList.contains('chat-thinking-streaming'),
				'The active phase should expose its fixed-height rolling body');

			part.finalizeTitleIfDefault();
			part.markAsInactive();

			assert.ok(part.domNode.classList.contains('chat-used-context-collapsed'),
				'The completed phase should remain represented by its collapsed summary');
			assert.ok(!contentList.classList.contains('chat-thinking-streaming'),
				'The completed phase body should no longer stay visible below the summary');

			const summaryButton = part.domNode.querySelector<HTMLButtonElement>('.chat-used-context-label .monaco-button');
			assert.ok(summaryButton, 'The completed phase should retain an expandable summary button');
			summaryButton.click();

			assert.ok(!part.domNode.classList.contains('chat-used-context-collapsed'),
				'Clicking the completed summary should reopen the full work history');
		});
	});
