...
suite('ChatToolInvocationPart todo synchronization', () => {
	const store = ensureNoDisposablesAreLeakedInTestSuite();

	test('replaces partial todos with the final merged list when the invocation completes', async () => {
		const todoSnapshots: IChatTodo[][] = [];
		const todoListService: IChatTodoListService = {
			_serviceBrand: undefined,
			onDidUpdateTodos: Event.None,
			getTodos: () => todoSnapshots[todoSnapshots.length - 1] ?? [],
			setTodos: (_sessionResource, todos) => todoSnapshots.push(todos.map(todo => ({ ...todo }))),
			migrateTodos: () => { },
		};

		const toolInvocation = new ChatToolInvocation(
			{
				invocationMessage: 'Updating plan',
				toolSpecificData: {
					kind: 'todoList',
					todoList: [
						{ id: '2', title: 'Implement change', status: 'in-progress' },
					],
				},
			},
			{
				...
				modelDescription: 'Update the current plan',
		...
		const instantiationService = {
			createInstance: () => ({
				...
				codeblocks: [],
				onNeedsRerender: Event.None,
				dispose: () => { },
			}),
		};

		store.add(new ChatToolInvocationPart(
			toolInvocation,
			{ element: { sessionResource: testSessionUri } } as never,
			{} as never,
			{} as never,
			{} as never,
			() => 500,
		...
		assert.deepStrictEqual(todoSnapshots, [[
			{ id: 2, title: 'Implement change', status: 'in-progress' },
		]]);

		// The final presenter replaces the partial payload with the complete merged
		// plan, but its kind remains `todoList`, so only the state transition changes.
		toolInvocation.toolSpecificData = {
			kind: 'todoList',
			todoList: [
				{ id: '1', title: 'Inspect current behavior', status: 'completed' },
				{ id: '2', title: 'Implement change', status: 'in-progress' },
			],
		};
