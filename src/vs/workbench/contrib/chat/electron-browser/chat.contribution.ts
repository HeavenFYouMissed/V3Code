...
import { ChatEditor, type IChatEditorOptions } from '../browser/widgetHosts/editor/chatEditor.js';
...
class ChatCommandLineHandler extends Disposable {
	...
	private registerListeners() {
		...
		ipcRenderer.on('vscode:openAgentWorkspace', (_, ...args: unknown[]) => {
			const sessionUriString = args[0] as string;
			this.logService.trace('vscode:openAgentWorkspace', sessionUriString);

			const sessionResource = URI.parse(sessionUriString);
			void this.openAgentWorkspace(sessionResource);
		});
	}

	private async openAgentWorkspace(sessionResource: URI): Promise<void> {
		// Agent Workspace now runs in an ordinary top-level CodeWindow instead of
		// an auxiliary editor window. The old auxiliary-window path applied this
		// class while constructing its container; the native Agent-companion
		// path has to establish the same workbench-level layout contract here.
		// Do this before opening ChatEditor so its first layout uses the real
		// three-pane geometry rather than correcting itself after a resize.
		this.layoutService.mainContainer.classList.add('agent-workspace-window');
		this.setAgentWorkspaceChromeHidden(true);

		// ACTIVE_GROUP can resolve to an auxiliary editor part restored for this
		// workspace. Agent Workspace is a full workbench window, so always anchor
		// its ChatEditor in the main editor part of the newly opened CodeWindow.
		await this.editorGroupsService.mainPart.whenReady;
		const options = {
			pinned: true,
			sticky: true,
			agentWorkspace: true,
		...
		await this.chatWidgetService.openSession(sessionResource, this.editorGroupsService.mainPart.activeGroup, options);

		// The Agent companion may already have restored this exact session as a
		// normal ChatEditor. In that case the resolver reuses the pane and there is
		// intentionally no second setInput call. Apply the window mode directly to
		// the live pane so restore order cannot decide whether the shell exists.
		const activePane = this.editorService.activeEditorPane;
		if (activePane instanceof ChatEditor) {
			activePane.setAgentWorkspaceEnabled(true);
		} else {
			this.logService.warn('[Agent Workspace] target did not activate a ChatEditor pane');
		}

		// Startup restoration can make workbench parts visible after the first
		// transient hide. Reassert the Agent contract once the target editor is
		// active, then lay out against the final three-pane geometry.
		this.setAgentWorkspaceChromeHidden(true);
		this.layoutService.layout();
		this.logService.info(`[Agent Workspace] activated session=${sessionResource.toString()}`);
	}

	private setAgentWorkspaceChromeHidden(hidden: boolean): void {
		for (const part of [
			Parts.TITLEBAR_PART,
			Parts.BANNER_PART,
			...
			this.layoutService.setPartHiddenTransient?.(hidden, part);
		}
	}
