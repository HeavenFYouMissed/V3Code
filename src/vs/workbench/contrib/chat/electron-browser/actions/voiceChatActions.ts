...
import { IChatExecuteActionContext, V3_VOICE_INPUT_START_COMMAND_ID, V3_VOICE_INPUT_STOP_COMMAND_ID } from '../../browser/actions/chatExecuteActions.js';
import { disarmV3VoicePressToTalkHold } from '../../../void/browser/v3codeVoicePressToTalk.js';
...
class VoiceChatSessions {
	...
	async start(controller: IVoiceChatSessionController, context?: IChatExecuteActionContext): Promise<IVoiceChatSession> {
		...
		// Moving focus during PTT pointer-hold cancels capture on the mic button.
		if (typeof context?.v3VoicePointerId !== 'number') {
...
async function startVoiceChatWithHoldMode(id: string, accessor: ServicesAccessor, target: 'view' | 'inline' | 'quick' | 'focused', context?: IChatExecuteActionContext): Promise<void> {
	...
	const session = await VoiceChatSessions.getInstance(instantiationService).start(controller, {
		...context,
		voice: { disableTimeout: true, ...context?.voice },
	});
...
export class StopListeningAction extends Action2 {
	...
	constructor() {
		super({
			...
			// No ChatExecute menu — V3Code uses a single press-to-talk mic (order 2).
			// Injecting StopListening here at order 3 collided with the paperclip and
			// re-rendered the toolbar mid-hold, killing pointer capture.
		});
	}
...
CommandsRegistry.registerCommand(V3_VOICE_INPUT_START_COMMAND_ID, async (accessor, context?: IChatExecuteActionContext) => {
	const instantiationService = accessor.get(IInstantiationService);
	const widget = context?.widget;
	const isPressToTalk = typeof context?.v3VoicePointerId === 'number';
	if (widget && !isPressToTalk) {
		widget.focusInput();
	}
	...
	const sessions = VoiceChatSessions.getInstance(instantiationService);
	try {
		await sessions.start(controller, {
			...context,
			voice: { disableTimeout: true, ...context?.voice },
		});
	} catch {
		...
		sessions.stop();
		throw new Error(localize('v3code.voice.startFailed', "Could not start voice input"));
	}
});

CommandsRegistry.registerCommand(V3_VOICE_INPUT_STOP_COMMAND_ID, (accessor) => {
	disarmV3VoicePressToTalkHold();
	VoiceChatSessions.getInstance(accessor.get(IInstantiationService)).stop();
});
