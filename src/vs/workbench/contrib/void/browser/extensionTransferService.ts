...
/**
 * Where a transfer writes to. Always resolved from the running product's own profile/environment —
 * never hardcoded, so the destinations stay correct for whatever product.json this build ships with.
 */
type TransferDestinations = { settings: URI, keybindings: URI, extensionsFolder: URI }
...
class ExtensionTransferService extends Disposable implements IExtensionTransferService {
	...
	/**
	 * Resolves the transfer destinations from the running product (settings/keybindings of the current
	 * profile + the extensions install folder), so a transfer can never write into another product's
	 * data folders and can only ever overwrite this product's own files.
	 */
	private _getDestinations(): TransferDestinations {
		const { settingsResource, keybindingsResource } = this._userDataProfileService.currentProfile
		// extensions are installed in the default profile's extensions folder (eg ~/<dataFolderName>/extensions); extensionsResource points at the extensions.json inside it
		const extensionsFolder = URI.joinPath(this._userDataProfilesService.defaultProfile.extensionsResource, '..')
		return { settings: settingsResource, keybindings: keybindingsResource, extensionsFolder }
	}

	async transferExtensions(os: 'mac' | 'windows' | 'linux' | null, fromEditor: TransferEditorType) {
		const transferTheseFiles = transferTheseFilesOfOS(os, fromEditor, this._getDestinations())
	...
	async deleteBlacklistExtensions(os: 'mac' | 'windows' | 'linux' | null) {
		if (os === null) return
		const fileService = this._fileService
		const extensionsURI = this._getDestinations().extensionsFolder
...
// sources are the other product's well-known install locations; destinations always come from `destinations` (the running product), never from a literal
const transferTheseFilesOfOS = (os: 'mac' | 'windows' | 'linux' | null, fromEditor: TransferEditorType, destinations: TransferDestinations): TransferFilesInfo => {
	if (os === null)
