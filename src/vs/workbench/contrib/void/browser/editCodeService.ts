...
import { IEditCodeService, AddCtrlKOpts, StartApplyingOpts, CallBeforeStartApplyingOpts, ApplyTurboDraftReviewResult, } from './editCodeServiceInterface.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		const initializeModel = async (model: ITextModel) => {
			...
			// Ephemeral diff-snapshot models (the v3code-edit-snapshot scheme created by the tool adapter
			// for the chat diff card) are read-only and owned by the adapter. Don't treat them as editable
			// files here — no diff-area tracking, no content listeners, no VoidModelService ref — and skip
			// the resolve round-trip that previously logged "Unable to resolve resource" on every edit.
			if (model.uri.scheme === 'v3code-edit-snapshot') { return }
	...
	/**
	 * Surfaces a failed edit stream to the user. `message` must already be a human sentence
	 * (see providerErrorMessage in sendLLMMessage.impl.ts) — never a raw stack or HTTP status.
	 */
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					// Deletions / "wrong" code get the darker red treatment (Turbo ghost draft).
					domNode.className = type === 'deletion' ? 'void-redDarkBG' : 'void-redBG'
	...
	public instantlyApplySearchReplaceBlocks({ uri, searchReplaceBlocks, clearEditorDiffUI, forceReview }: { uri: URI, searchReplaceBlocks: string, clearEditorDiffUI?: boolean, forceReview?: boolean }) {
		// start diffzone
		...
		const onDone = () => {
			...
			// forceReview (Turbo Draft) always keeps DiffZones for Tab/Del cherry-pick
			if (!forceReview && (clearEditorDiffUI || this._settingsService.state.globalSettings.autoAcceptLLMChanges)) {
	...
	public applyTurboDraftReview({ uri, searchReplaceBlocks, expectedSource, turboRunId }: {
		uri: URI;
		...
		const { model } = this._voidModelService.getModel(uri);
		if (!model) {
			return { ok: false, reason: 'missing-model', message: 'File model is not available.' };
		}

		const live = normalize(model.getValue(EndOfLinePreference.LF));
		const expected = normalize(expectedSource);
		if (live !== expected) {
			return { ok: false, reason: 'source-changed', message: 'File changed while Turbo Draft was running.' };
		}

		for (const id of this.diffAreasOfURI[uri.fsPath] ?? []) {
			const area = this.diffAreaOfId[id];
			if (!area) { continue; }
			if (area.type === 'DiffZone' || area.type === 'CtrlKZone') {
				return { ok: false, reason: 'conflict', message: 'Another review or Ctrl+K zone is already open on this file.' };
			}
		}

		let finalText: string;
		try {
			finalText = applySearchReplaceBlocksToString(live, searchReplaceBlocks);
		} catch (e) {
			return {
				ok: false,
		...
		if (finalText === live) {
			return { ok: false, reason: 'zero-diff', message: 'Computed draft matches the current file.' };
		}
		...
		if (!res) {
			return { ok: false, reason: 'missing-model', message: 'Could not start a DiffZone for this file.' };
		}
		const { diffZone, onFinishEdit } = res;
		diffZone.turboRunId = turboRunId;
		...
		const pendingDiffs = Object.keys(diffZone._diffOfId).length;
		if (pendingDiffs === 0) {
			this.acceptOrRejectAllDiffAreas({ uri, removeCtrlKs: false, behavior: 'reject', _addToHistory: false });
			return { ok: false, reason: 'zero-diff', message: 'DiffZone produced no reviewable hunks.' };
		}

		return { ok: true, diffAreaId: diffZone.diffareaid, pendingDiffs };
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		// Settles a failed stream: same cleanup as onDone EXCEPT the Ctrl+K zone is kept (so the
		// user can retry or dismiss) and nothing is auto-accepted. Notifies the user, then throws
		// so the runWriteover loop rejects applyDonePromise — callers rely on that rejection.
		// Must only be called from inside runWriteover, never from an IPC event callback.
		const onError = (e: { message: string; fullError: Error | null; }) => {
			diffZone._streamState = { isStreaming: false, }
			this._onDidChangeStreamingInDiffZone.fire({ uri, diffareaid: diffZone.diffareaid })
			if (ctrlKZoneIfQuickEdit) {
				ctrlKZoneIfQuickEdit._linkedStreamingDiffZone = null
				this._onDidChangeStreamingInCtrlKZone.fire({ uri, diffareaid: ctrlKZoneIfQuickEdit.diffareaid })
			}
			this._refreshStylesAndDiffsInURI(uri)
			onFinishEdit()
			this._undoHistory(uri)
			this._notifyError(localize('void.editCode.editFailed', "The edit failed and no changes were made. {0}", e.message))
			throw e.fullError || new Error(e.message)
		}
		...
		const runWriteover = async () => {
			...
			while (shouldSendAnotherMessage) {
				...
				const errorRef: { current: { message: string; fullError: Error | null } | null } = { current: null }
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onError: (e) => {
						// this runs inside an IPC event listener — a throw here would vanish there
						// (never surfacing, never settling applyDonePromise, leaving the Ctrl+K
						// input disabled forever). Record and settle; the loop below re-throws.
						errorRef.current = e
						resMessageDonePromise()
					},
					onAbort: () => {
				...
				// should never happen, just for safety (a null id with no error means nothing was dispatched)
				if (streamRequestIdRef.current === null && !errorRef.current) { return }

				await messageDonePromise
				if (errorRef.current) {
					onError(errorRef.current) // cleans up, notifies the user, and throws → applyDonePromise rejects
				}
				if (aborted) {
	...
	private _instantlyApplySRBlocks(uri: URI, blocksStr: string) {
		...
		for (const b of blocks) {
			// Exact-offset fast path (mirrors searchReplaceOnString.ts): a mid-line exact match
			// must replace only the matched characters — the old line-granular expansion silently
			// deleted the rest of those lines while reporting success.
			...
			// Mid-line guard for the whitespace fallback: replacing these whole lines must remove
			// exactly ORIGINAL (modulo whitespace); otherwise neighboring text ORIGINAL never
			// mentioned would be destroyed — refuse instead of corrupting the file.
			...
			if (removeWhitespaceExceptNewlines(spanText) !== removeWhitespaceExceptNewlines(b.orig)) {
				throw new Error(`The edit was not applied. The ORIGINAL text matches only PART of a line, so applying it would destroy the rest of that line:\n${b.orig}. Include the full line(s) in ORIGINAL and retry.`)
			}
	...
	private _initializeSearchAndReplaceStream(opts: StartApplyingOpts & { from: 'ClickApply' }): [DiffZone, Promise<void>] | undefined {
		...
		// Settles a failed stream: cleanup, undo, notify the user, then throw so the
		// runSearchReplace loop rejects applyDonePromise — callers rely on that rejection.
		// Must only be called from inside runSearchReplace, never from an IPC event callback.
		const onError = (e: { message: string; fullError: Error | null; }) => {
			onDone()
			this._undoHistory(uri)
			this._notifyError(localize('void.editCode.applyFailed', "Apply failed and no changes were made. {0}", e.message))
			throw e.fullError || new Error(e.message)
		}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`V3Code: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
						...
						try {
							...
							errorRef.current = { message: e instanceof Error ? e.message : e + '', fullError: e instanceof Error ? e : null }
							resMessageDonePromise()
						}
					},
					onError: (e) => {
						// this runs inside an IPC event listener — a throw here would vanish there
						// (never surfacing, never settling applyDonePromise). Record and settle;
						// the loop below re-throws.
						errorRef.current = e
						resMessageDonePromise()
					},
					onAbort: () => {
				...
				// should never happen, just for safety (a null id with no error means nothing was dispatched)
				if (streamRequestIdRef.current === null && !errorRef.current) { break }
