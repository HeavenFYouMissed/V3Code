...
if (isWatch) {
	...
	if (!fs.existsSync('src2')) {
		try {
			console.log('[buildreact] Running initial scope-tailwind build to create src2 folder...');
			execSync(
				'npx scope-tailwind ./src -o src2/ -s void-scope -c styles.css -p "void-"',
				{ stdio: 'inherit' }
			);
			console.log('[buildreact] src2/ created successfully.');
		} catch (err) {
			console.error('[buildreact] Error running initial scope-tailwind build:', err);
			process.exit(1);
		}
	}
	...
	console.log('[buildreact] Watchers started. Press Ctrl+C to stop both watchers.');
} else {
	// Build mode
	console.log('[buildreact] Building...');
	...
	console.log('[buildreact] Build complete.');
	copyReactBundleToHostOut();
}

function copyReactBundleToHostOut() {
	const localOut = path.join(__dirname, 'out');
	// react/ -> repo root is 7 levels up (see build.js path comment block).
	const hostOut = path.resolve(__dirname, '../../../../../../../out/vs/workbench/contrib/void/browser/react/out');
	if (!fs.existsSync(localOut)) {
		console.warn('[buildreact] Local react/out missing -- tsup may have failed.');
		return;
	}
	if (!fs.existsSync(path.dirname(hostOut))) {
		console.warn('[buildreact] Host out/ tree missing -- run a full compile once. V3Code may load a stale settings bundle until then.');
		return;
	}
	fs.mkdirSync(hostOut, { recursive: true });
	fs.cpSync(localOut, hostOut, { recursive: true });
	console.log(`[buildreact] Copied React bundle -> ${hostOut}`);
}
