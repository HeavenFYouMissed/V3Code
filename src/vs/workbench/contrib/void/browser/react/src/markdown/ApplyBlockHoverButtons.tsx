...
import { getBasename, ListableToolItem, voidOpenFileFn, ToolChildrenWrapper } from '../shared-tsx/chatShared.js'
...
const ApplyButtonsForEdit = ({
	...
	const onClickSubmit = useCallback(async () => {
		...
		if (!applyDonePromise) {
			notificationService.info(`V3Code Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`)
		}

		// on any failure, interrupt the stream if it is still going. The error itself is surfaced
		// by EditCodeService with a specific, actionable message — do not double-notify here
		// (this catch also fires on a user-initiated stop, which needs no toast at all).
		applyDonePromise?.catch(() => {
			const uri = getUriBeingApplied(applyBoxId)
