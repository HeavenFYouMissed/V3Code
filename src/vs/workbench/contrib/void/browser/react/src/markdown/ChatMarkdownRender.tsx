...
import { getBasename, getRelative, voidOpenFileFn } from '../shared-tsx/chatShared.js'
...
const Codespan = ({ text, className, onClick, tooltip }: { text: string, className?: string, onClick?: () => void, tooltip?: string }) => {
	...
	// break-words: a codespan is ONE unbreakable token by default, so a long path or expression
	// (`/Users/daniel/dev/calculator/`) pushed the whole message wider than the chat column and
	// bled past the gutter. overflow-wrap only breaks when it genuinely has to, so short spans
	// are unaffected.
	return <code
		className={`font-mono font-medium rounded-sm bg-void-bg-1 px-1 break-words ${className}`}
		onClick={onClick}
		{...tooltip ? {
