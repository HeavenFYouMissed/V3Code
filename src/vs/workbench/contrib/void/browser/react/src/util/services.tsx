...
import { RefreshModelStateOfProvider, ProviderHealth } from '../../../../../../../workbench/contrib/void/common/refreshModelService.js'
...
import { IChatThreadService, SubagentState, ThreadsState, ThreadStreamState } from '../../../chatThreadService.js'
...
let chatSubagentState: SubagentState = {}
const chatSubagentStateListeners: Set<(s: SubagentState) => void> = new Set()
...
let providerHealthState: Partial<Record<RefreshableProviderName, ProviderHealth>> = {}
const providerHealthListeners: Set<(s: Partial<Record<RefreshableProviderName, ProviderHealth>>) => void> = new Set()
...
let semanticIndexStatus: IndexStatus
const semanticIndexListeners: Set<(s: IndexStatus) => void> = new Set()
...
export const _registerServices = (accessor: ServicesAccessor) => {
	...
	const stateServices = {
		...
		semanticIndexService: accessor.get(ISemanticIndexService),
	}

	const { settingsStateService, chatThreadsStateService, refreshModelService, themeService, editCodeService, voidCommandBarService, modelService, mcpService, semanticIndexService } = stateServices
	...
	// background subagents (Agents panel)
	chatSubagentState = { ...chatThreadsStateService.subagentState }
	disposables.push(
		chatThreadsStateService.onDidChangeSubagentState(() => {
			// shallow-copy so React state-setters see a new reference and re-render
			chatSubagentState = { ...chatThreadsStateService.subagentState }
			chatSubagentStateListeners.forEach(l => l(chatSubagentState))
		})
	)
	...
	providerHealthState = { ...refreshModelService.healthOfProvider }
	disposables.push(
		refreshModelService.onDidChangeHealth(() => {
			// spread so React state identity changes on every probe result
			providerHealthState = { ...refreshModelService.healthOfProvider }
			providerHealthListeners.forEach(l => l(providerHealthState))
		})
	)
	...
	semanticIndexStatus = semanticIndexService.getStatus()
	disposables.push(
		semanticIndexService.onDidChangeStatus((status) => {
			semanticIndexStatus = status
			semanticIndexListeners.forEach(l => l(semanticIndexStatus))
		})
	)
...
const getReactAccessor = (accessor: ServicesAccessor) => {
	const reactAccessor = {
		...
		IExternalAgentsService: accessor.get(IExternalAgentsService),

		IStorageService: accessor.get(IStorageService),

		IVibeModeService: (() => { try { return accessor.get(IVibeModeService); } catch { return null as unknown as IVibeModeService; } })(),

		ISemanticIndexService: accessor.get(ISemanticIndexService),

		IChatOutputRendererService: accessor.get(IChatOutputRendererService),

		IV3CodeAccountService: accessor.get(IV3CodeAccountService),
...
export const useSubagentState = () => {
	const [s, ss] = useState(chatSubagentState)
	useEffect(() => {
		ss(chatSubagentState)
		chatSubagentStateListeners.add(ss)
		return () => { chatSubagentStateListeners.delete(ss) }
	}, [ss])
	return s
}
...
export const useProviderHealth = () => {
	const [s, ss] = useState(providerHealthState)
	useEffect(() => {
		ss(providerHealthState)
		providerHealthListeners.add(ss)
		return () => { providerHealthListeners.delete(ss) }
	}, [ss])
	return s
}
...
export const useSemanticIndexState = () => {
	...
	const [s, ss] = useState(indexService.getStatus())
	useEffect(() => {
		const listener = (status: IndexStatus) => { ss(status) }
		semanticIndexListeners.add(listener)
		return () => { semanticIndexListeners.delete(listener) }
	}, [])
	return s
}
