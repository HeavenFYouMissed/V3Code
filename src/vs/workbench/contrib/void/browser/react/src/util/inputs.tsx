...
import { getBasename, getFolderName } from '../shared-tsx/chatShared.js';
...
import { workspaceIndexUri } from '../../../../common/semanticIndex/workspaceIndexPath.js';
	...
	const searchCodebase = async (t: string): Promise<Option[]> => {
		...
		try {
			...
			for (const hit of hits) {
				...
				const fileUri = workspaceIndexUri(workspaceFolders, filePath);
				if (!fileUri) continue;
				opts.push({
					leafNodeType: 'File',
					uri: fileUri,
	...
	return <>
		...
		{isMenuOpen && (
			<div
				...
				style={{
					position: strategy,
					// See the model dropdown below: null coords on the first paint after open put
					// this at the screen corner until floating-ui measured. Stay hidden instead.
					top: y ?? 0,
					left: x ?? 0,
...
export const VoidSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, onPaste: externalOnPaste, onBlur: externalOnBlur, ...inputProps }: {
	value: string;
	...
	// Right-click / context-menu paste inside the VS Code workbench inserts text WITHOUT firing the
	// input's onChange, so a pasted value (e.g. an API key) would never be saved and no models would
	// load — the user had to type and delete a character to force a change event. Commit the value
	// once the browser has applied the paste. (Cmd+V already fires onChange; setSettingOfProvider is
	// idempotent, so double-covering is harmless.)
	const handlePaste = useCallback((e: React.ClipboardEvent<HTMLInputElement>) => {
		externalOnPaste?.(e);
		const el = e.currentTarget;
		requestAnimationFrame(() => onChangeValue(el.value));
	}, [onChangeValue, externalOnPaste]);

	// Safety net: whatever is in the box gets committed when focus leaves, regardless of how the text
	// got there (covers any paste path that doesn't fire onChange).
	const handleBlur = useCallback((e: React.FocusEvent<HTMLInputElement>) => {
		onChangeValue(e.currentTarget.value);
		externalOnBlur?.(e);
	}, [onChangeValue, externalOnBlur]);
		...
		<div className={`inline-block relative ${className}`}>
			...
			>
				{options.map((option, optionIndex) => {
					...
					return (
						// Must mirror the real row's box exactly (px-2 pr-4, w-4 icon col, gap-x-1),
						// or the menu is measured narrower than it renders and the inner
						// overflow-auto clips the end of long model names.
						<div key={`${optionName}\u0000${optionDetail}\u0000${optionIndex}`} className="flex items-center px-2 pr-4 whitespace-nowrap">
			...
			{isOpen && (
				<div
					...
					style={{
						position: strategy,
						// floating-ui has not measured anything on the FIRST paint after open, so x
						// and y are null. Falling back to 0 with strategy 'fixed' put the menu at the
						// top-left corner of the SCREEN, where it sat until the first positioning
						// pass landed and snapped it into place. Worst right after a cold launch,
						// when nothing is warm yet — which is exactly when a new user opens the model
						// picker for the first time.
						//
						// Keep it out of the layout until it has a real position instead of drawing
						// it in the wrong one.
						top: y ?? 0,
						left: x ?? 0,
						visibility: x === null || y === null ? 'hidden' : 'visible',
				...
				><div className='overflow-auto max-h-80'>
						...
						{options.map((option, optionIndex) => {
							...
							return (
								<div
									key={`${optionName}\u0000${optionDetail}\u0000${optionIndex}`}
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	// Plain read-only chat code should not mount live Monaco editors. Multiple simple
	// widgets share Monaco's layout machinery, and parent/card updates can make all
	// instances relayout together. A static <pre> is stable, selectable, cheap, and
	// leaves Monaco for real editor/diff surfaces.
	const capped = maxHeight !== undefined && Number.isFinite(maxHeight)
	return <pre
		className='relative z-0 px-3 py-2 bg-void-bg-3 rounded-lg border border-void-border-3/40 overflow-x-auto text-xs font-mono leading-[17px] whitespace-pre'
		data-language={language ?? 'plaintext'}
		style={{
			maxHeight: capped ? `${maxHeight}px` : undefined,
			overflowY: showScrollbars || capped ? 'auto' : 'hidden',
			tabSize: 2,
		}}
	><code>{initValue}</code></pre>
	...
	useEffect(() => {
		...
		const editor = instantiationService.createInstance(
			...
			divRef.current,
			{
				...
				readOnly: true,
				renderSideBySide: false, // unified inline diff (red removed / green added stacked)
