...
export { IVoidCommandBarService } from './voidCommandBarServiceTypes.js';
export type { CommandBarStateType } from './voidCommandBarServiceTypes.js';
import { IVoidCommandBarService } from './voidCommandBarServiceTypes.js';
import type { CommandBarStateType } from './voidCommandBarServiceTypes.js';
...
export class VoidCommandBarService extends Disposable implements IVoidCommandBarService {
	...
	constructor(
		...
		const setActiveURI = (uri: URI | null) => {
		...
		const onCodeEditorAdd = (editor: ICodeEditor) => {
			...
			const d2 = editor.onDidChangeModel((e) => {
				...
				setActiveURI(e.newModelUrl);
			});
			disposablesOfEditorId[id].push(d2);
			const d3 = editor.onDidFocusEditorText(() => {
				const model = editor.getModel();
				if (model) { setActiveURI(model.uri); }
			});
			disposablesOfEditorId[id].push(d3);
		}
		...
		// state updaters — register BEFORE cold hydrate so we never miss DiffZones that already exist
		...
		// Cold hydrate: reconcile DiffZones that already exist on open models
		for (const uri of this._listenToTheseURIs) {
			const diffZones = this._getDiffZonesOnURI(uri);
			if (diffZones.length === 0) { continue; }
			if (!this.sortedURIs.find(uri2 => uri2.fsPath === uri.fsPath)) {
				this._addURIEntryToState(uri);
			}
			const zoneIds = [...(this._editCodeService.diffAreasOfURI[uri.fsPath] ?? [])]
				.filter(id => this._editCodeService.diffAreaOfId[id]?.type === 'DiffZone');
			const sortedDiffIds = this._computeSortedDiffs(zoneIds);
			this._setState(uri, {
				sortedDiffZoneIds: zoneIds,
				sortedDiffIds,
				isStreaming: this._isAnyDiffZoneStreaming(zoneIds),
				diffIdx: sortedDiffIds.length > 0 ? 0 : null,
			});
			this._onDidChangeState.fire({ uri });
		}

		// Seed active URI from the focused editor (not only future model changes)
		const focused = this._codeEditorService.getFocusedCodeEditor()
			?? this._codeEditorService.getActiveCodeEditor();
		const focusedUri = focused?.getModel()?.uri;
		if (focusedUri) {
			setActiveURI(focusedUri);
		}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidAcceptDiffAction', 'V3Code: Accept Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidRejectDiffAction', 'V3Code: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToNextDiffAction', 'V3Code: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToPrevDiffAction', 'V3Code: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToNextUriAction', 'V3Code: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToPrevUriAction', 'V3Code: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidAcceptFileAction', 'V3Code: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidRejectFileAction', 'V3Code: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidAcceptAllDiffsAction', 'V3Code: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidRejectAllDiffsAction', 'V3Code: Reject All Diffs in All Files'),
			keybinding: {
