...
const shouldExcludeDirectory = (name: string) => {
	...
	if (name.match(/\bdist\b/)) return true
...
export const computeDirectoryTree1Deep = async (
	...
	// resolve() throws for a path that does not exist, which reached the model as a raw
	// filesystem error with no hint about what to do next.
	let stat;
	try {
		stat = await fileService.resolve(rootURI, { resolveMetadata: false });
	} catch {
		return { children: null, hasNextPage: false, hasPrevPage: false, itemsRemaining: 0, notFound: true };
	}
	if (!stat.isDirectory) {
...
export const stringifyDirectoryTree1Deep = (params: BuiltinToolCallParams['ls_dir'], result: BuiltinToolResultType['ls_dir']): string => {
	if (result.notFound) {
		return `${params.uri.fsPath} does not exist. Nothing was listed. Check the parent directory first, or create it with create_file_or_folder (a uri ending in "/" makes a folder).`;
	}
	if (!result.children) {
		return `${params.uri.fsPath} is a FILE, not a directory — use read_file to read it.`;
	}
	...
	// An empty directory used to stringify to nothing but the path, which reads as a failed or
	// empty tool result rather than a real, informative answer.
	if (entries.length === 0 && !result.hasNextPage) {
		return `${output}(this directory is empty)`;
	}
...
const renderChildrenCombined = async (
	...
	// The intended per-directory cap for ALL levels below the root. This must be passed
	// DOWN the recursion unchanged — previously the depth-0 `Infinity` below was passed
	// into children, so `Infinity ?? DEFAULT` re-resolved to Infinity at every depth and
	// the cap never re-engaged (e.g. extensions/ expanded all ~80 packs and ate the whole
	// char budget before the walk reached src/).
	const childMaxItemsPerDir = options.maxItemsPerDir ?? DEFAULT_MAX_ITEMS_PER_DIR;
		...
		if (child.isDirectory && !isGitIgnoredDirectory) {
			...
			if (eChildren && eChildren.length > 0) {
				const {
					...
					fileService,
					fileCount,
					{ maxDepth, currentDepth: nextDepth, maxItemsPerDir: childMaxItemsPerDir }
				);
...
class DirectoryStrService extends Disposable implements IDirectoryStrService {
	...
	async getDirectoryStrTool(uri: URI) {
		...
		// The cap was computed into `c` and then immediately thrown away by the next line, so it
		// never applied. Harmless today because `content` is already budget-bounded while it is
		// built, but keep the guard real and disclose it when it fires.
		const capped = content.length > MAX_DIRSTR_CHARS_TOTAL_TOOL
		let c = `Directory of ${uri.fsPath}:\n${content.substring(0, MAX_DIRSTR_CHARS_TOTAL_TOOL)}`
		if (wasCutOff || capped) c = `${c}\n...Result was truncated...`
