...
export interface MCPConfigFileEntryJSON {
	...
	// Pre-registered OAuth app (providers that refuse dynamic client registration —
	// Slack/Google Drive class). The matching client secret NEVER lives in config; it
	// goes to the OS secret store via the Set Client Secret command.
...
export type MCPServer = {
	// Command-based server properties
	tools: MCPTool[],
	/** 'ready' = stopped with cached tools: usable on demand, but NOT connected. */
	status: 'loading' | 'success' | 'offline' | 'ready' | 'needs-user-interaction',
	command?: string,
	error?: string,
} | {
	tools?: undefined,
...
/** A configured server is only connected when the runtime succeeded and it is enabled. */
export function isMcpServerConnected(server: MCPServer | undefined, isEnabled: boolean | undefined): boolean {
	return server?.status === 'success' && isEnabled === true;
}

/** Remote servers are only usable after authentication exposes at least one tool. */
export function isMcpServerUsable(server: MCPServer | undefined, isEnabled: boolean | undefined, requiresTools = false): boolean {
	if (server?.status !== 'success' || isEnabled !== true) {
		return false;
	}
	return !requiresTools || server.tools.length > 0;
}
