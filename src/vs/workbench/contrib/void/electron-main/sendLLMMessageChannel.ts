...
import { getGrokPlanStatus } from './llmMessage/grokSubscriptionAuth.js';
import { getClaudePlanStatus } from './llmMessage/claudeSubscriptionAuth.js';
import { getCopilotPlanStatus } from './llmMessage/copilotSubscriptionAuth.js';
import { getGeminiPlanStatus } from './llmMessage/geminiSubscriptionAuth.js';
import { getCursorLocalStatus } from './llmMessage/cursorLocalAuth.js';
import { getOpenaiPlanStatus } from './llmMessage/openaiPlanSubscriptionAuth.js';
...
export class LLMMessageChannel implements IServerChannel {
	...
	private readonly llmMessageEmitters = {
		...
		onUsage: new Emitter<EventLLMMessageOnUsageParams>(),
	}
	...
	listen(_: unknown, event: string): Event<any> {
		...
		else if (event === 'onUsage_sendLLMMessage') return this.llmMessageEmitters.onUsage.event;
		// list
	...
	async call(_: unknown, command: string, params: any): Promise<any> {
		try {
			...
			else if (command === 'grokPlanStatus') {
				return await getGrokPlanStatus()
			}
			else if (command === 'claudePlanStatus') {
				return await getClaudePlanStatus()
			}
			else if (command === 'copilotStatus') {
				return await getCopilotPlanStatus()
			}
			else if (command === 'geminiPlanStatus') {
				return await getGeminiPlanStatus()
			}
			else if (command === 'cursorLocalStatus') {
				return await getCursorLocalStatus((params as { endpoint?: string } | undefined)?.endpoint)
			}
			else if (command === 'openaiPlanStatus') {
				return await getOpenaiPlanStatus()
			}
			else {
	...
	private _callSendLLMMessage(params: MainSendLLMMessageParams) {
		...
		const mainThreadParams: SendLLMMessageParams = {
			...
			// fired exactly once per request by the sendLLMMessage wrapper (final/error/abort)
			onUsage: (p) => {
				this.llmMessageEmitters.onUsage.fire({ requestId, ...p });
			},
