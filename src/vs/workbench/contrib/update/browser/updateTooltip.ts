...
export class UpdateTooltip extends Disposable {
	...
	private renderTitleAndInfo(title: string, update?: IUpdate) {
		...
		// Release notes button. Upstream defaults the version to productService.version, which is
		// never undefined — so the button was ALWAYS shown, and clicking it ran a command that is
		// only registered when product.json defines releaseNotesUrl (ShowReleaseNotesAction.AVAILABLE
		// in update.contribution.ts). V3Code has no releaseNotesUrl, so every user who opened this
		// tooltip saw a Release Notes button that did nothing at all. Gate on the same condition the
		// registration uses — read here from productService rather than importing the action, so the
		// tooltip keeps no dependency on the contribution. Defining releaseNotesUrl lights this back
		// up with no change needed here.
		this.releaseNotesVersion = this.productService.releaseNotesUrl ? (version ?? this.productService.version) : undefined;
