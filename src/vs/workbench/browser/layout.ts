...
import { UnifiedSidebarPart } from './parts/unifiedsidebar/unifiedSidebarPart.js';
...
enum LayoutClasses {
	...
	UNIFIED_SIDEBAR_HIDDEN = 'nounifiedsidebar',
...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
	...
	private readonly transientHiddenParts = new Set<Parts>();
	private readonly transientPartVisibility = new Map<Parts, boolean>();
	...
	private unifiedSidebarPartView!: ISerializableView;
	...
	private shouldRestoreEditors(contextService: IWorkspaceContextService, initialEditorsState: IInitialEditorsState | undefined): boolean {
		...
		// The Agent Workspace window is opened on the same workspace as the IDE it was
		// launched from, so it shares that workspace's storage and would restore the
		// IDE's saved editor grid. That drags ordinary file tabs into the shell's
		// locked chat group, which then has to shovel them into the utility pane. The
		// shell opens the session it was told to open and inherits nothing else.
		if (this.environmentService.isAgentWorkspaceWindow) {
			return false;
		}
	...
	isVisible(part: Parts, targetWindow: Window = mainWindow): boolean {
		...
		if (targetWindow === mainWindow && this.transientHiddenParts.has(part)) {
			return false;
		}
		...
		switch (part) {
			...
			case Parts.UNIFIED_SIDEBAR_PART:
				return !this.stateModel.getRuntimeValue(LayoutStateKeys.UNIFIED_SIDEBAR_HIDDEN);
			case Parts.PANEL_PART:
	...
	getMaximumEditorDimensions(container: HTMLElement): IDimension {
		...
		if (container === this.mainContainer) {
			...
			const takenWidth =
				...
				(this.isVisible(Parts.UNIFIED_SIDEBAR_PART) ? this.unifiedSidebarPartView.minimumWidth : 0) +
	...
	protected createWorkbenchLayout(): void {
		...
		const sideBar = this.getPart(Parts.SIDEBAR_PART);
		const unifiedSidebarPart = this.getPart(Parts.UNIFIED_SIDEBAR_PART) as UnifiedSidebarPart;
		...
		this.unifiedSidebarPartView = unifiedSidebarPart;
		...
		const viewMap: Record<string, ISerializableView> = {
			...
			[Parts.UNIFIED_SIDEBAR_PART]: this.unifiedSidebarPartView,
		...
		// Apply Cursor body/workbench visibility classes for the agents rail (grid visibility comes from descriptor).
		const unifiedSidebarHidden = this.stateModel.getRuntimeValue(LayoutStateKeys.UNIFIED_SIDEBAR_HIDDEN);
		this.mainContainer.classList.toggle('unifiedsidebarvisible', !unifiedSidebarHidden);
		this.mainContainer.classList.toggle('unifiedsidebarhidden', unifiedSidebarHidden);
		mainWindow.document.body.classList.toggle('unifiedsidebarvisible', !unifiedSidebarHidden);
		mainWindow.document.body.classList.toggle('unifiedsidebarhidden', unifiedSidebarHidden);

		for (const part of [titleBar, editorPart, activityBar, panelPart, sideBar, unifiedSidebarPart, statusBar, auxiliaryBarPart, bannerPart]) {
			this._register(part.onDidVisibilityChange(visible => {
				if (!this.inMaximizedAuxiliaryBarTransition) {
					...
					} else if (part === unifiedSidebarPart) {
						this.setUnifiedSidebarHidden(!visible);
		...
		this._register(this.storageService.onWillSaveState(() => {
			...
			// Unified Side Bar Size
			const unifiedSidebarSize = this.stateModel.getRuntimeValue(LayoutStateKeys.UNIFIED_SIDEBAR_HIDDEN)
				? this.workbenchGrid.getViewCachedVisibleSize(this.unifiedSidebarPartView)
				: this.workbenchGrid.getViewSize(this.unifiedSidebarPartView).width;
			this.stateModel.setInitializationValue(LayoutStateKeys.UNIFIED_SIDEBAR_SIZE, unifiedSidebarSize as number);
	...
	resizePart(part: Parts, sizeChangeWidth: number, sizeChangeHeight: number): void {
		...
		switch (part) {
			case Parts.SIDEBAR_PART:
				...
				break;
			case Parts.UNIFIED_SIDEBAR_PART:
				viewSize = this.workbenchGrid.getViewSize(this.unifiedSidebarPartView);
				this.workbenchGrid.resizeView(this.unifiedSidebarPartView, {
					width: viewSize.width + sizeChangePxWidth,
					height: viewSize.height
				});
	...
	getLayoutClasses(): string[] {
		return coalesce([
			...
			!this.isVisible(Parts.UNIFIED_SIDEBAR_PART) ? LayoutClasses.UNIFIED_SIDEBAR_HIDDEN : undefined,
	...
	private setUnifiedSidebarHidden(hidden: boolean): void {
		// Unlike the classic sidebar, the Agents unified rail is meant to coexist
		// with a maximized chat column (Cursor Flow). Do not un-maximize aux bar.
		this.stateModel.setRuntimeValue(LayoutStateKeys.UNIFIED_SIDEBAR_HIDDEN, hidden);

		// Adjust CSS
		if (hidden) {
			this.mainContainer.classList.add(LayoutClasses.UNIFIED_SIDEBAR_HIDDEN);
		} else {
			this.mainContainer.classList.remove(LayoutClasses.UNIFIED_SIDEBAR_HIDDEN);
		}

		// Cursor CSS parity: body + workbench visibility classes
		this.mainContainer.classList.toggle('unifiedsidebarvisible', !hidden);
		this.mainContainer.classList.toggle('unifiedsidebarhidden', hidden);
		mainWindow.document.body.classList.toggle('unifiedsidebarvisible', !hidden);
		mainWindow.document.body.classList.toggle('unifiedsidebarhidden', hidden);

		// Propagate to grid
		this.workbenchGrid.setViewVisible(this.unifiedSidebarPartView, !hidden);
	}

	/**
	 * Pin Agents flush against Chat. Critical: `adjustPartPositions` moves
	 * sidebar/aux but used to leave unifiedsidebar stranded (IDE "flop").
	 */
	alignUnifiedSidebarToChat(side: 'left' | 'right'): void {
		if (!this.workbenchGrid || !this.unifiedSidebarPartView || !this.auxiliaryBarPartView) {
			return;
		}
		if (!this.isVisible(Parts.UNIFIED_SIDEBAR_PART)) {
			return;
		}
		// Ensure chat column exists as the anchor.
		if (!this.isVisible(Parts.AUXILIARYBAR_PART)) {
			this.setPartHidden(false, Parts.AUXILIARYBAR_PART);
		}
		const size = this.workbenchGrid.getViewSize(this.unifiedSidebarPartView);
		const width = size.width > 0 ? size.width : this.stateModel.getInitializationValue(LayoutStateKeys.UNIFIED_SIDEBAR_SIZE);
		this.workbenchGrid.moveView(
			this.unifiedSidebarPartView,
			width,
			this.auxiliaryBarPartView,
			side === 'left' ? Direction.Left : Direction.Right,
		);
		this.workbenchGrid.resizeView(this.unifiedSidebarPartView, {
			height: this.workbenchGrid.getViewSize(this.unifiedSidebarPartView).height,
			width,
		});
	}
	...
	private adjustPartPositions(sideBarPosition: Position, panelAlignment: PanelAlignment, panelPosition: Position): void {
		...
		// Keep Agents glued to Chat after any sidebar/aux reshuffle.
		if (this.isVisible(Parts.UNIFIED_SIDEBAR_PART)) {
			// Sidebar LEFT (IDE) => Agents on the RIGHT of Chat.
			// Sidebar RIGHT (V3 chat-left) => Agents on the LEFT of Chat.
			this.alignUnifiedSidebarToChat(sideBarPosition === Position.LEFT ? 'right' : 'left');
		}
	}
	...
	setAuxiliaryBarMaximized(maximized: boolean, options?: IAuxiliaryBarMaximizeOptions): boolean {
		if (
		...
		if (maximized) {
			const state = {
				...
				unifiedSidebarVisible: this.isVisible(Parts.UNIFIED_SIDEBAR_PART)
			};
			...
			try {
				...
				if (state.sideBarVisible && !options?.skipHideSidebar) {
					this.setSideBarHidden(true);
				}
				if (state.unifiedSidebarVisible && !options?.skipHideUnifiedSidebar) {
					this.setUnifiedSidebarHidden(true);
				}
				if (state.panelVisible && !options?.skipHidePanel) {
				...
				// Persist whether the panel was intentionally left alone so the
				// restore half below does not "put it back" and pop the terminal.
				this.stateModel.setRuntimeValue(LayoutStateKeys.AUXILIARYBAR_LAST_NON_MAXIMIZED_VISIBILITY, { ...state, skipPanel: !!options?.skipHidePanel });
			} finally {
			...
			try {
				...
				}													// (skipped: panel was never Flow's to touch)
				this.setSideBarHidden(!state?.sideBarVisible);	// sizes we had
				this.setUnifiedSidebarHidden(!state?.unifiedSidebarVisible);
	...
	/**
	 * True when hiding `part` would leave no workbench part visible except the
	 * status bar. We refuse those hides: an all-but-status-bar grid is invalid,
	 * and core layout "repairs" it by reopening a part (which is how the terminal
	 * panel came back full-height in the wrong place). Keeping the grid valid means
	 * nothing reopens on its own. Mirrors Cursor's shipped `setPartHidden` guard.
	 *
	 * Zen mode (setPanelHidden/setAuxiliaryBarHidden/setSideBarHidden directly) and
	 * the auxiliary-bar maximize choreography both use the PRIVATE setters, so they
	 * bypass this public entry point and are unaffected.
	 */
	private wouldLeaveOnlyStatusBarVisible(part: Parts): boolean {
		const considered = [
			Parts.EDITOR_PART,
			Parts.PANEL_PART,
			Parts.SIDEBAR_PART,
			Parts.AUXILIARYBAR_PART,
			Parts.UNIFIED_SIDEBAR_PART,
		];
		for (const candidate of considered) {
			if (candidate === part) { continue; }
			if (this.isVisible(candidate, mainWindow)) { return false; }
		}
		return true;
	}

	setPartHidden(hidden: boolean, part: Parts): void {
		if (hidden && this.wouldLeaveOnlyStatusBarVisible(part)) {
			return; // refuse: would collapse the grid to only the status bar
		}
		switch (part) {
			case Parts.ACTIVITYBAR_PART:
			...
			case Parts.UNIFIED_SIDEBAR_PART:
				return this.setUnifiedSidebarHidden(hidden);
			case Parts.EDITOR_PART:
	...
	setPartHiddenTransient(hidden: boolean, part: Parts): void {
		let visible: boolean;
		if (hidden) {
			// Remember the live layout before the Agent Workspace shell replaces it.
			// The Agent companion is retired rather than converted back into an IDE;
			// keeping this state local still makes the transient API balanced without
			// writing any part visibility into shared workspace storage.
			if (!this.transientPartVisibility.has(part)) {
				this.transientPartVisibility.set(part, this.isVisible(part, mainWindow));
			}
			this.transientHiddenParts.add(part);
			visible = false;
		} else {
			this.transientHiddenParts.delete(part);
			visible = this.transientPartVisibility.get(part) ?? this.isVisible(part, mainWindow);
			this.transientPartVisibility.delete(part);
		}

		// This deliberately updates only the current window's grid and layout
		// classes. Calling the ordinary setters here would write workspace-scoped
		// state and make the next fresh IDE window inherit this shell.
		switch (part) {
			case Parts.TITLEBAR_PART:
				this.workbenchGrid.setViewVisible(this.titleBarPartView, visible);
				break;
			case Parts.BANNER_PART:
				this.workbenchGrid.setViewVisible(this.bannerPartView, visible);
				break;
			case Parts.ACTIVITYBAR_PART:
				this.workbenchGrid.setViewVisible(this.activityBarPartView, visible);
				break;
			case Parts.SIDEBAR_PART:
				this.mainContainer.classList.toggle(LayoutClasses.SIDEBAR_HIDDEN, !visible);
				this.workbenchGrid.setViewVisible(this.sideBarPartView, visible);
				break;
			case Parts.UNIFIED_SIDEBAR_PART:
				this.mainContainer.classList.toggle(LayoutClasses.UNIFIED_SIDEBAR_HIDDEN, !visible);
				this.mainContainer.classList.toggle('unifiedsidebarvisible', visible);
				this.mainContainer.classList.toggle('unifiedsidebarhidden', !visible);
				mainWindow.document.body.classList.toggle('unifiedsidebarvisible', visible);
				mainWindow.document.body.classList.toggle('unifiedsidebarhidden', !visible);
				this.workbenchGrid.setViewVisible(this.unifiedSidebarPartView, visible);
				break;
			case Parts.PANEL_PART:
				this.mainContainer.classList.toggle(LayoutClasses.PANEL_HIDDEN, !visible);
				this.workbenchGrid.setViewVisible(this.panelPartView, visible);
				break;
			case Parts.AUXILIARYBAR_PART:
				this.mainContainer.classList.toggle(LayoutClasses.AUXILIARYBAR_HIDDEN, !visible);
				this.workbenchGrid.setViewVisible(this.auxiliaryBarPartView, visible);
				break;
			case Parts.STATUSBAR_PART:
				this.mainContainer.classList.toggle(LayoutClasses.STATUSBAR_HIDDEN, !visible);
				this.workbenchGrid.setViewVisible(this.statusBarPartView, visible);
				break;
			case Parts.EDITOR_PART:
				this.mainContainer.classList.toggle(LayoutClasses.MAIN_EDITOR_AREA_HIDDEN, !visible);
				this.workbenchGrid.setViewVisible(this.editorPartView, visible);
				break;
		}
	}
	...
	getVisibleNeighborPart(part: Parts, direction: Direction): Parts | undefined {
		...
		for (const neighborView of neighborViews) {
			const neighborPart =
				[Parts.ACTIVITYBAR_PART, Parts.EDITOR_PART, Parts.PANEL_PART, Parts.AUXILIARYBAR_PART, Parts.SIDEBAR_PART, Parts.UNIFIED_SIDEBAR_PART, Parts.STATUSBAR_PART, Parts.TITLEBAR_PART]
	...
	private createGridDescriptor(): ISerializedGrid {
		...
		const unifiedSidebarSize = this.stateModel.getInitializationValue(LayoutStateKeys.UNIFIED_SIDEBAR_SIZE);
		...
		const unifiedSidebarNode: ISerializedLeafNode = {
			type: 'leaf',
			data: { type: Parts.UNIFIED_SIDEBAR_PART },
			size: unifiedSidebarSize,
			visible: !unifiedSidebarHidden
		};
		...
		// Cursor rule: agents rail opposite the classic sidebar (LEFT sidebar → append right; else splice left).
		if (!unifiedSidebarHidden) {
			const visibleUnifiedSize = unifiedSidebarSize;
			for (const node of middleSection) {
				if (node.type === 'leaf' && (node.data as { type: Parts }).type === Parts.EDITOR_PART) {
					node.size = Math.max(0, node.size - visibleUnifiedSize);
					break;
				}
				if (node.type === 'branch') {
					const hasEditor = (n: ISerializedNode): boolean =>
						n.type === 'leaf'
							? (n.data as { type: Parts }).type === Parts.EDITOR_PART
							: n.data.some(hasEditor);
					if (hasEditor(node)) {
						node.size = Math.max(0, node.size - visibleUnifiedSize);
						break;
					}
		...
		if (this.stateModel.getRuntimeValue(LayoutStateKeys.SIDEBAR_POSITON) === Position.LEFT) {
			middleSection.push(unifiedSidebarNode);
		} else {
			middleSection.splice(0, 0, unifiedSidebarNode);
		}
...
const LayoutStateKeys = {
	...
	UNIFIED_SIDEBAR_SIZE: new InitializationStateKey<number>('unifiedSideBar.size', StorageScope.PROFILE, StorageTarget.MACHINE, 300),
	...
	AUXILIARYBAR_LAST_NON_MAXIMIZED_VISIBILITY: new RuntimeStateKey('auxiliaryBar.lastNonMaximizedVisibility', StorageScope.WORKSPACE, StorageTarget.MACHINE, {
		...
		unifiedSidebarVisible: false,
		// True when the panel was intentionally excluded from this maximize
		// transaction (agent Flow), so restore must not re-show it.
		skipPanel: false
	}),
	...
	UNIFIED_SIDEBAR_HIDDEN: new RuntimeStateKey<boolean>('unifiedSideBar.hidden', StorageScope.WORKSPACE, StorageTarget.MACHINE, true),
...
class LayoutStateModel extends Disposable {
	...
	private applyAuxiliaryBarMaximizedOverride(): void {
		this.setRuntimeValue(LayoutStateKeys.AUXILIARYBAR_LAST_NON_MAXIMIZED_VISIBILITY, {
			...
			unifiedSidebarVisible: !this.getRuntimeValue(LayoutStateKeys.UNIFIED_SIDEBAR_HIDDEN),
			skipPanel: false
		});

		this.setRuntimeValue(LayoutStateKeys.SIDEBAR_HIDDEN, true);
		this.setRuntimeValue(LayoutStateKeys.UNIFIED_SIDEBAR_HIDDEN, true);
