...
export class ActivitybarPart extends Part {
	...
	// V3Code: a slimmer activity rail. Width matches compositeSize below so
	// the icon buttons are not wider than the bar (which clipped/"cut" them).
	static readonly ACTIVITYBAR_WIDTH = 42;
	static readonly COMPACT_ACTIVITYBAR_WIDTH = 36;

	static readonly ICON_SIZE = 20;
	...
	private createCompositeBar(): PaneCompositeBar {
		...
		return this.instantiationService.createInstance(ActivityBarCompositeBar, this.location, {
			...
			// V3Code: accounts + settings are hosted in the title bar top-right
			// instead (see titlebarPart.activityActionsEnabled), so the activity bar does NOT
			// render its own bottom global composite — passing false here avoids a duplicate.
	...
	override layout(width: number, height: number): void {
		// See Part.applyLayoutGap. The activity bar is in the gap layer's selector list too.
		({ width, height } = this.applyLayoutGap(width, height, 0, 0));
