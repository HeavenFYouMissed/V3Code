...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	// The aux bar hosts the chat by default. 170 (the stock side-bar floor) let the panel be
	// squeezed until the composer and picker rows were unusable; 300 matches the preferredWidth
	// floor below so a sash drag can't go below what the chat layout is designed for.
	...
	constructor(
		...
		super(
			...
			{
				// Chat column has no composite title strip (Cursor-faithful).
				// hasTitle:false reclaims the 35px PartLayout title reservation so the
				// composer sits at the card bottom instead of floating above a dead band.
				// Safe while shouldShowCompositeBar() is false — enabling a TITLE-position
				// composite bar again would require hasTitle:true.
				hasTitle: false,
	...
	protected shouldShowCompositeBar(): boolean {
		// Cursor-faithful Chat column: never show the Chat|MCP composite strip.
		// MCP / Memory live on the primary activity bar. Title chrome is CSS-hidden;
		// chat tabs (+ / history / …) live inside ChatViewTabsControl.
		return false;
	}
