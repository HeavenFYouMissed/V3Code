...
export class SideBySideEditor extends AbstractEditorWithViewState<ISideBySideEditorViewState> implements IEditorPaneWithSelection {
	...
	override async setInput(input: SideBySideEditorInput, options: ISideBySideEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<void> {
		const oldInput = this.input instanceof SideBySideEditorInput ? this.input : undefined;
		await super.setInput(input, options, context, token);

		// Create new side by side editors if either we have not
		// been created before or either side needs a different editor pane.
		// Different resources commonly share the same pane type; retaining those
		// panes lets each side update its input without rebuilding the whole split.
		if (!oldInput || (!input.matches(oldInput) && !this.canReuseEditors(input, oldInput))) {
			if (oldInput) {
				this.disposeEditors();
			}
	...
	private canReuseEditors(input: SideBySideEditorInput, oldInput: SideBySideEditorInput): boolean {
		const registry = Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane);
		const secondaryDescriptor = registry.getEditorPane(input.secondary);
		const oldSecondaryDescriptor = registry.getEditorPane(oldInput.secondary);
		const primaryDescriptor = registry.getEditorPane(input.primary);
		const oldPrimaryDescriptor = registry.getEditorPane(oldInput.primary);

		return !!secondaryDescriptor
			&& secondaryDescriptor.typeId === oldSecondaryDescriptor?.typeId
			&& !!primaryDescriptor
			&& primaryDescriptor.typeId === oldPrimaryDescriptor?.typeId;
	}
