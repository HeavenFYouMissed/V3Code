...
export class EditorParts extends MultiWindowParts<EditorPart, IEditorPartsMemento> implements IEditorGroupsService, IEditorPartsView {
	...
	protected override saveState(): void {
		...
		// The Agent Workspace shell owns the editor parts of its own window. Leave the
		// workspace-scoped part layout exactly as the IDE last wrote it, for the same
		// reason the editor grid itself is not persisted from this window.
	...
	private createState(): IEditorPartsUIState {
		return {
			auxiliary: this.parts
				...
				// Agent Workspace used an auxiliary editor window during its early
				// prototype. It is now a normal CodeWindow and must never leak that
				// legacy window back into the next IDE startup.
				.filter(state => !this.isLegacyAgentWorkspaceAuxiliaryState(state)),
			mru: this.mostRecentActiveParts.map(part => this.parts.indexOf(part))
		};
	}

	private isLegacyAgentWorkspaceAuxiliaryState(state: IAuxiliaryEditorPartState): boolean {
		if (state.agentWorkspace === true) {
			return true;
		}

		return this.serializedNodeContainsAgentWorkspaceChat(state.state.serializedGrid.root);
	}

	private serializedNodeContainsAgentWorkspaceChat(node: ISerializedNode): boolean {
		if (node.type === 'branch') {
			return node.data.some(child => this.serializedNodeContainsAgentWorkspaceChat(child));
		}

		if (!isSerializedEditorGroupModel(node.data)) {
			return false;
		}

		return node.data.editors.some(editor => {
			if (editor.id !== 'workbench.input.chatSession') {
				return false;
			}

			try {
				const serializedChatEditor = JSON.parse(editor.value) as { readonly options?: { readonly agentWorkspace?: boolean } };
				return serializedChatEditor.options?.agentWorkspace === true;
			} catch {
				return false;
			}
		});
	...
	private async restoreState(state: IEditorPartsUIState): Promise<void> {
		if (state.auxiliary.length) {
			...
			for (const auxiliaryEditorPartState of state.auxiliary) {
				// Do not restore Agent Workspace auxiliary windows written by older
				// builds. The current implementation swaps between top-level
				// CodeWindows and opens the chat in the main editor part.
				if (this.isLegacyAgentWorkspaceAuxiliaryState(auxiliaryEditorPartState)) {
					continue;
				}
