...
export class AuxiliaryEditorPart {
	...
	async create(label: string, options?: IAuxiliaryEditorPartOpenOptions): Promise<ICreateAuxiliaryEditorPartResult> {
		...
		const agentWorkspace = Boolean(options?.agentWorkspace);
		...
		function updateCompact(newCompact: boolean): void {
			...
			compact = newCompact;
			auxiliaryWindow.updateOptions({ compact, agentWorkspace });
			titlebarPart?.updateOptions({ compact });
			editorPart.updateOptions({ compact, agentWorkspace });

			const oldStatusbarVisible = statusbarVisible;
			statusbarVisible = !agentWorkspace && !compact && that.configurationService.getValue<boolean>(AuxiliaryEditorPart.STATUS_BAR_VISIBILITY) !== false;
		...
		const useCustomTitle = isNative && hasCustomTitlebar(this.configurationService) && !agentWorkspace; // custom title in aux windows only enabled in native
		if (useCustomTitle) {
		...
		let statusbarVisible = !agentWorkspace && !compact && this.configurationService.getValue<boolean>(AuxiliaryEditorPart.STATUS_BAR_VISIBILITY) !== false;
		disposables.add(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(AuxiliaryEditorPart.STATUS_BAR_VISIBILITY)) {
				statusbarVisible = !agentWorkspace && !compact && this.configurationService.getValue<boolean>(AuxiliaryEditorPart.STATUS_BAR_VISIBILITY) !== false;
...
class AuxiliaryEditorPartImpl extends EditorPart implements IAuxiliaryEditorPart {
	...
	private isAgentWorkspace = false;
	...
	constructor(
		...
		super(editorPartsView, `workbench.parts.auxiliaryEditor.${id}`, groupsLabel, windowId, instantiationService, themeService, configurationService, storageService, layoutService, hostService, contextKeyService, environmentService);
	}
	...
	updateOptions(options: { compact: boolean; agentWorkspace?: boolean }): void {
		this.isCompact = options.compact;
		this.isAgentWorkspace = options.agentWorkspace ?? this.isAgentWorkspace;
		...
		} else if (this.isAgentWorkspace) {
			// Every Agent surface uses the native single-row tab strip. The chat
			// editor is sticky because the session rail replaces close-and-reopen
			// navigation; Browser, Terminal and Files remain ordinary closeable tabs.
			this.optionsDisposable.value = this.enforcePartOptions({
				showTabs: 'multiple',
				wrapTabs: false,
				tabActionUnpinVisibility: false,
				closeEmptyGroups: true
			});
		} else {
	...
	override removeGroup(group: number | IEditorGroupView, preserveFocus?: boolean): void {
		...
		// The locked Chat group is the structural spine of the Agent workspace.
		// Keep the empty group alive long enough for its shell to restore the
		// session instead of allowing a utility group to consume the window.
		if (this.isAgentWorkspace && groupView.isLocked) {
			return;
		}
