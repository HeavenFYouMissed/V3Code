...
import { EditorResourceAccessor, Verbosity, IEditorPartOptions, SideBySideEditor, DEFAULT_EDITOR_ASSOCIATION, EditorInputCapabilities, IUntypedEditorInput, preventEditorClose, EditorCloseMethod, EditorsOrder, IToolbarActions, IEditorCommandsContext } from '../../../common/editor.js';
...
import { ActionBar, ActionsOrientation, prepareActions } from '../../../../base/browser/ui/actionbar/actionbar.js';
...
export class MultiEditorTabsControl extends EditorTabsControl {
	...
	protected override create(parent: HTMLElement): HTMLElement {
		...
		// Sessions workplace: Cursor-style leading escape (Browser / +) BEFORE tabs.
		if (this.isPermanentSessionsRail) {
			this.createEditorTitleStartToolBar(this.tabsAndActionsContainer);
		}
		...
		if (this.isPermanentSessionsRail) {
			// Empty groups normally wait for the first editor before
			// populating MenuId.EditorTitle. Sessions uses that menu as its
			// permanent workspace launcher, so populate it immediately.
	...
	private titleStartActionsContainer: HTMLElement | undefined;
	private titleStartActionsToolbar: WorkbenchToolBar | undefined;
	// Created lazily in createEditorTitleStartToolBar — field initializers have
	// not run yet when EditorTabsControl's constructor calls create().
	private titleStartActionsDisposables: DisposableStore | undefined;
	private titleStartActionsToolbarDisposables: DisposableStore | undefined;

	private createEditorTitleStartToolBar(parent: HTMLElement): void {
		this.titleStartActionsToolbarDisposables ??= this._register(new DisposableStore());
		this.titleStartActionsDisposables ??= this._register(new DisposableStore());

		this.titleStartActionsContainer = $('.editor-title-start-actions');
		const scrollbarNode = assertReturnsDefined(this.tabsScrollbar).getDomNode();
		parent.insertBefore(this.titleStartActionsContainer, scrollbarNode);

		const context: IEditorCommandsContext = { groupId: this.groupView.id };
		this.titleStartActionsToolbar = this.titleStartActionsToolbarDisposables.add(this.instantiationService.createInstance(WorkbenchToolBar, this.titleStartActionsContainer, {
			actionViewItemProvider: (action, options) => createActionViewItem(this.instantiationService, action, { ...options, menuAsChild: false }),
			orientation: ActionsOrientation.HORIZONTAL,
			ariaLabel: localize('ariaLabelWorkplaceLeading', "Workplace leading actions"),
			getKeyBinding: action => this.getKeybinding(action),
			actionRunner: this.titleStartActionsToolbarDisposables.add(new EditorCommandsContextActionRunner(context)),
			anchorAlignmentProvider: () => AnchorAlignment.LEFT,
			renderDropdownAsChildElement: false,
			telemetrySource: 'editorPartLeading',
			resetMenu: MenuId.EditorTitleStart,
			highlightToggledItems: true,
		}));
		this.titleStartActionsToolbar.context = context;
	}

	private updateEditorTitleStartToolbar(): void {
		if (!this.titleStartActionsToolbar || !this.titleStartActionsDisposables) {
			return;
		}

		this.titleStartActionsDisposables.clear();
		const editorActions = this.groupView.createEditorActions(this.titleStartActionsDisposables, MenuId.EditorTitleStart);
		this.titleStartActionsDisposables.add(editorActions.onDidChange(() => this.updateEditorTitleStartToolbar()));
		const { primary, secondary } = editorActions.actions;
		this.titleStartActionsToolbar.setActions(prepareActions(primary), prepareActions(secondary));
	}
	...
	protected override updateEditorActionsToolbar(): void {
		...
		this.updateEditorTitleStartToolbar();
		...
		// During construction of an empty Sessions group, this runs before the first
		// dimensions arrive; the first normal layout will size the populated toolbar.
		if (this.dimensions) {
			this.layout(this.dimensions);
		}
	}
	...
	private doLayoutTabsWrapping(dimensions: IEditorTitleControlDimensions): boolean {
		...
		// The Sessions workspace rail is a single permanent row: tabs scroll
		// left, mode pills and "+" stay pinned right. wrapTabs would invent a
		// second tab structure (multi-line + last-tab-margin) that climbs into
		// the Files/Changes banner below the rail and fights it.
		if (this.isPermanentSessionsRail) {
			if (didTabsWrapMultiLine) {
	...
	private get visible(): boolean {
		// The Sessions window uses the native editor title as its permanent
		// workspace tab rail. Keep it alive before the first editor opens so
		// its mode actions and "+" launcher remain available. Multi-row editor
		// groups instantiate both sticky and regular controls; only the regular
		// control owns the empty rail.
		return this.tabsModel.count > 0 || this.isPermanentSessionsRail;
	}

	private get isPermanentSessionsRail(): boolean {
		return IsSessionsWindowContext.getValue(this.contextKeyService) === true
