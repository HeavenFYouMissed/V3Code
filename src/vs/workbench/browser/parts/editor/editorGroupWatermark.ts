...
import { V3CodeStartScreen } from './v3codeStartScreen.js';
...
// Inside a project the watermark is a keyboard cheat sheet, so it should advertise
// what THIS product does rather than stock editor commands. Anything without a
// keybinding, or not contributed by a given build, is dropped by `filterEntries`.
const newAgent: WatermarkEntry = { text: localize('watermark.newAgent', "New Agent"), id: 'workbench.action.openAgentsWindow' };
const connectMobile: WatermarkEntry = { text: localize('watermark.connectMobile', "Connect to Mobile"), id: 'v3code.remote.showQr' };
...
export class EditorGroupWatermark extends Disposable {
	...
	private readonly startScreenHost: HTMLElement;
	private readonly startScreen: V3CodeStartScreen;
	...
	constructor(
		...
		this.startScreenHost = elements.startScreen;

		this.startScreen = this._register(instantiationService.createInstance(V3CodeStartScreen, this.startScreenHost));
	...
	private render(): void {
		...
		// A window with no project is the one moment the product has to explain itself:
		// a list of key combinations assumes you already know you need a folder open.
		// Once a project IS open the shortcut list is the right thing again - by then
		// you know what you are looking at and only want the keys - so it stays, just
		// pointed at this product's own features rather than stock ones.
		const showStartScreen = this.workbenchState === WorkbenchState.EMPTY && this.startScreen.hasContent();
		this.startScreenHost.classList.toggle('is-visible', showStartScreen);
		if (showStartScreen) {
			void this.startScreen.render();
			return;
		}
		clearNode(this.startScreenHost);
