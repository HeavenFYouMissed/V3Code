...
import { Part, V3_PART_LAYOUT_GAP } from '../../part.js';
...
export class EditorPart extends Part<IEditorPartMemento> implements IEditorPart, IEditorGroupsView {
	...
	get minimumWidth(): number { return Math.min(this.centeredLayoutWidget.minimumWidth, this.layoutService.getMaximumEditorDimensions(this.layoutService.getContainer(getWindow(this.container))).width) + V3_PART_LAYOUT_GAP * 2; }
	get maximumWidth(): number { return this.centeredLayoutWidget.maximumWidth; }
	get minimumHeight(): number { return Math.min(this.centeredLayoutWidget.minimumHeight, this.layoutService.getMaximumEditorDimensions(this.layoutService.getContainer(getWindow(this.container))).height) + V3_PART_LAYOUT_GAP * 2; }
	...
	protected override get layoutGap(): number { return V3_PART_LAYOUT_GAP; }

	override layout(width: number, height: number, top: number, left: number): void {
		// The grid gives us the CELL; the floating-card CSS makes our element 2*gap smaller and
		// offsets it. Convert before anything downstream sizes itself, or every descendant is
		// laid out past the card's right and bottom edges.
		({ width, height, top, left } = this.applyLayoutGap(width, height, top, left));
	...
	protected override saveState(): void {
		...
		// The Agent Workspace shell reuses this window's editor grid to host its own
		// chat and utility surfaces. Those editors and that split belong to the shell,
		// not to the workspace, so this window must neither write nor clear the shared
		// editor layout: writing it makes the next ordinary IDE window restore agent
		// chats, Files, Terminal and Browser as normal editor tabs, and clearing it
		// would discard the layout the IDE legitimately saved.
...
export class MainEditorPart extends EditorPart {
	...
	constructor(
		...
		super(editorPartsView, Parts.EDITOR_PART, '', mainWindow.vscodeWindowId, instantiationService, themeService, configurationService, storageService, layoutService, hostService, contextKeyService, environmentService);
	}
}
