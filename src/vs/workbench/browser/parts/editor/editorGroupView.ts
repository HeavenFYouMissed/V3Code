...
export class EditorGroupView extends Themable implements IEditorGroupView {
	...
	createEditorActions(disposables: DisposableStore, menuId = MenuId.EditorTitle): IActiveEditorActions {
		...
		} else if (IsSessionsWindowContext.getValue(this.scopedContextKeyService) === true) {
			// The Sessions window uses the editor title as its permanent
			// workspace tab rail: its mode controls (Terminal, Changes,
			// Files, the "+" launcher) must survive an empty group.
			const editorTitleMenu = disposables.add(this.menuService.createMenu(menuId, this.scopedContextKeyService, { emitEventsForSubmenuChanges: true, eventDebounceDelay: 0 }));
			onDidChange = editorTitleMenu.onDidChange;
