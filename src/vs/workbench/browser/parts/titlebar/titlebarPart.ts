...
export class BrowserTitlebarPart extends Part implements ITitlebarPart {
	...
	private get activityActionsEnabled(): boolean {
		// V3Code: always host the accounts + settings (global activity) actions
		// in the title bar's top-right, regardless of activity bar position. The activity
		// bar's own bottom global composite is disabled (see ActivitybarPart) so they don't
		// duplicate. This keeps the side activity bar AND puts profile/settings up top.
