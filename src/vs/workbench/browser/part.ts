 ...
 */
/**
 * Gap between a part's floating card and its grid cell, in CSS px.
 *
 * Mirrors V3_GAP / V3_LAYOUT_GAP in v3codeGreyChromeContribution.ts. If that CSS is ever
 * removed, this must go to 0 in the same change or every part shrinks twice.
 */
export const V3_PART_LAYOUT_GAP = 5;
...
export abstract class Part<MementoType extends object = object> extends Component<MementoType> implements ISerializableView {
	...
	/**
	 * Gap between this part's card and its grid cell, in CSS px.
	 *
	 * MUST stay equal to V3_LAYOUT_GAP in v3codeGreyChromeContribution.ts. The floating-card
	 * chrome shrinks a part with `margin: Npx` plus `width/height: calc(100% - 2Npx)`, so the
	 * dimension the grid hands us describes the CELL, not the card we actually occupy. Parts
	 * the gap layer does not touch (titlebar, statusbar, banner) leave this at 0.
	 */
	protected get layoutGap(): number { return 0; }

	/**
	 * Convert a grid-cell rect into the rect this part's card really occupies.
	 *
	 * Without this, PartLayout writes inline pixels onto `.content` from the cell size while
	 * the card around it is 2*gap smaller. The origin is unchanged by `margin`, so the excess
	 * lands entirely on the right and bottom: DOM children get silently clipped there, and a
	 * WebContentsView — which no CSS can clip — renders straight over the sash.
	 */
	protected applyLayoutGap(width: number, height: number, top: number, left: number): { width: number; height: number; top: number; left: number } {
