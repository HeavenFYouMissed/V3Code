...
export const enum Parts {
	...
	/**
	 * Cursor-style dedicated Agents / sessions rail. Independent from
	 * `SIDEBAR_PART` (Explorer / Search / SCM). Part id matches Cursor's
	 * `workbench.parts.unifiedsidebar` for layout/CSS parity.
	 */
	UNIFIED_SIDEBAR_PART = 'workbench.parts.unifiedsidebar',
...
export interface IAuxiliaryBarMaximizeOptions {
	/**
	 * Keep the primary sidebar visible while the auxiliary bar occupies the
	 * remaining workbench area.
	 */
	readonly skipHideSidebar?: boolean;
	/**
	 * Keep the dedicated Agents unified sidebar visible while chat maximizes
	 * (Cursor agent-mode Flow).
	 */
	readonly skipHideUnifiedSidebar?: boolean;
	/**
	 * Do NOT hide the bottom panel (terminal) when chat maximizes, and do NOT
	 * re-show it when chat un-maximizes. The panel is not part of the agent
	 * surface: hiding it here made the maximize snapshot remember it and replay
	 * it full-height on restore, which is the terminal-pop deformation.
	 */
	readonly skipHidePanel?: boolean;
}
...
export interface IWorkbenchLayoutService extends ILayoutService {
	...
	/**
	 * Set a part's visibility for the lifetime of the current workbench window
	 * without writing the change to workspace layout storage.
	 *
	 * This is intended for purpose-built window shells, such as Agent Workspace,
	 * that reuse the editor grid but must not alter the layout inherited by a
	 * fresh IDE window.
	 */
	setPartHiddenTransient?(hidden: boolean, part: Parts): void;

	/**
	 * Place the Agents unified sidebar flush against Chat (auxiliary bar).
	 * `'left'` = Agents | Chat (V3 Flow). `'right'` = Chat | Agents (IDE).
	 */
