...
// V3Code ships its own SVG file-icon theme (extensions/theme-v3code) instead of
// upstream's font-glyph Seti theme. Seti renders every icon as a single flat
// glyph from seti.woff, which cannot carry the multi-colour shapes that make
// per-language icons legible at 16px. ID format is `{publisher}.{name}-{iconThemeId}`.
const DEFAULT_FILE_ICON_THEME_ID = 'v3code.theme-v3code-v3code-minimal';
