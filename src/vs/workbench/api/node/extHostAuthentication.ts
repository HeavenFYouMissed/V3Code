...
/**
 * V3Code: at most one loopback flow binds the registered auth port at a time, across
 * every provider instance. Two concurrent sign-ins used to race for the port; the
 * loser silently fell back to a random port that strict providers reject with
 * invalid_redirect_uri (an error only the browser ever sees).
 */
let loopbackFlowQueue: Promise<unknown> = Promise.resolve();
...
export class NodeDynamicAuthProvider extends DynamicAuthProvider {
	...
	private async _createWithLoopbackServer(scopes: string[], progress: vscode.Progress<IProgressStep>, token: vscode.CancellationToken): Promise<IAuthorizationTokenResponse> {
		// Serialize on the module-level queue; a flow cancelled while waiting never binds.
		const queued = loopbackFlowQueue.then(() => {
			if (token.isCancellationRequested) {
				throw new CancellationError();
			}
			return this._doCreateWithLoopbackServer(scopes, progress, token);
		});
		loopbackFlowQueue = queued.catch(() => { });
		return raceCancellationError(queued, token);
	}

	private async _doCreateWithLoopbackServer(scopes: string[], progress: vscode.Progress<IProgressStep>, token: vscode.CancellationToken): Promise<IAuthorizationTokenResponse> {
		...
		if (server.usedFallbackPort) {
			// The registered redirect URI could not be bound; strict providers will refuse
			// this attempt in the browser with no signal back to the editor. Tell the user
			// before they wait on a flow that cannot complete.
			progress.report({
				message: nls.localize('fallbackPort', "The usual sign-in port is busy, so this attempt may be refused by the provider. If sign-in fails, wait a few seconds and try again."),
			});
		}
		...
		try {
			...
			// Wait for the authorization code via the loopback server — bounded: an
			// abandoned browser tab must not hold the flow queue for the whole session.
			let code: string | undefined;
			try {
				const response = await raceCancellationError(raceTimeout(promise, 5 * 60 * 1000), token);
				if (!response) {
					throw new Error('Timed out waiting for the browser sign-in to complete. Try again.');
				}
				code = response.code;
			} catch (err) {
			...
			// V3Code: outcome-aware cleanup. The old unconditional 5s delayed stop meant a
			// CANCELLED flow kept the registered port busy — and the user's immediate retry
			// (the natural double-click) silently landed on a random fallback port that
			// strict providers reject. Only a successful flow keeps serving briefly, so the
			// browser's "you're signed in" landing page can render.
			if (succeeded) {
				// Shorter than the next flow's bind-retry budget (6 x 500ms), so a queued
				// sign-in that starts immediately after this one still wins the port.
				setTimeout(() => {
					void server.stop();
				}, 1500);
			} else {
