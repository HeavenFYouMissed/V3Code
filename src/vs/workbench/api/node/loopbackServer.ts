...
export class LoopbackAuthServer implements ILoopbackServer {
	...
	private _usedFallbackPort = false;

	constructor(
		private readonly _logger: ILogger,
		private readonly _appUri: URI,
		private readonly _appName: string,
		// V3Code: injectable so tests never bind the real auth port.
		private readonly _preferredPort: number = DEFAULT_AUTH_FLOW_PORT
	) {
	...
	/**
	 * V3Code: true when the preferred port stayed busy and an OS-assigned port was bound
	 * instead. Strict providers only accept their registered redirect URI, so a flow on a
	 * fallback port can fail with invalid_redirect_uri — visible only in the browser.
	 */
	get usedFallbackPort(): boolean { return this._usedFallbackPort; }
	...
	async start(): Promise<void> {
		...
		// V3Code: strict providers (Stripe-class) accept only their registered redirect URI
		// http://127.0.0.1:33418/. A silent random-port fallback made them reject the
		// sign-in with invalid_redirect_uri — an error that renders in the BROWSER, so the
		// editor only ever saw a timeout. The port is usually held briefly by a flow that
		// was just cancelled; retry it before falling back, and mark the fallback loudly.
		let retriesLeft = 6;
		server.on('error', err => {
			if ('code' in err && err.code === 'EADDRINUSE') {
				if (retriesLeft > 0) {
					retriesLeft--;
					this._logger.warn(`Auth callback port ${this._preferredPort} is busy (another sign-in just finished or is in progress); retrying...`);
					setTimeout(() => server.listen(this._preferredPort, '127.0.0.1'), 500);
				} else {
					this._logger.error(`Auth callback port ${this._preferredPort} stayed busy; falling back to a random port. Providers that require an exact registered redirect URI will refuse this sign-in — close other sign-in attempts and try again.`);
					this._usedFallbackPort = true;
		...
		server.listen(this._preferredPort, '127.0.0.1');
