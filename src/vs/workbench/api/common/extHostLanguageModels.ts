...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
	...
	/** Vendors registered only on the workbench (e.g. v3code) — not in this extension host. */
	private _isWorkbenchNativeVendor(vendor: string | undefined): boolean {
		return !!vendor && !this._languageModelProviders.has(vendor);
	}
	...
	async getLanguageModelByIdentifier(extension: IExtensionDescription, modelId: string | undefined): Promise<vscode.LanguageModelChat | undefined> {
		...
		if (!model) {
			...
			// Workbench-native vendors (v3code BYOK tiers) are not mirrored into the
			// extension host — don't retry or warn; copilot's LM proxy cannot wrap them.
			if (this._isWorkbenchNativeVendor(vendor)) {
				return undefined;
			}
	...
	async selectLanguageModels(extension: IExtensionDescription, selector: vscode.LanguageModelChatSelector) {
		...
		const modelPromises = models
			.filter(identifier => !this._isWorkbenchNativeVendor(this.getVendorFromModelIdentifier(identifier)))
			.map(identifier => this.getLanguageModelByIdentifier(extension, identifier));
