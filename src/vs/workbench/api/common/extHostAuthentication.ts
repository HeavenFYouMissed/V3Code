...
/**
 * V3Code: whether this build's URL scheme is one the hosted vscode.dev redirect pages
 * can bounce back to. A fork's scheme never receives them, so registering or using
 * those redirects only produces sign-ins that cannot return to the app — and a fork
 * must not register under another product's client_uri while doing it.
 */
function usesVSCodeAppScheme(appUriScheme: string): boolean {
	return appUriScheme === 'vscode' || appUriScheme === 'vscode-insiders';
}
			...
			if (serverMetadata.registration_endpoint) {
				try {
					const registration = await fetchDynamicRegistration(serverMetadata, this._initData.environment.appName, resourceMetadata?.scopes_supported,
						usesVSCodeAppScheme(this._initData.environment.appUriScheme) ? undefined : { clientUri: null, includeHostedRedirects: false });
...
export class DynamicAuthProvider implements vscode.AuthenticationProvider {
	...
	constructor(
		...
		this._createFlows = [];
		// V3Code: the URL Handler flow's redirect page (vscode.dev) bounces only to the
		// vscode:// scheme family, so on a fork's local desktop it opens a browser that can
		// never return — there, the loopback and device-code flows serve instead. Remote
		// workspaces have no loopback, so the flow stays registered rather than leaving
		// zero flows (pre-change parity; it may still fail at the redirect for forks).
		if (_serverMetadata.authorization_endpoint && (usesVSCodeAppScheme(_initData.environment.appUriScheme) || _initData.remote.isRemote)) {
