...
import { SemanticEmbedChannel } from '../../workbench/contrib/void/electron-main/semanticEmbedChannel.js';
import { EvalSandboxChannel } from '../../workbench/contrib/void/electron-main/evalSandboxChannel.js';
import { BundleReconstructChannel } from '../../workbench/contrib/void/electron-main/bundleReconstructChannel.js';
import { CloudIndexChannel } from '../../workbench/contrib/void/electron-main/cloudIndexChannel.js';
import { WebSearchChannel } from '../../workbench/contrib/void/electron-main/webSearchChannel.js';
import { ResourceMonitorChannel } from '../../workbench/contrib/void/electron-main/resourceMonitorChannel.js';
import { V3CodeRemoteChannel } from '../../workbench/contrib/void/electron-main/remoteChannel.js';
import { ImageGenChannel } from '../../workbench/contrib/void/electron-main/imageGenChannel.js';
import { V3CodeVoiceSessionChannel } from '../../workbench/contrib/void/electron-main/v3codeVoiceSessionChannel.js';
import { MemoryChannel } from '../../workbench/contrib/void/electron-main/memoryChannel.js';
import { SemanticIndexNodeChannel } from '../../workbench/contrib/void/electron-main/semanticIndexNodeChannel.js';
import { BeastChannel } from '../../workbench/contrib/void/electron-main/beastChannel.js';
import { DebugCollectorChannel } from '../../workbench/contrib/void/electron-main/debugCollectorChannel.js';
import { V3_DEBUG_COLLECTOR_CHANNEL } from '../../workbench/contrib/void/common/debugSessionTypes.js';
import { ComputerUseChannel } from '../../workbench/contrib/computerUse/electron-main/computerUseChannel.js';
import { COMPUTER_USE_CHANNEL_NAME } from '../../workbench/contrib/computerUse/common/computerUseTypes.js';
import { LocalInferenceChannel } from '../../workbench/contrib/void/electron-main/localInferenceChannel.js';
import { VoidSCMService } from '../../workbench/contrib/void/electron-main/voidSCMMainService.js';
import { IVoidSCMService } from '../../workbench/contrib/void/common/voidSCMTypes.js';
import { MCPChannel } from '../../workbench/contrib/void/electron-main/mcpChannel.js';
import { V3codeMcpServerChannel } from '../../workbench/contrib/void/electron-main/v3codeMcpServerChannel.js';
/**
...
export class CodeApplication extends Disposable {
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		const semanticEmbedChannel = new SemanticEmbedChannel();
		mainProcessElectronServer.registerChannel('void-channel-semanticEmbed', semanticEmbedChannel);

		const evalSandboxChannel = new EvalSandboxChannel();
		mainProcessElectronServer.registerChannel('void-channel-evalSandbox', evalSandboxChannel);

		const bundleReconstructChannel = new BundleReconstructChannel();
		mainProcessElectronServer.registerChannel('void-channel-bundleReconstruct', bundleReconstructChannel);

		const cloudIndexChannel = new CloudIndexChannel();
		mainProcessElectronServer.registerChannel('void-channel-cloudIndex', cloudIndexChannel);

		const webSearchChannel = new WebSearchChannel(this.configurationService);
		mainProcessElectronServer.registerChannel('void-channel-webSearch', webSearchChannel);

		const resourceMonitorChannel = new ResourceMonitorChannel();
		mainProcessElectronServer.registerChannel('void-channel-resourceMonitor', resourceMonitorChannel);

		// Remote / mobile pairing. One CLI for the whole app, so two windows cannot race
		// to pair the same machine against a single-use key.
		// The pairing CLI is resolved against the user's login shell PATH, not the app's:
		// launched from the dock, `vgo` installed by Homebrew or npm is otherwise invisible.
		const remoteChannel = new V3CodeRemoteChannel(
		...
		mainProcessElectronServer.registerChannel('void-channel-remote', remoteChannel);

		const imageGenChannel = new ImageGenChannel();
		mainProcessElectronServer.registerChannel('void-channel-imageGen', imageGenChannel);

		const voiceSessionChannel = new V3CodeVoiceSessionChannel();
		mainProcessElectronServer.registerChannel('void-channel-voiceSession', voiceSessionChannel);

		const memoryChannel = new MemoryChannel();
		mainProcessElectronServer.registerChannel('void-channel-memory', memoryChannel);

		const semanticIndexNodeChannel = new SemanticIndexNodeChannel();
		mainProcessElectronServer.registerChannel('void-channel-semanticIndexNode', semanticIndexNodeChannel);

		// The open-roots callback is read at prune time (minutes after launch),
		// when windowsMainService exists — beastdb pruning must never touch the
		// db of a workspace that is open in some window.
		const beastChannel = new BeastChannel(() => {
			const roots: string[] = [];
			for (const window of this.windowsMainService?.getWindows() ?? []) {
				if (isSingleFolderWorkspaceIdentifier(window.openedWorkspace)) {
					roots.push(window.openedWorkspace.uri.fsPath);
				}
			}
			return roots;
		}, this.logService);
		mainProcessElectronServer.registerChannel('void-channel-beast', beastChannel);

		// Debug mode's runtime-evidence sink. Started per Debug chat, stopped with it; the
		// renderer reaches it through IDebugSessionService. Owned by `disposables` so its
		// child processes are killed on the way out rather than orphaned on quit.
		const debugCollectorChannel = disposables.add(new DebugCollectorChannel(this.logService));
		mainProcessElectronServer.registerChannel(V3_DEBUG_COLLECTOR_CHANNEL, debugCollectorChannel);

		const computerUseChannel = disposables.add(new ComputerUseChannel(this.logService, this.environmentMainService, this.configurationService));
		mainProcessElectronServer.registerChannel(COMPUTER_USE_CHANNEL_NAME, computerUseChannel);

		const localInferenceChannel = new LocalInferenceChannel();
		mainProcessElectronServer.registerChannel('void-channel-localInference', localInferenceChannel);
		...
		// V3Code added this — exposes the editor's built-in intelligence tools
		// (semantic_search, LSP context bridge, memory) to external agents over a
		// local HTTP/MCP endpoint. Needs the IPC server so it can route tool calls
		// back into the renderer where those tools actually run.
		const v3codeMcpServerChannel = this._register(new V3codeMcpServerChannel(mainProcessElectronServer));
		mainProcessElectronServer.registerChannel('void-channel-mcpExpose', v3codeMcpServerChannel);
