...
import { IReadonlyVSDataTransfer, matchesMimeType, UriList } from '../../../../base/common/dataTransfer.js';
...
/**
 * macOS image clipboard entries can also expose a temporary file URI. If an image-specific
 * provider returns an edit, make the generic path edits secondary so pasting a screenshot does
 * not insert `/var/folders/.../Screenshot.png` alongside the attachment.
 */
function yieldToImageEdits(dataTransfer: IReadonlyVSDataTransfer): DocumentPasteEdit['yieldTo'] {
	const imageMimeTypes = new Set<string>();
	for (const [mimeType] of dataTransfer) {
		if (matchesMimeType('image/*', [mimeType])) {
			imageMimeTypes.add(mimeType);
		}
	}
	return imageMimeTypes.size
		? Array.from(imageMimeTypes, mimeType => ({ mimeType }))
		: undefined;
}
...
export class DefaultTextPasteOrDropEditProvider extends SimplePasteAndDropProvider {
	...
	protected async getEdit(dataTransfer: IReadonlyVSDataTransfer, _token: CancellationToken): Promise<DocumentPasteEdit | undefined> {
		...
		return {
			...
			yieldTo: yieldToImageEdits(dataTransfer),
