...
export class NativeTitlebarPart extends TitlebarPart {
	...
	protected override createContentArea(parent: HTMLElement): HTMLElement {
		...
		const agentsTitle = localize('agentsWindowTitle', "V3Code Agents");
		if (isMacintosh) {
		...
		// Chromeless Agents shell hides the grid titlebar, so layout() never
		// pushes a traffic-light height. Pin RYG orbs into the sidebar chrome
		// band (~52px) so they stop clipping the top window edge on macOS.
		if (isMacintosh && !hasNativeTitlebar(this.configurationService)) {
			const chromeHeight = Math.round(52 * getZoomFactor(targetWindow));
			this.cachedWindowControlHeight = chromeHeight;
	...
	override layout(width: number, height: number): void {
		...
		// Agents is chromeless (titlebar height 0 in the grid) but macOS still
		// needs a traffic-light band. Keep the chrome height pinned even when
		// the titlebar part itself is not visible / has zero layout height.
		if (isMacintosh && !hasNativeTitlebar(this.configurationService)) {
			const chromeHeight = Math.round(52 * getZoomFactor(getWindow(this.element)));
			if (chromeHeight !== this.cachedWindowControlHeight) {
				this.cachedWindowControlHeight = chromeHeight;
				this.nativeHostService.updateWindowControls({
					targetWindowId: getWindowId(getWindow(this.element)),
					height: chromeHeight,
