...
export class SessionsTerminalContribution extends Disposable implements IWorkbenchContribution {
	...
	private _terminalEditorContainment: Promise<void> = Promise.resolve();
	...
	constructor(
		...
		// Terminal is one contained workspace mode. Individual terminal
		// instances live in the terminal view's own internal tab rail.
		const terminalViewVisible = SessionsTerminalViewVisibleContext.bindTo(contextKeyService);
		const updateTerminalViewVisible = () => terminalViewVisible.set(
			layoutService.isVisible(Parts.PANEL_PART) && viewsService.isViewVisible(TERMINAL_VIEW_ID)
		);
		updateTerminalViewVisible();
		...
		// Sessions has one outer Terminal mode. If any generic workbench
		// command creates a terminal editor, immediately transfer that instance
		// into the terminal panel's internal instance rail.
		this._register(this._terminalEditorService.onDidChangeInstances(() => {
			void this.containTerminalEditors();
		}));
		void this.containTerminalEditors();
	...
	async containTerminalEditors(): Promise<void> {
		this._terminalEditorContainment = this._terminalEditorContainment
			.then(async () => {
				for (const instance of [...this._terminalEditorService.instances]) {
					await this._terminalService.moveToTerminalView(instance);
				}
			})
			.catch(error => {
				this._logService.error('[SessionsTerminal] Failed to contain terminal editor', error);
			});

		await this._terminalEditorContainment;
	}
...
class OpenSessionInTerminalAction extends Action2 {
	...
	override async run(_accessor: ServicesAccessor): Promise<void> {
		...
		await contribution.containTerminalEditors();
		const instances = await contribution.ensureTerminal(cwd, false, info?.agentHostCwd ? activeSession : undefined);
		if (instances.length === 0) {
			return;
		}

		await contribution.containTerminalEditors();
		terminalService.setActiveInstance(instances[0]);

		// PanelPart is a full-width body overlay inside RightWorkspaceView.
		// The permanent editor title remains Row 1; the terminal's own title
		// and instance rail are fully contained underneath it.
