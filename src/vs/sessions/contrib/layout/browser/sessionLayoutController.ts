...
export class LayoutController extends Disposable {
	...
	constructor(
		...
		// The permanent workplace rail owns Browser / files / tabs. `useModal:
		// 'all'` redirects ACTIVE_GROUP opens into a separate modal part, which
		// is why users had to manually "add Browser to the workplace". Prefer
		// the workplace; RequiresModal editors can still open as modal via 'some'.
		if (this._configurationService.getValue<string>('workbench.editor.useModal') === 'all') {
			void this._configurationService.updateValue('workbench.editor.useModal', 'some');
		}
		...
		// Independent of working-set restore: selecting a session must always
		// reveal the workplace rail and seed Browser when the group is empty,
		// so launch no longer requires "open chat → open Browser" by hand.
		this._register(runOnChange(activeSessionForWorkingSet, (session) => {
			if (session) {
				void this._ensureWorkplaceSurface();
			}
		}));
		if (activeSessionForWorkingSet.get()) {
			void this._ensureWorkplaceSurface();
		}
	}
	...
	private async _applyWorkingSet(sessionResource: URI | undefined): Promise<void> {
		...
		return this._workingSetSequencer.queue(async () => {
			if (workingSet === 'empty') {
				...
				if (sessionResource) {
					await this._ensureWorkplaceSurface();
				}
				return;
			}
	...
	/**
	 * Makes the permanent workplace rail usable as soon as a session is
	 * active: show the editor part and, when the main group is empty, seed a
	 * Browser tab directly into that group (never the modal part).
	 */
	private async _ensureWorkplaceSurface(): Promise<void> {
		if (!this._layoutService.isVisible(Parts.EDITOR_PART, mainWindow)) {
			this._layoutService.setPartHidden(false, Parts.EDITOR_PART);
		}

		const mainGroup = this._editorGroupsService.mainPart.activeGroup;
