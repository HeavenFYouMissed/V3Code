...
// === CHAT SURFACE: AGENTS_WINDOW_CHAT ===
// Engine: NATIVE, but in the SEPARATE Agents Window renderer (its own IChatService instance,
// NOT the main window's). Registers 3 mutually-exclusive ChatBar views gated by context keys:
//   ChatViewPane (transcript)        when !IsNewChatSession && !IsNewChatInSession
//   NewChatViewPane (composer only)  when IsNewChatSession
//   NewChatInSessionViewPane (composer only) when !IsNewChatSession && IsNewChatInSession
// KNOWN BUG: after send, the context key must flip so ChatViewPane becomes visible — if it
// doesn't, "nothing shows" (see sendNewChatRequest + sessionsOpenerParticipant.ts).
// See CHAT_SURFACES_MAP.md.
