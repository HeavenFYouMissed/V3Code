...
// === CHAT SURFACE: AGENTS_WINDOW_CHAT (composer-only "New Session" pane) ===
// Engine: NATIVE (Agents Window renderer). This pane is INPUT ONLY — it has NO transcript.
// After send, IsNewChatSessionContext must flip to false so ChatViewPane (the transcript)
// takes over; if that handoff fails the user sees "nothing" after sending. This is the
// prime suspect for the Agents Window "message sent but nothing shows" bug.
...
class NewChatWidget extends Disposable {
	...
	private async _send(query: string, attachedContext?: IChatRequestVariableEntry[]): Promise<void> {
		...
		// Let errors propagate to NewChatInputWidget._send, which logs and
		// surfaces them; swallowing here rendered failures as a silent blank.
