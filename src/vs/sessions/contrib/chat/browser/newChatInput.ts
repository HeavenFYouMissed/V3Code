...
export class NewChatInputWidget extends Disposable implements IHistoryNavigationWidget {
	...
	private async _send(): Promise<void> {
		...
		try {
			...
			// Surface the failure — a silent log line renders as a blank send while
			// the widget resets as if it succeeded. Input text and attachments are
			// preserved (they only clear inside the try on success).
			this.logService.error('Failed to send request:', e);
			this.notificationService.error(localize('sessionsChatInput.sendFailed', "Failed to send message: {0}", toErrorMessage(e)));
		}
