...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	/**
	 * Visual margins are zero: the auxiliary bar sits flush inside the right
	 * region sheet; dividers are 1px CSS box-shadow hairlines (no layout space).
	 */
	...
	private static readonly DEFAULT_MINIMUM_WIDTH = 200;
	...
	constructor(
		...
		super(
			...
			{
				// Files/Changes owns a local rail header beneath the outer
				// editor tab strip. This keeps its tree/actions contained,
				// matching the compound surface in Cursor's agents window.
				hasTitle: true,
	...
	override updateStyles(): void {
		...
		// No inline backgroundColor: the glass fill (translucent color-mix of
		// --part-background) is applied in style.css and an inline opaque
		// paint here would defeat it.
	...
	override layout(width: number, height: number, top: number, left: number): void {
		...
		// Margins and borders are zero — the auxiliary bar fills its grid cell
		// exactly. Dividers are CSS box-shadow hairlines (no layout space).
