...
class SessionsSetUpWidget extends Disposable {
	...
	private _createWelcomeFooter(disposables: DisposableStore): HTMLElement {
		...
		const providerName = defaultChatAgent?.provider?.default?.name ?? this.productService.nameLong;
		...
		const footer = localize(
			{ key: 'welcomeFooter', comment: ['{Locked="["}', '{Locked="]({1})"}', '{Locked="]({2})"}'] },
			"By continuing, you agree to {0}'s [Terms]({1}) and [Privacy Statement]({2}).",
			providerName, termsUrl, privacyUrl
		);
