...
import { RightWorkspaceView, WorkplaceStageMode } from './rightWorkspaceView.js';
...
export class Workbench extends Disposable implements IAgentWorkbenchLayoutService {
	...
	private rightWorkspaceView!: RightWorkspaceView;
	...
	private _savePartSizes(): void {
		...
		const sizes: IPartSizesState = {
			...
			rightBlock: this.workbenchGrid.getViewSize(this.rightWorkspaceView).width,
			auxiliaryBar: this.rightWorkspaceView.getPartCachedVisibleWidth(Parts.AUXILIARYBAR_PART),
			chatBar: getSize(this.chatBarPartView, 'width', this.partVisibility.chatBar) ?? this._savedPartSizes.chatBar,
			editor: this.rightWorkspaceView.getPartCachedVisibleWidth(Parts.EDITOR_PART),
			panel: this.rightWorkspaceView.getPartCachedVisibleWidth(Parts.PANEL_PART),
		};
	...
	private renderWorkbench(instantiationService: IInstantiationService, notificationService: NotificationService, storageService: IStorageService, configurationService: IConfigurationService): void {
		...
		// The editor's default is authoritative: on desktop it is always
		// visible (its tab bar is the right block's permanent strip), so a
		// stale saved `false` from an older layout must not hide it.
		this.partVisibility.editor = visibilityDefaults.editor || (savedPartVisibility.editor ?? false);
		...
		const workbenchClasses = coalesce([
			...
			(isMacintosh && isNative) ? 'sessions-vibrancy' : undefined,
	...
	initLayout(accessor: ServicesAccessor): void {
		...
		// Selecting an already-open editor does not fire onWillOpenEditor.
		// Leaving Terminal mode and enforcing browser full-surface behavior
		// therefore also happens on active-editor changes.
		this._register(this.editorService.onDidActiveEditorChange(() => {
			const activeEditor = this.editorService.activeEditor;
			if (activeEditor && this.partVisibility.panel) {
				this.setPanelHidden(true);
			}
			if (activeEditor && this.partVisibility.auxiliaryBar && this.editorUsesFullWorkspace(activeEditor.typeId)) {
				this.setAuxiliaryBarHidden(true);
			}
		}));

		// The native editor tab strip is permanent on desktop, even with no
		// open editors. Phone retains the existing modal behavior.
	...
	private editorUsesFullWorkspace(typeId: string): boolean {
		return typeId === 'workbench.editorinputs.browser' || typeId === 'workbench.editors.terminal';
	}
	...
	createWorkbenchLayout(): void {
		...
		const initialSizes = this.layoutPolicy.getPartSizes(
			this._mainContainerDimension.width,
			this._mainContainerDimension.height
		);
		const initialRightWorkspaceWidth = this._savedPartSizes.rightBlock ?? initialSizes.rightBlockSize;
		const initialAuxiliaryBarWidth = Math.max(200, Math.min(
			this._savedPartSizes.auxiliaryBar ?? initialSizes.auxiliaryBarSize,
			Math.max(200, initialRightWorkspaceWidth - 260),
			420,
		));
		this.rightWorkspaceView = this._register(new RightWorkspaceView(
			editorPart,
			panelPart,
			auxiliaryBarPart,
			initialRightWorkspaceWidth,
			initialAuxiliaryBarWidth,
			this.partVisibility.editor,
			this.partVisibility.panel,
			this.partVisibility.auxiliaryBar,
		));
		this._register(this.rightWorkspaceView.onDidChangeMode(mode => this.syncPartVisibilityFromStageMode(mode)));
		this._register(this.rightWorkspaceView.onDidRequestMode(mode => this.handleWorkplaceStageModeRequest(mode)));
		this._register(this.paneCompositeService.onDidPaneCompositeOpen(({ composite, viewContainerLocation }) => {
			...
			if (id === 'workbench.view.agentSessions.changesContainer') {
				this.rightWorkspaceView.setMode('changes');
			} else if (id === 'workbench.sessions.auxiliaryBar.filesContainer') {
				this.rightWorkspaceView.setMode('files');
			}
		}));
		...
		const viewMap: { [key: string]: ISerializableView } = {
			...
			[RightWorkspaceView.TYPE]: this.rightWorkspaceView,
		};
		...
		// Glass chat is an in-grid column (real sash), not an absolute overlay.
		chatBarPart.element.classList.add('sessions-chat-glass');
		chatBarPart.element.classList.remove('sessions-chat-overlay');

		// Floating top-left chrome when the Sessions sidebar is collapsed so
		// traffic lights stay clear and the sidebar can be reopened.
		const collapsedChrome = document.createElement('div');
		collapsedChrome.classList.add('sessions-collapsed-chrome');
		if (isMacintosh && isNative) {
			const spacer = document.createElement('div');
			spacer.classList.add('window-controls-container');
			collapsedChrome.appendChild(spacer);
		}
		const collapsedActions = document.createElement('div');
		collapsedActions.classList.add('sessions-collapsed-chrome-actions');
		collapsedChrome.appendChild(collapsedActions);
		this.mainContainer.appendChild(collapsedChrome);
		this._register(this.instantiationService.createInstance(MenuWorkbenchToolBar, collapsedActions, Menus.SidebarTitle, {
			hiddenItemStrategy: HiddenItemStrategy.NoHide,
			toolbarOptions: { primaryGroup: () => true },
			telemetrySource: 'sessionsCollapsedChrome',
		}));
	 ...
	 *
	 * Structure:
	 * - Titlebar (top, full width, chromeless / hidden)
	 * - Content (horizontal): Sidebar | Glass Chat | Workplace Stage
	 *
	 * Dual sashes between Sessions↔Chat and Chat↔Stage.
	 */
	...
	private createDesktopGridDescriptor(width: number, height: number): ISerializedGrid {
		...
		const editorOrPanelVisible = this.partVisibility.editor || this.partVisibility.panel;
		const rightBlockVisible = editorOrPanelVisible || this.partVisibility.auxiliaryBar;
		const requestedRightBlockWidth = this._savedPartSizes.rightBlock ?? sizes.rightBlockSize;
		const rightBlockWidth = Math.min(
			requestedRightBlockWidth,
			Math.max(320, width - effectiveSideBarWidth)
		);
		const effectiveRightBlockWidth = rightBlockVisible ? rightBlockWidth : 0;

		const chatBarWidth = this._savedPartSizes.chatBar
			?? Math.max(0, width - effectiveSideBarWidth - effectiveRightBlockWidth);
		...
		const rightWorkspaceNode: ISerializedLeafNode = {
			type: 'leaf',
			data: { type: RightWorkspaceView.TYPE },
			size: rightBlockWidth,
			visible: rightBlockVisible
		};

		const contentSection: ISerializedNode = {
			type: 'branch',
			data: [sideBarNode, chatBarNode, rightWorkspaceNode],
			size: contentHeight
		};
	...
	layout(): void {
		...
		if (previousClass !== undefined && previousClass !== currentClass) {
			if (currentClass === 'phone' && !this.mobileTopBarElement) {
				...
				// Chromeless Agents shell — never restore the grid titlebar.
				this.workbenchGrid.setViewVisible(this.titleBarPartView, false);
		...
		// The right region (editor/aux/panel) is pinned flush to the window's
		// right and bottom edges — no outer gutter. Column separation is drawn
		// with 1px CSS dividers instead of margin gaps (see style.css).
	...
	isVisible(part: Parts, targetWindow?: Window): boolean {
		switch (part) {
			case Parts.TITLEBAR_PART:
				// Agents is chromeless: the workspace tab rail is the top edge.
				// Phone still uses MobileTitlebarPart outside the grid.
				return false;
			case Parts.SIDEBAR_PART:
	...
	alignUnifiedSidebarToChat(_side: 'left' | 'right'): void {
		// Sessions workbench has no unified Agents rail.
	}
	...
	/**
	 * Right block behavior: exactly one stage mode owns the workplace body
	 * (Browser/Code, Terminal, Files, or Changes). Never leave the stage empty.
	 */
	private _fallbackRightBlockSurface(): void {
		// Phone always needs a surface in the right stack. Desktop Agents can
		// collapse the Workplace stage entirely (Cursor-style expand chat).
		if (this.layoutPolicy.viewportClass.get() !== 'phone') {
			return;
		}
		if (!this.partVisibility.editor && !this.partVisibility.panel && !this.partVisibility.auxiliaryBar) {
			this.setAuxiliaryBarHidden(false);
		}
	}

	private updateRightWorkspaceVisibility(): void {
		if (!this.workbenchGrid || !this.rightWorkspaceView) {
			return;
		}

		const visible = this.partVisibility.editor
			|| this.partVisibility.panel
			|| this.partVisibility.auxiliaryBar;
		this.workbenchGrid.setViewVisible(this.rightWorkspaceView, visible);
	}

	private _applyingStageMode = false;

	private syncPartVisibilityFromStageMode(mode: WorkplaceStageMode): void {
		const editor = mode === 'browser' || mode === 'code';
		const panel = mode === 'terminal';
		const auxiliaryBar = mode === 'files' || mode === 'changes';

		this.partVisibility.editor = editor;
		this.partVisibility.panel = panel;
		this.partVisibility.auxiliaryBar = auxiliaryBar;
		this.mainContainer.classList.toggle(LayoutClasses.MAIN_EDITOR_AREA_HIDDEN, !editor);
		this.mainContainer.classList.toggle(LayoutClasses.PANEL_HIDDEN, !panel);
		this.mainContainer.classList.toggle(LayoutClasses.AUXILIARYBAR_HIDDEN, !auxiliaryBar);
		this.updateRightWorkspaceVisibility();
		this._savePartVisibility();
	}

	private handleWorkplaceStageModeRequest(mode: WorkplaceStageMode): void {
		const OPEN_SESSION_TERMINAL = 'agentSession.openInTerminal';
		const SHOW_CHANGES = 'sessions.rightBlock.showChanges';
		const SHOW_FILES = 'sessions.rightBlock.showFiles';
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			switch (mode) {
				case 'browser':
				case 'code':
					void commands.executeCommand(BrowserViewCommandId.Open);
					break;
				case 'terminal':
					void commands.executeCommand(OPEN_SESSION_TERMINAL);
					break;
				case 'changes':
					void commands.executeCommand(SHOW_CHANGES);
					break;
				case 'files':
					void commands.executeCommand(SHOW_FILES);
					break;
			}
		});
	}
	...
	private setAuxiliaryBarHidden(hidden: boolean): void {
		if (this._applyingStageMode) {
			return;
		}
		if (this.partVisibility.auxiliaryBar === !hidden) {
			if (!hidden) {
				this.rightWorkspaceView.setMode(
					this.rightWorkspaceView.currentMode === 'changes' ? 'changes' : 'files'
				);
			}
			return;
		}
		...
		this._applyingStageMode = true;
		try {
			if (!hidden) {
				this.partVisibility.panel = false;
				this.mainContainer.classList.toggle(LayoutClasses.PANEL_HIDDEN, true);
				this.partVisibility.editor = false;
				this.mainContainer.classList.toggle(LayoutClasses.MAIN_EDITOR_AREA_HIDDEN, true);
			}

			this.partVisibility.auxiliaryBar = !hidden;
			this.mainContainer.classList.toggle(LayoutClasses.AUXILIARYBAR_HIDDEN, hidden);

			this.rightWorkspaceView.setPartVisible(Parts.AUXILIARYBAR_PART, !hidden);
			this.updateRightWorkspaceVisibility();
		} finally {
			this._applyingStageMode = false;
		}
		...
		if (hidden) {
			this._fallbackRightBlockSurface();
		}
	...
	private setEditorHidden(hidden: boolean): void {
		...
		if (this.partVisibility.editor === !hidden) {
			if (!hidden) {
				this.rightWorkspaceView.setMode(
					this.rightWorkspaceView.currentMode === 'code' ? 'code' : 'browser'
				);
			}
			return;
		}
		...
		try {
			if (!hidden) {
				...
				this.mainContainer.classList.toggle(LayoutClasses.AUXILIARYBAR_HIDDEN, true);
			}

			this.partVisibility.editor = !hidden;
			this.mainContainer.classList.toggle(LayoutClasses.MAIN_EDITOR_AREA_HIDDEN, hidden);

			this.rightWorkspaceView.setPartVisible(Parts.EDITOR_PART, !hidden);
			this.updateRightWorkspaceVisibility();
		} finally {
			this._applyingStageMode = false;
		}
	...
	private setPanelHidden(hidden: boolean): void {
		...
		if (this.partVisibility.panel === !hidden) {
			if (!hidden) {
				this.rightWorkspaceView.setMode('terminal');
			}
			return;
		}
		...
		try {
			...
			this.rightWorkspaceView.setPartVisible(Parts.PANEL_PART, !hidden);
			this.updateRightWorkspaceVisibility();
		} finally {
			this._applyingStageMode = false;
		}
	...
	getSize(part: Parts): IViewSize {
		if (part === Parts.EDITOR_PART || part === Parts.PANEL_PART || part === Parts.AUXILIARYBAR_PART) {
			return this.rightWorkspaceView.getPartSize(part);
		}
	...
	setSize(part: Parts, size: IViewSize): void {
		if (part === Parts.EDITOR_PART || part === Parts.PANEL_PART || part === Parts.AUXILIARYBAR_PART) {
			this.rightWorkspaceView.resizePart(part, size.width);
			return;
		}
	...
	resizePart(part: Parts, sizeChangeWidth: number, sizeChangeHeight: number): void {
		if (part === Parts.EDITOR_PART || part === Parts.PANEL_PART || part === Parts.AUXILIARYBAR_PART) {
			const currentSize = this.rightWorkspaceView.getPartSize(part);
			this.rightWorkspaceView.resizePart(part, currentSize.width + sizeChangeWidth);
			return;
		}
	...
	private getPartView(part: Parts): ISerializableView | undefined {
		switch (part) {
			...
			case Parts.EDITOR_PART:
			case Parts.PANEL_PART:
				return this.rightWorkspaceView;
			case Parts.CHATBAR_PART:
	...
	getMaximumEditorDimensions(_container: HTMLElement): IDimension {
		const rightWorkspaceSize = this.workbenchGrid.getViewSize(this.rightWorkspaceView);
		return new Dimension(
			Math.max(0, rightWorkspaceSize.width),
			Math.max(0, rightWorkspaceSize.height - 40),
		);
	}
	...
	toggleMaximizedPanel(): void {
		// The panel already owns the complete body of RightWorkspace when
		// selected, so there is no second maximized geometry to enter.
	}
	...
	getVisibleNeighborPart(part: Parts, direction: Direction): Parts | undefined {
		...
		if (neighborView === this.rightWorkspaceView) {
			if (this.partVisibility.panel) {
