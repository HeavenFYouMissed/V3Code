...
export interface IPartSizeDefaults {
	...
	/** Total width of the tabbed workspace pinned to the right edge. */
	readonly rightBlockSize: number;
	/** Width of the Files/Changes rail when attached to the workspace. */
...
export class SessionsLayoutPolicy extends Disposable {
	...
	getPartVisibilityDefaults(viewportClass?: ViewportClass): IPartVisibilityDefaults {
		...
		switch (vc) {
			...
			case 'desktop':
				...
				// The editor is ALWAYS visible on desktop: its native tab bar is
				// the permanent top strip of the right block (tabs + the "+"
				// menu + the Changes/Files pills all live there).
				return { sidebar: true, auxiliaryBar: true, panel: false, chatBar: true, editor: true };
		}
	}
	...
	getPartSizes(width: number, _height: number, viewportClass?: ViewportClass): IPartSizeDefaults {
		...
		switch (vc) {
			...
			case 'tablet':
			case 'desktop': {
				// The right workspace is deliberately dominant. Files/Changes
				// attach as a contained rail on its right, while the active
				// editor, browser, or terminal uses the remaining width.
				// Narrow desktop windows keep smaller-but-usable minima rather
				// than switching to the touch-only phone layout.
				const compact = width < 900;
				const sideBarSize = compact ? 220 : 300;
				const rightBlockSize = Math.max(compact ? 420 : 560, Math.round(width * 0.56));
				const auxiliaryBarSize = Math.max(200, Math.min(320, Math.round(rightBlockSize * 0.36)));
				return {
					sideBarSize,
					rightBlockSize,
					auxiliaryBarSize,
					panelSize: 300,
					chatBarWidth: Math.max(0, width - sideBarSize - rightBlockSize),
				};
			}
		}
	}
