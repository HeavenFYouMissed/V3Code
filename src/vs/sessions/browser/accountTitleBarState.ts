...
function getCopilotPresentation(
	...
	if (sentiment.disabled || sentiment.untrusted) {
		return {
			...
			kind: 'warning',
			icon: Codicon.account,
			label: localize('copilotUnavailable', "V3Code Unavailable"),
			ariaLabel: sentiment.untrusted
				? localize('copilotUnavailableUntrustedAria', "V3Code Agent is unavailable in untrusted workspaces")
				: localize('copilotUnavailableDisabledAria', "V3Code Agent is disabled"),
		};
	}
	...
	if (entitlement === ChatEntitlement.Free && typeof remainingPercent === 'number' && remainingPercent <= 25) {
		return {
			...
			ariaLabel: localize('copilotTokensRemainingAria', "{0}% V3Code Agent tokens remaining", remainingPercent),
		};
	}
...
function getQuotaReachedAriaLabel(chatQuotaExceeded: boolean, completionsQuotaExceeded: boolean): string {
	if (chatQuotaExceeded && completionsQuotaExceeded) {
		return localize('copilotAllQuotaReachedAria', "V3Code Agent chat and inline suggestion quota reached");
	}

	if (chatQuotaExceeded) {
		return localize('copilotChatQuotaReachedAria', "V3Code Agent chat quota reached");
	}

	return localize('copilotCompletionsQuotaReachedAria', "V3Code Agent inline suggestion quota reached");
}
