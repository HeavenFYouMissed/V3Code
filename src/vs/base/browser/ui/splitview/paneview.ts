...
export class PaneView extends Disposable {
	...
	constructor(container: HTMLElement, options: IPaneViewOptions = {}) {
		...
		this._dragEnabled = options.dragEnabled !== false;
	...
	addPane(pane: Pane, size: number, index = this.splitview.length): void {
		...
		} else if (pane.draggableElement) {
			pane.draggableElement.draggable = false;
		}
	}
	...
	set dragEnabled(enabled: boolean) {
		...
		for (const paneItem of this.paneItems) {
			const el = paneItem.pane.draggableElement;
			if (el) {
				el.draggable = enabled && !!this.dnd;
			}
		}
	}
