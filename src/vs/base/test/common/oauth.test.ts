...
suite('OAuth', () => {
	...
	suite('Network Functions', () => {
		...
		// V3Code: fork identity — no borrowed client_uri, no hosted redirect pages the
		// fork's URL scheme can never receive. The loopback redirect URIs stay registered.
		test('fetchDynamicRegistration fork overrides omit client_uri and hosted redirects', async () => {
			fetchStub.resolves({
				ok: true,
				json: async () => ({ client_id: 'generated-client-id', client_name: 'Fork App' })
			} as Response);
			...
			await fetchDynamicRegistration(serverMetadata, 'Fork App', undefined, { clientUri: null, includeHostedRedirects: false });

			const requestBody = JSON.parse(fetchStub.firstCall.args[1].body as string);
			assert.strictEqual(requestBody.client_name, 'Fork App');
			assert.strictEqual(Object.prototype.hasOwnProperty.call(requestBody, 'client_uri'), false, 'client_uri must be omitted entirely');
			assert.deepStrictEqual(requestBody.redirect_uris, [
				'http://127.0.0.1/',
				`http://127.0.0.1:${DEFAULT_AUTH_FLOW_PORT}/`
			]);
			assert.strictEqual(requestBody.application_type, 'native');
		});
