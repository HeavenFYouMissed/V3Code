...
/**
 * V3Code: registration identity knobs for forks. Defaults preserve upstream behavior.
 * - `clientUri: null` omits the RFC 7591 client_uri entirely (it is optional); a fork
 *   must not register itself under another product's homepage while presenting its own
 *   client_name — providers that review registrations reject the mismatch.
 * - `includeHostedRedirects: false` drops the vscode.dev redirect pages, which bounce
 *   only to the vscode:// scheme family; a fork's URL scheme never receives them.
 */
export interface IDynamicRegistrationOverrides {
	readonly clientUri?: string | null;
	readonly includeHostedRedirects?: boolean;
}

export async function fetchDynamicRegistration(serverMetadata: IAuthorizationServerMetadata, clientName: string, scopes?: string[], overrides?: IDynamicRegistrationOverrides): Promise<IAuthorizationDynamicClientRegistrationResponse> {
	if (!serverMetadata.registration_endpoint) {
		throw new Error('Server does not support dynamic registration');
	}

	const clientUri = overrides?.clientUri === undefined ? 'https://code.visualstudio.com' : overrides.clientUri;
	const hostedRedirects = overrides?.includeHostedRedirects === false ? [] : [
