...
/**
 * The extended (1 hour) prompt cache TTL via
 * `cache_control: { type: 'ephemeral', ttl: '1h' }` is GA on the Claude 4.5
 * generation and newer, and on every gen-5+ family (opus / sonnet / haiku /
 * fable / mythos / ...). The 4.0/4.1 generation (`claude-opus-4`,
 * `claude-opus-4-1`, `claude-sonnet-4`) and the Claude 3.x line are
 * intentionally excluded — they never offered extended TTL.
 *
 * This is computed by PARSING the version rather than maintaining an explicit
 * allow-list, so future models — `claude-opus-4-9`, `claude-opus-5`,
 * `claude-fable-5`, `claude-mythos-5`, etc. — light up automatically with no
 * code change. (That avoids silently regressing a new model back to the
 * expensive 5-minute-only cache.)
 *
 * Accepts either an id string, a {@link LanguageModelChat}, or an
 * {@link IChatEndpoint} — when given an endpoint/chat the model **family**
 * is also checked, so a per-model family override lights this up
...
export function modelSupportsExtendedCacheTtl(model: LanguageModelChat | IChatEndpoint | string): boolean {
	...
	const family = typeof model === 'string' ? model : model.family;
	const matches = (s: string) => {
		// Normalize "4.6" -> "4-6" so both id and family naming schemes parse.
		const n = s.toLowerCase().replace(/\./g, '-');
		// Tier-named models only: claude-<family>-<major>[-<minor>]... The leading
		// letters requirement also rejects the old tier-last 3.x ids (claude-3-7-sonnet).
		const m = /^claude-[a-z]+-(\d+)(?:-(\d+))?/.exec(n);
		if (!m) {
			return false;
		}
		const major = parseInt(m[1], 10);
		const minor = m[2] !== undefined ? parseInt(m[2], 10) : 0;
		// Gen 5+ (any family, incl. fable/mythos), or the 4.5 generation onward.
