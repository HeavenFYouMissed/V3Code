...
// Central registry and dispatcher for Copilot tools—both built-in and contributed. Manages tool definitions, 
// applies model-specific overrides, and delegates tool execution to handlers. Bridges vscode.lm.tools with 
// Copilot's ToolRegistry and provides caching/filtering logic for performance.
export class ToolsService extends BaseToolsService {
	declare _serviceBrand: undefined;

	// Lazy-loaded built-in tool registry keyed by ToolName; instantiated on first access via instantiation service
	private readonly _copilotTools: Lazy<Map<ToolName, ICopilotTool<unknown>>>;

	// Lazy-loaded extension-provided tool overrides for altering built-in tool definitions per model/config
	...
	// Returns all available tools (built-in + contributed), sorted with built-ins first, filtered by model capabilities,
	// with alternative definitions applied. Caches input/output to avoid re-processing on unchanged tool list.
	...
	// Invokes a named tool with input options and traces execution via OpenTelemetry. Handles both built-in and MCP tools,
	// propagates W3C trace context to downstream services, records tool metrics and debug logs, and manages subagent trace context.
