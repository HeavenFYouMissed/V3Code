...
export class CustomEndpointOAIEndpoint extends OpenAIEndpoint {
	...
	public override getExtraHeaders(): Record<string, string> {
		...
		// Only emit a credential header when a key is actually configured. A keyless custom
		// endpoint (local llama.cpp / LM Studio / gateway) otherwise received a malformed
		// `Authorization: Bearer ` / `x-api-key: ` and rejected every request — the same
		// empty-credential bug fixed for the parent OpenAIEndpoint in commit 9fe70be0, which
		// this subclass didn't inherit because it fully overrides getExtraHeaders.
		if (this.useMessagesApi) {
			if (!userSuppliedAuth && this._apiKey) {
				headers['x-api-key'] = this._apiKey;
			}
			headers['anthropic-version'] = '2023-06-01';
			Object.assign(headers, this.getAnthropicBetaHeader());
		} else if (!userSuppliedAuth && this._apiKey) {
