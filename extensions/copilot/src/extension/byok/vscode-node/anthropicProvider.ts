...
import { addMessagesApiCacheControl, buildToolInputSchema, clearAllCacheControl } from '../../../platform/endpoint/node/messagesApi';
import { ILogService } from '../../../platform/log/common/logService';
import { ContextManagementResponse, CUSTOM_TOOL_SEARCH_NAME, getContextManagementFromConfig, isAnthropicContextEditingEnabled, modelSupportsExtendedCacheTtl, modelSupportsMemory } from '../../../platform/networking/common/anthropic';
...
export class AnthropicLMProvider extends AbstractLanguageModelChatProvider {
	...
	async provideLanguageModelChatResponse(model: ExtendedLanguageModelChatInformation<LanguageModelChatConfiguration>, messages: Array<LanguageModelChatMessage | LanguageModelChatMessage2>, options: ProvideLanguageModelChatResponseOptions, progress: Progress<LanguageModelResponsePart2>, token: CancellationToken): Promise<void> {
		...
		const doRequest = async () => {
			...
			const messagesResult = { messages: convertedMessages, system: [system] };
			const useExtendedCacheTtl = modelSupportsExtendedCacheTtl(model.id);
			const cacheTtl = useExtendedCacheTtl ? '1h' as const : undefined;
			...
			// Own the cache policy in BYOK Anthropic. Prompt renderers can emit many
			// `CacheControl` data parts, which the converter turns into default 5m
			// Anthropic breakpoints. That was the cost bug: giant rolling context blocks
			// were still being written as 5m cache even after tools/system moved to 1h.
			// Clear them and re-add the deterministic 4-breakpoint layout used by the
			// maintained Messages API path: tool, system, and the two most recent
			// cacheable message blocks. For supported Claude models all four use 1h.
			clearAllCacheControl(messagesResult);
			addMessagesApiCacheControl(messagesResult, cacheTtl);

			// --- Extended (1-hour) prompt cache on the stable prefix -------------------
			// The default ephemeral cache is 5 minutes. In an interactive coding agent the
			// gaps between turns (reading code, thinking, slow tool calls) routinely exceed
			// 5 minutes, so the large stable prefix (tool definitions + system prompt) keeps
			// expiring and is re-WRITTEN at the 1.25x premium every few minutes — that is the
			// "cache gone in 5 minutes" cost. Giving the prefix the 1-hour TTL turns that into
			// a single 2x write that is then re-read at 0.10x for the whole hour. Anthropic
			// requires longer-TTL breakpoints to appear before shorter ones; with message
			// breakpoints also using 1h in BYOK, there is no mixed-TTL ordering hazard.
			if (useExtendedCacheTtl) {
				const stableCacheControl = { type: 'ephemeral' as const, ttl: '1h' as const };
				// Marking the LAST non-deferred function/web-search tool caches the entire
				// tool-definition block before it. Skip deferred tools (they load on demand)
				// and the native memory tool (its type does not accept cache_control).
				for (let i = tools.length - 1; i >= 0; i--) {
					const tool = tools[i] as { defer_loading?: boolean; type?: string; cache_control?: { type: 'ephemeral'; ttl?: '5m' | '1h' } };
					if (tool.defer_loading || tool.type === 'memory_20250818') {
						continue;
					}
					tool.cache_control = stableCacheControl;
					break;
				}
				// The system prompt is stable across turns: upgrade its breakpoint (which the
				// prompt renderer leaves at the 5m default) to the 1-hour TTL.
				if (system) {
					system.cache_control = stableCacheControl;
				}
				// Sending a 1h TTL requires the extended-cache beta header.
			...
			this._logCacheBreakpoints(params, betas);
			...
			const pendingLoggedChatRequest = this._requestLogger.logChatRequest(
				...
				{
					model: model.id,
					messages: anthropicMessagesToRawMessagesForLogging(messagesResult.messages, system),
	...
	private _logCacheBreakpoints(params: Anthropic.Beta.Messages.MessageCreateParamsStreaming, betas: string[]): void {
		...
		const note = (position: string, cacheControl: CacheControl | undefined) => {
			if (!cacheControl) {
				return;
			}
			counts.total++;
			const ttl = cacheControl.ttl ?? '5m';
			if (ttl === '1h') {
				counts.oneHour++;
			} else {
				counts.fiveMinute++;
			}
			positions.push(`${position}:${ttl}`);
		};

		params.tools?.forEach((tool, i) => {
			note(`tools[${i}]`, (tool as { cache_control?: CacheControl }).cache_control);
		});
		if (Array.isArray(params.system)) {
			params.system.forEach((block, i) => {
				note(`system[${i}]`, (block as { cache_control?: CacheControl }).cache_control);
			});
		}
			...
			if (Array.isArray(message.content)) {
				message.content.forEach((block, j) => {
					if (typeof block === 'object') {
						note(`messages[${i}].block[${j}]`, (block as { cache_control?: CacheControl }).cache_control);
					}
				});
			}
		});

		this._logService.info(`[anthropic-cache-request] breakpoints=${counts.total} 1h=${counts.oneHour} 5m=${counts.fiveMinute} extendedBeta=${betas.includes('extended-cache-ttl-2025-04-11')} positions=${positions.join(',')}`);
	}
	...
	private async _makeRequest(anthropicClient: Anthropic, progress: RecordedProgress<LMResponsePart>, params: Anthropic.Beta.Messages.MessageCreateParamsStreaming, betas: string[], token: CancellationToken, issuedTime: number): Promise<{ ttft: number | undefined; ttfte: number | undefined; usage: APIUsage | undefined; contextManagement: ContextManagementResponse | undefined }> {
		...
		for await (const chunk of stream) {
			...
			if (chunk.type === 'message_start') {
				...
				// Prompt-cache efficiency probe. A healthy steady state is read >> write, with
				// the prefix landing in the 1h column on the first turn and read back after. If
				// `write` stays high and `1h` stays 0, the extended cache isn't engaging.
				const cacheUsage = chunk.message.usage as {
					input_tokens: number;
					cache_read_input_tokens?: number;
					cache_creation_input_tokens?: number;
					cache_creation?: { ephemeral_5m_input_tokens?: number; ephemeral_1h_input_tokens?: number };
				};
				this._logService.info(`[anthropic-cache] read=${cacheUsage.cache_read_input_tokens ?? 0} write=${cacheUsage.cache_creation_input_tokens ?? 0} (1h=${cacheUsage.cache_creation?.ephemeral_1h_input_tokens ?? 0} 5m=${cacheUsage.cache_creation?.ephemeral_5m_input_tokens ?? 0}) uncached=${cacheUsage.input_tokens}`);
